...
test("opts.ikeyJsonPath points the dispatcher at the caller's ikey.json (no env override)", async () => {
  ...
  // lib/ is shared (relative require or symlink), the spawner must forward the
  // plugin's real ikey.json via opts.ikeyJsonPath. With NO env override set, an
  // enabled ikey.json passed this way must still produce a probe.
  const tmp = mkTmp();
  const probe = path.join(tmp, "probe.json");
  const ikeyJsonPath = path.join(tmp, "ikey.json");
  fs.writeFileSync(
