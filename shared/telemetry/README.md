...
`skill_started` telemetry over the 1DS Common Schema 4.0 envelope.
A detached dispatcher child resolves the destination iKey and collector URL, then POSTs the event.
The hook that emitted it returns before the POST happens.
...
module.exports = {
  ...
  async resolve({ event, cfg, cloud, geoName, configDir }) { /* ... */ },
...
**Runtime and identity context (on every event):**
...
**Tool and agent versions (when available, otherwise omitted):**

- `orgId`, `tenantId` - Dataverse organization GUID and Entra tenant GUID from `pac auth who`
...
The PAC cloud and environment-geo classifications select a regional collector.
They are not included in the event payload.

**Per-event:**

- `skillName` (on every event)
- `eventInfo` - structured event data serialized as a JSON string on the wire.
  Power Pages populates `eventInfo.aadObjectId` with the signed-in principal's Entra object ID from `pac auth who` when available.

## What is NEVER sent

File paths, cwd, env vars, site names, Dataverse URLs, stack traces, `err.message` text, skill arguments, tool inputs, prompt text, usernames, and hostnames.
...
- **Default-on.**
  Telemetry is enabled by default with no first-run prompt.
  When PAC is authenticated, events can include the Dataverse organization ID, Entra tenant ID, and signed-in principal's Entra object ID described above.
...
│  ├─ pac-auth.js            # parses `pac auth who` for orgId, tenantId, objectId, cloud, and environment geo
