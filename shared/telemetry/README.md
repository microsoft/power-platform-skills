...
hook (~5ms when disabled, ~3-5s otherwise — incl. when the user opted out)
  ...
  └─ fireAndForget(event, opts)         ← shared/telemetry/lib/emit-spawn.js
       ...
       └─ spawn(emit-dispatcher.js, detached)   ← runs in background
            ...
            ├─ user opt-out (config.json choice OR env var POWER_PLATFORM_SKILLS_TELEMETRY_<PLUGIN>=off; config wins) → exit (mirror written, no POST)
...
- **Opt out for automation** via the per-plugin env var
  `POWER_PLATFORM_SKILLS_TELEMETRY_<PLUGIN>` (e.g.
  `POWER_PLATFORM_SKILLS_TELEMETRY_POWER_PAGES=off|on`). It mirrors the slash
  skill's transmission toggle (suppresses transmission only; the local mirror is
  still written) and is evaluated **below** a persisted `config.json` choice — a
  saved `/<plugin>:telemetry` selection always wins; the env var governs only when
  nothing is stored. The dispatcher reads it inside the transmission gate, after
  the local mirror is written, so parity with the slash skill is exact.
