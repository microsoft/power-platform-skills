...
The user invoked `/<plugin>:telemetry [on | off | status]` to control usage telemetry for this plugin.
Default to `status` when no argument is given.
...
- `off` stops transmission to Microsoft.
  **Nothing leaves the machine.**
- `on` re-enables transmission. The choice is **per-user and per-plugin** and
  takes effect on the next event (no restart).
- Telemetry records skill and runtime fields.
  When PAC is authenticated, it also records the Dataverse organization ID, Entra tenant ID, and signed-in principal's Entra object ID.
  It never includes file paths, prompts, tool inputs, site names, URLs, credentials, usernames, or hostnames.
