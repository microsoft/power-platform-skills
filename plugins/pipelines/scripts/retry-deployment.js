#!/usr/bin/env node
// Retry a failed deployment stage run.
// Usage: node retry-deployment.js --envUrl <url> --stageRunId <guid> [--tenant <id>]
...
async function main() {
  ...
  const result = await makeRequest({
    url: `${envUrl}/api/data/v9.1/RetryFailedDeploymentAsync`,
    method: 'POST',
    headers: {
  ...
  if (result.statusCode < 200 || result.statusCode >= 300) {
    ...
    console.error(JSON.stringify({ error: `Retry failed with status ${result.statusCode}`, details: errorData }));
    process.exit(1);
  }

  console.log(JSON.stringify({ status: 'success', stageRunId, message: 'Deployment retry initiated' }));
  process.exit(0);
}
