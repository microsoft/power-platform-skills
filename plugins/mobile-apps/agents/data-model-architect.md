...
| View/open PDF | Model a durable HTTPS URL when the app has one; local `file://` viewing with native PDF viewer 0.2.9+ needs no Dataverse column. `content://` and `blob:` remain unsupported. |
