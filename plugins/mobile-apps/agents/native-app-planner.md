...
| `native-pdf-viewer` | `@microsoft/extension-native-pdf-viewer` | `/add-native pdf-viewer` |
| `pen-input` | `@microsoft/extension-pen-input` | `/add-native pen-input` |
| `geolocation` | `@microsoft/powerapps-geolocation-control` | `/add-native geolocation` |
...
Do not propose `native-pdf-viewer` or `pen-input` unless the exact extension package is present in the template allowlist output (`@microsoft/extension-native-pdf-viewer` and `@microsoft/extension-pen-input`). Do not propose `geolocation` unless `@microsoft/powerapps-geolocation-control` is present, and only for continuous/background tracking or durable Dataverse upload — use one-shot `location` (`expo-location`) for a single foreground coordinate read. When proposing `geolocation`, record that its Dataverse target table must already exist and must be verified by `/add-native geolocation` (default entity set `msdyn_locationrecords`, or a custom `tableName` whose `fieldMap` columns exist). Do not propose `pdf-report` unless `expo-print` is present. Do not propose local share/open behavior for generated PDFs unless `expo-sharing` is present. If a user asks to view a PDF but only a local generated file URI will exist, do not use `native-pdf-viewer`; plan `pdf-report` plus `sharing` only when both `expo-print` and `expo-sharing` are present, or plan `pdf-report` plus Dataverse File upload when `expo-print` is present and retained storage is required. If neither package path is present, drop the PDF capability and add a transparency note.

PDF fallback order:
1. Existing HTTPS PDF URL + `@microsoft/extension-native-pdf-viewer` present -> `native-pdf-viewer`.
2. App-generated PDF + `expo-print` present -> `pdf-report`.
3. App-generated PDF + `expo-print` and `expo-sharing` present -> `pdf-report` plus `sharing` when local share/open is required.
...
- Do not use Power Apps extensions as generic replacements: PDF viewer is HTTPS viewing, pen input is ink/signature capture, generated reports are `expo-print`.
...
- `native-pdf-viewer` means opening an HTTPS PDF URL with `@microsoft/extension-native-pdf-viewer`. It never supports `file://`, `content://`, `blob:`, or `http://`.
- `pen-input` means signature/ink capture with `@microsoft/extension-pen-input`. It returns PNG data URI and needs a Dataverse Image/File/child-row target when persisted.
- `geolocation` means continuous/background GPS tracking with durable storage and inline Dataverse sync via `@microsoft/powerapps-geolocation-control`. Auth is MSAL-only; native uploads each fix to an existing Dataverse table (default entity set `msdyn_locationrecords`). It is distinct from one-shot `location` (`expo-location`). Plan it only for continuous tracking or durable upload, require `/add-native geolocation` to verify the target table exists before use, and never propose the `GeolocationExtension`/HostingSDK path.
...
| Storage/output target | `n/a`, `Dataverse Image`, `Dataverse File`, `child Evidence table`, `on-device/share-only`, or `HTTPS URL only` |
