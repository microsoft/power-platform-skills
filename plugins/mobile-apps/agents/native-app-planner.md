...
```bash
node -e "const p = require('${CLAUDE_PLUGIN_ROOT}/template/package.json'); const deps = Object.keys({...p.dependencies, ...p.devDependencies}); console.log(deps.filter(d => d.startsWith('expo-') || d.startsWith('react-native-') || d.startsWith('@react-native-community/') || d.startsWith('@powerapps/extension-')).join('\n'));"
```
...
| `native-pdf-viewer` | `@powerapps/extension-native-pdf-viewer` | `/add-native pdf-viewer` |
| `pen-input` | `@powerapps/extension-pen-input` | `/add-native pen-input` |
...
- `native-pdf-viewer` means opening an HTTPS PDF URL with `@powerapps/extension-native-pdf-viewer`. It never supports `file://`, `content://`, `blob:`, or `http://`.
- `pen-input` means signature/ink capture with `@powerapps/extension-pen-input`. It returns PNG data URI and needs a Dataverse Image/File/child-row target when persisted.
