...
- **Pure-JavaScript UI hints** — calendar management, scheduling, month/week/day views, agendas, appointment calendars, availability, visits, or routes by date should be passed to `screen-planner`. They are not native capabilities. The screen planner records any required JS-only package with an exact version under `## Screens → JavaScript Dependencies`.
