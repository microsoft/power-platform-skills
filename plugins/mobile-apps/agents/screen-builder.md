...
- **Read only your screen's spec — with one exception: `### Navigation Contracts`.** Open `native-app-plan.md`, find the section under `## Screens` → `### Per-Screen Specs` → your screen, and use only that for layout/data/state decisions. **However, you MUST also read the `### Navigation Contracts` table** (sibling section under `## Screens`) before writing any navigation call or `useLocalSearchParams<...>`. The contracts table is the ONE place cross-screen knowledge lives — it tells you what query/path params each route accepts and, when present, the stable semantic Destination ID and exact external parameter schema. Locked conventions: `?editId=<guid>` for create-or-edit forms (never `?id=` / `?recordId=`); `[id]` for primary entity path param; `[<entity>Id]` for nested entities; all route param values typed `string`. If a route you need is NOT in the table, STOP and report `BLOCKED [<screen_name>]: route <path> not in Navigation Contracts — need planner clarification on params.` Do not invent params.

- **Semantic navigation contract.** If `src/navigation/linkContract.ts` exists
  and the destination row has a Destination ID, import and call its typed
  `navigateTo({ schemaVersion: '1', destination, params })` helper for in-app
  navigation. Do not hand-build an Expo Router href for that destination.
  Direct `router.push`/`navigate`/`replace` remains valid only for a row whose
  Destination ID is `—` or before the shared contract has been generated.
  Never pass a raw route, URL, or unknown parameter to the semantic helper.
...
- **Native capabilities: use `src/native/` wrappers, NOT raw Expo modules.** `/add-native` creates typed wrappers under `src/native/` (e.g., `camera.ts`, `cameraUpload.ts`, `secureStore.ts`, `documentPicker.ts`, `pdfReport.ts`, `pdfViewer.ts`, `penInput.ts`, `geolocation.ts`), while `/add-push-notifications` creates `pushNotifications.ts`. For non-Dataverse native workflows, always import from these wrappers — never import Expo, React Native Firebase, or Power Apps native modules directly in screen files. The wrappers handle permissions, platform differences, URL validation, and discriminated-union results. Push permission and topic lifecycle are app-level concerns; screens may call the wrapper's preference actions but must not subscribe to topics or install listeners. `expo-haptics` remains unavailable.
