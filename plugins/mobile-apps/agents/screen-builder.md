...
- **Explicit Microsoft scanner controls** use [Microsoft OOB controls](${PLUGIN_ROOT}/skills/add-native/references/oob-controls.md) and the installed `src/native/nativeBarcodeScanner.ts` wrapper. Never import `@microsoft/power-apps-native-barcode-scanner` directly, install it from a screen builder, or render it as `BarcodeScannerView`. Its native API opens its own scanner. Keep generic barcode/QR and embedded camera screens on the existing Expo wrapper. If the approved Microsoft wrapper/dependency is missing, return `NEEDS_CONTEXT` to the orchestrator instead of inventing imports. Button launches and downstream writes need an in-flight guard and explicit cancellation/permission/error states.
