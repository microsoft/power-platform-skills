...
- **Use shared code via path aliases — NEVER re-define inline.** The project has `@/components`, `@/hooks`, `@/utils`, `@/tokens` configured in tsconfig. Import from them:
  - **Components:** `import { LoadingState, ErrorState, EmptyState, ScreenHeader, ModalHeader, BottomActionBar, FloatingActionButton, FilterChipRow, FormField, RowPick, PrimaryActionButton, SecondaryActionButton, DestructiveActionButton, StatusPill, AvatarInitials, InfoRow, ActionRow, SectionHeader } from '@/components'`
...
- **HARD RULE — Cross-entity Field Resolution.** Before writing the screen's `select: [...]` or load step, walk every UI field your spec displays. Follow [`shared/references/data-performance.md` § Cross-entity Reads](${PLUGIN_ROOT}/shared/references/data-performance.md#cross-entity-reads) mechanically.
  ...
  2. **For chained fetches, branch on archetype × cardinality:**
     ...
     |---|---|---|
     | List (`top ≥ 5`), Tab-root, Dashboard | 1:1 (N:1 lookup chain) | **STOP — do not create an N+1 fetch.** Use the direct formatted lookup or return `BLOCKED` for an external projection. |
     ...
     | Detail/Form/Modal | 1:many | Scaffold a bounded chained `<ChildService>.getAll({ filter: \`_<parentid>_value eq '${id}'\`, select: [...] })`. |
     | Any | M:N | Require an explicit generated intersect-table service/query contract; otherwise return `BLOCKED`. |

  3. **Verify before exit.** Every UI field must have a primary `select`,
     formatted lookup, or bounded chained fetch. Otherwise return `BLOCKED`.
...
Read `${PLUGIN_ROOT}/shared/references/mobile-visual-quality-contract.md`
before translating the spec. Its semantic typography, spacing, action,
surface, composition, and imagery rules are binding.
...
- **Composition recipe** — required for every generated/modified screen. Resolve
  the named recipe from `mobile-visual-quality-contract.md` and preserve its
  hierarchy. Do not replace it with a generic stack of cards.
- **Imagery contract** — when present, use only the approved source and apply
  its purpose, aspect/crop, fallback, and accessibility treatment. When absent,
  do not fetch stock imagery or invent a decorative hero.
...
3. **## Composition** — use the approved recipe family and screen mapping.

4. **## Imagery** — enforce source eligibility, crop, fallback, and accessibility.

5. **## Components** — follow component specs (button shapes, card styles, list row heights, badge treatments). These override per-screen spec defaults.

6. **## Negatives** — these are **HARD RULES**. Every line prefixed with ✗ is a forbidden pattern. Before returning your screen, verify against EVERY negative:
...
2. `brand/design-system.md` ## Components, ## Composition, ## Imagery, ## Palette, ## Typography
...
### Composition recipe → hierarchy

Resolve the screen's required recipe from
`mobile-visual-quality-contract.md`. Preserve the ordered hierarchy and map
domain content into it. Do not add sections merely to fill space, and do not
collapse a commerce, workflow, form, or confirmation recipe into generic
header + cards + button.

### Imagery contract → media behavior

When the spec includes imagery, verify all five fields before writing an image:
approved source, user purpose, aspect/crop, fallback, and accessibility. Use
`contentFit="cover"` only when cropping is approved; otherwise use `contain`.
Meaningful imagery gets an accessible label. Decorative imagery is hidden from
accessibility. If the source is unavailable, render the specified fallback
without leaving blank hero space.
...
7. **Button hierarchy** — Tamagui 2 does not forward text-style props from `Button` to its label. Primary actions use a confirmed Config v5 theme such as `<Button theme="blue">`, a shared action component, or explicit frame/text composition: `<Button bg="$blue10"><Button.Text color="$color1">Save</Button.Text></Button>`. NEVER put `color`, `fontSize`, or `fontWeight` directly on `Button`, and never use `theme="active"` / `theme="primary"` unless that exact theme is defined in the project's `tamagui.config.ts`. If host-theme token resolution still renders the CTA as pale or disabled-looking, move primary styling into a shared app button component with explicit brand-locked colors. Destructive actions use verified danger tokens or a confirmed theme; secondary actions use `variant="outlined"` or `chromeless`.
   Prefer `PrimaryActionButton`, `SecondaryActionButton`, and
   `DestructiveActionButton` from `@/components`. A direct primary `<Button>`
   must explicitly use the approved 48/52/56px minimum height and radius token.
...
15. **Typography** — if plan specifies font pairing, headings use `fontFamily="$heading"`, UI chrome uses `fontFamily="$body"`. Apply per-archetype rules from screen-templates.md "Typography by Archetype" table. Apply negative tracking (`letterSpacing` -0.5 to -1.0) on titles at `fontSize="$7"` and above.
    Use only the semantic role tokens from
    `mobile-visual-quality-contract.md`; never introduce raw numeric
    `fontSize` values in screen TSX.
