...
- `adapted_components`, `adapted_behavior_shard`, `adapted_pcf_plan`, `adapted_state`, `adapted_server_side_assets`, `adapted_localization`, `adapted_assets` — optional source contracts supplied only for Canvas/MSAPP modernization. Verbose screen/control formula files plus global behaviors, workflows, and control coverage are never supplied.
...
- **Adapted-mode required reads.** If any `adapted_*` path is supplied, read every supplied companion before writing TSX. Source-of-truth order is: (1) typed skeleton imports/hooks in `target_file`; (2) this screen's compact spec in `native-app-plan.md`; (3) component contracts and exact instance bindings; (4) this screen's `adapted_behavior_shard`, which contains compact screen intent, semantic `controlIntents[]`, exact core behavior, structured native intent hints, compact `workflowRefs[]`, and unmatched review formulas; (5) approved PCF dispositions; (6) state placement; (7) server-side write guards; (8) localization/assets. **Never read/request global `behaviors.json`, `workflows.json`, control coverage, or verbose screen/control formula files**—they are audit-only. A generic shell that ignores the shard is not a successful modernization.
...
- **PCF disposition is binding.** When `adapted_pcf_plan` is supplied, filter `controls[]` to this screen before coding. Every row must have `approval.status: approved`; return `BLOCKED:` for pending/blocked rows, unapproved package additions, missing generated-service dependencies, or essential PCFs marked unsupported. Implement exactly the approved disposition—never use the extractor's heuristic hint as authority and never host/copy PCF code. For `native-replacement` and `server-dependency`, place `// source-pcf: <pcfId> <disposition>` immediately above the owning native component/handler. For `explicit-unsupported`, render the exact approved visible `unsupportedUx` and place `// source-pcf-unsupported: <pcfId> — <reason>` immediately above it. Marker-only TODOs do not count.
