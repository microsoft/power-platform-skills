...
- `adapted_screen_plan`, `adapted_screen_controls`, `adapted_components`, `adapted_behaviors`, `adapted_workflows`, `adapted_control_intent_coverage`, `adapted_pcf_plan`, `adapted_state`, `adapted_server_side_assets`, `adapted_localization`, `adapted_assets` — optional source contracts supplied only for Canvas/MSAPP modernization
...
- **Adapted-mode required reads.** If any `adapted_*` path is supplied, read every supplied companion before writing TSX. Source-of-truth order is: (1) typed skeleton imports and hooks in `target_file`; (2) screen plan/controls for source workflow and control evidence; (3) component contracts and exact instance bindings; (4) normalized behaviors filtered to this screen; (5) approved pathological workflows filtered to this screen; (6) approved PCF dispositions filtered to this screen; (7) state-placement report; (8) server-side calculated/rollup/managed-column guards; (9) localization and asset manifests. A generic list/form shell that ignores the source behaviors is not a successful modernization.
...
- **Approved pathological workflows are binding shared code.** When `adapted_workflows` is supplied, filter `workflows[]` to this screen. Every row must be `approval.status: approved`; pending/blocked/unresolved decisions return `BLOCKED:`. The orchestrator has already generated each `proposal.target.module` under `src/features/`; import and invoke its exact `proposal.target.exportName` at the prewired `// source-workflow-call: <workflowId>` call site. Build native progress/result/retry UX around its typed result. Do not create/edit the module, inline its steps, duplicate its source operations or behavior markers, collapse it into one callback, or ask the user routine code/UX questions. If the module/import/call marker is missing, return `BLOCKED:` so the orchestrator repairs its owned prerequisite.
...
Skip when `adapted_behaviors` is absent. Filter `actions`, `visibility`, `validations`, and `derivations` to this screen. Preserve each action's `controlFlow[]` frames and `sourceStatement`; a leaf nested under a branch, loop, error fallback, or concurrent group must not become an unconditional handler. If `adapted_workflows` assigns an approved workflow to this screen, exclude its `source.behaviorIds[]` from direct translation here—the orchestrator-owned workflow module implements and marks those behaviors. This screen owns only the typed invocation and native UX around its result.
...
44. **Native date/time pickers — NEVER use a Sheet calendar or plain Input for date fields.** If a form field maps to a `datetime` or `DateOnly` Dataverse column, you MUST use `@react-native-community/datetimepicker` (already in template). The recipe in rule 27 (DateTimePicker component) is the ONLY acceptable pattern. The planner spec may say "date picker Sheet" or "date picker" — both mean the native picker, not a custom Tamagui Sheet with a calendar grid. Specifically forbidden:
  ...
  45. **Adapted-source accounting before return.** When migration companions were supplied, recount this screen's normalized behaviors, approved workflows, control-intent rows, and approved PCF rows. Confirm at least 80% of behavior entries are wired to real code or their approved shared workflow module, every workflow has its exact prewired invocation/result UX, every high-risk control-intent row has an implementation/approved unsupported UI/named block, and every PCF has its exact implementation marker or approved visible unsupported state. List every remaining unmatched formula, workflow call/module gap, PCF gap, missing component callback, missing asset byte, unsupported control, or unresolved backend in `DONE_WITH_CONCERNS:`. Do not return `DONE` for a visually complete screen that lost source business behavior.
