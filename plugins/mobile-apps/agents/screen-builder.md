...
- **Mode-specific example boundary.** Inspection, dispatch, audit, and
  work-order snippets in this reference demonstrate APIs and interaction
  patterns only. Use them only when the Product Experience Contract and screen
  spec select the matching employee/workflow surface. Never carry their status
  cards, operational copy, dashboard anatomy, or entity flow into discovery,
  booking, content, finance, healthcare, consumer, or UI-only screens.
...
- **Use shared code via path aliases — NEVER re-define inline.** The project has `@/components`, `@/hooks`, `@/utils`, `@/tokens` configured in tsconfig. Import from them:
  - **Components:** `import { LoadingState, ErrorState, EmptyState, ScreenShell, ModalHeader, BottomActionBar, FloatingActionButton, FilterChipRow, FormField, RowPick, StatusPill, AvatarInitials, InfoRow, ActionRow, SectionHeader, EntityImage } from '@/components'`
...
- **Prototype domain boundary takes precedence over generated-service rules.**
  When the work order or project contains
  `.tmp/prototype-domain-model.json` and `src/data/index.ts`, import domain
  types, query hooks, mutations, media resolution, and actionability helpers
  only from `@/data`. Never import `@/data/fixtures`,
  `@/data/repositories`, `@/generated`, a connector client, or call a
  repository directly from a screen. The same hook contract survives
  prototype-to-Dataverse conversion.
...
- **Route shell and header contract are mandatory.** The selected pack screen has a literal `headerMode`. Wrap every loading, error, empty, and populated branch in the same `<ScreenShell headerMode="<pack value>" title="...">`. `ScreenShell` is the only route-level `SafeAreaView` owner: do not import or render a second `SafeAreaView` inside an app route. `root` has no leading dismiss affordance; `back` uses the shell's stack-or-fallback behavior; `close` uses the shell's close behavior; `none` renders no header. Use a literal header mode so `validate-screen-shells.js` can prove it agrees with the pack. Do not combine `ScreenShell` with `contentInsetAdjustmentBehavior="automatic"`; the shell owns the insets.
...
- **Experience contract controls the primary screen.** Read
  `<working_dir>/.tmp/experience-contract.json` and
  `<working_dir>/.tmp/experience-screen-contract.json` before writing JSX.
  For the contract primary screen, its `userOutcome`, `compositionKind`, focal
  point, ordered region anchors, visible primary action, signature motifs, and
  forbidden defaults are hard requirements. Do not re-infer an industry,
  replace a discovery/capture/workflow/inbox/detail-first flow with a dashboard
  or List screen, or add an unapproved generic search/catalog/metric/card
  structure. A high/strict Reference Contract overrides generated composition
  only where it explicitly conflicts with the experience contract.
- **Primary-screen runtime anchors are mandatory.** When `target_file` matches
  the experience screen contract's primary file, render one literal `testID`
  for every ordered region (`experience-region-<normalized-region>`), one
  `testID="experience-primary-action"` on the visible action, and one literal
  `testID="experience-motif-<normalized-id>"` on each required motif. Keep
  the region anchors in visual source order. Do not hide them in a conditional
  branch that makes an approved primary composition disappear. If a marker is
  not applicable because a binding Reference Contract overrides it, update the
  screen contract through the planner; do not silently drop it here.
- **Foundation primitives are mandatory.** Read
  `<working_dir>/.tmp/experience-foundation-contract.json` together with the
  screen contract. For every `Foundation primitives` entry in your screen spec,
  import the exact component from `@/components/experience/<component>` and
  render it rather than recreating its motif with local JSX. The scaffold owner
  creates these files before builders launch. If the manifest, component file,
  or named export is missing, return `BLOCKED [<screen_name>]: required experience foundation primitive is missing.` Do not replace it with a generic
  Card, List row, or temporary inline component.
  **Local-first media is mandatory when contracted.** When
  `assetPolicy.media` is `local-first`, use bundled/local asset references and
  the foundation primitive's local fallback. Do not add `http://`, `https://`,
  Unsplash, or remote placeholder URLs to the primary screen/foundation files.
  Loading, empty, error, and offline states must preserve the selected discovery
  or workflow intent: explain how to continue browsing, recover a saved cart,
  or retry a local action rather than falling back to an operations/status page.
- **Screen build pack is the execution source.** Before reading broad Markdown
  guidance, validate and read `<working_dir>/.tmp/screen-build-pack.json`. Find
  your `screens[]` entry by route/file and use its purpose, first viewport,
  action, states, dependencies, test IDs, foundation primitives, fixture adapter,
  asset policy, `headerMode`, `data.viewModel`, `data.recordIdentity`,
  `data.mediaPolicy`, `data.mediaFields`, forbidden defaults, and revision as
  binding execution input.
  Markdown remains the readable source for lookup writes and detailed data
  contracts, not a second product-strategy source. If the pack is absent only
...
- **Canonical data identity is mandatory.** When the pack names
  ...
  it. Convert each service
  record once before rendering; use `experienceRecord.id` for `keyExtractor`,
  detail routes, mutations, and cart/save identity. Pass
  `assetKey={experienceRecord.assetKey}` and
  `assetRecipe={getExperienceAsset(experienceRecord.assetKey)}` plus
  `media={resolveExperienceMedia(experienceRecord)}` to `EntityImage`. For
  `remote-cdn-cached`, `EntityImage` owns device caching and its local fallback;
  screens never contain a literal CDN URL. Never create screen-local `PRODUCT_COPY`, index-keyed arrays,
  or title-derived identity. If the required view-model or local asset manifest
  is absent, return `BLOCKED [<screen_name>]: canonical experience data assets are missing.`
- **Category-to-catalog context is mandatory for discovery.** When a category
  affordance navigates to Catalog, read the exact `categoryId` or
  `categorySlug` parameter from Navigation Contracts, push that canonical value
  from the selected category record, and initialize Catalog's filter from it
  before rendering product content. Do not use a category row index, display
  label guess, or an unfiltered catalog fallback. If the route parameter is not
  in Navigation Contracts, return `BLOCKED [<screen_name>]: category navigation context is missing from Navigation Contracts.`
...
- **HARD RULE — Materialize global Profile access.** Read `pack.navigation.globalRoutePolicy`. When `profileAccess` is `header-action` and your screen has `navigation.role: durable-destination`, pass a labeled, 44-point `rightAction` to `ScreenShell` that navigates to the literal `profileRoute`. Use the person icon plus an accessibility label containing `Profile`; do not hide it in an unlabeled overflow menu. When Profile is a destination, rely on the labeled tab/drawer item instead and do not duplicate the header action.
- **HARD RULE — Sticky content clearance is calculated, not guessed.** For every `sticky-bottom` action, import `useBottomActionClearance` with `BottomActionBar`. Compute `contentBottomInset = useBottomActionClearance({ actionHeight: <actual bar height>, tabBarHeight: <64 when tabs are visible, otherwise 0>, spacing: 16 })`; apply it to the owning `ScrollView`/`FlatList` `contentContainerStyle.paddingBottom`. Render `BottomActionBar safeArea tabBarClearance="above"` when tabs remain visible. This formula reserves sticky action + tab bar + device bottom inset + spacing so the last card and active keyboard field can scroll fully above the controls.
...
- **Required reading before writing any TSX.** You MUST load these from the plugin root:
  ...
  - Read `<working_dir>/.tmp/experience-contract.json` and
    `<working_dir>/.tmp/experience-screen-contract.json` before choosing a
    layout. The contract primary composition and `forbiddenDefaults` override
    generic archetype samples. Consult `${PLUGIN_ROOT}/shared/references/universal-patterns.md`
    only when a narrowly relevant domain pattern supports an explicit contract
    requirement; industry labels never select a primary composition.
...
- **The Tamagui × Expo scope rule** (apply mechanically, no judgment calls):
  ...
  | Theme switching, breakpoints (`useMedia()`) | Scroll insets: use `contentInsetAdjustmentBehavior="never"` when `ScreenShell` owns route insets; `automatic` is reserved for a native Stack header selected by the pack |
...
- **Generated services are the real-app fallback data layer.** Apply the rules
...
- **Dataverse image rendering rule (detail/list screens).** If a screen displays a Dataverse Image column, include the real image/base64 field in `select` and use `<EntityImage source={rowData.imageField} />` to safely display it. For a contract-owned media model, render canonical presentation data instead: `<EntityImage assetKey={item.assetKey} assetRecipe={getExperienceAsset(item.assetKey)} media={resolveExperienceMedia(item)} />`. Do not rely on guessed URL/display pseudo-fields, and do not use raw `Image` components which are prone to XML parsing crashes (`[xmldom error]`) when fed CDN mockup URLs.
...
Before using that prose, read `screen_build_pack_path` and verify it with:

```bash
node "${PLUGIN_ROOT}/scripts/validate-screen-build-pack.js" \
  --project-root "<working_dir>" \
  --pack ".tmp/screen-build-pack.json"
```

The matching pack entry owns the compact execution fields. Confirm that its
`route`, `file`, `purpose`, `states`, dependencies, and test IDs agree with the
assigned task. Record the pack `revision` in your final summary. Only an
...
- **Experience composition** (required on the contract primary screen) — read
  ...
  signature motifs, forbidden defaults, and `experience-*` runtime anchors.
  If the assigned primary screen lacks this field or conflicts with either
  experience sidecar, return `BLOCKED [<screen_name>]: primary experience composition is missing or stale in native-app-plan.md.` Do not improvise a default layout.
- **Foundation primitives** (required where present) — use the exact
  manifest-selected components and imports. A motif marker may live inside its
  foundation component, while regions and the visible primary action remain in
  the primary screen's own JSX order.
...
- Layout delta — combine the screen-specific layout delta with the archetype pattern, Shared Conventions, Product Experience Contract, and Design Translation. A compact spec is intentional; absence of universal chrome/layout rules is not a missing requirement.
...
**Design fields (per-screen — OVERRIDE-ONLY; absence = inherit from the Product Experience Contract):**

The planner emits these fields ONLY when a screen intentionally breaks from the app-level default. **If a field is absent from your spec, that is by design — read the corresponding value from `## Design → Product Experience Contract`, `### Design Translation`, and the experience sidecars.** Do NOT treat absence as a gap or `BLOCKED` reason; the inheritance is the contract.

Compact specs are expected. Before reporting `NEEDS_CONTEXT`, first check `### Shared Conventions`, `## Design → Product Experience Contract`, `brand/design-system.md`, the archetype references, and universal builder rules. Only block when a genuinely app-specific fact is missing (service, route, lookup binding, destructive behavior, required native capability, or user-facing copy that cannot be inferred from the entity/domain).
...
- **Density mode** (if present) — `sparse` / `balanced` / `dense`. If absent, use the contract first-viewport density.
...
## Step 1a — Read Product Experience Design Constraints

After reading your per-screen spec, read `<working_dir>/.tmp/experience-contract.json`, `<working_dir>/.tmp/experience-screen-contract.json`, and `<plan_path> → ## Design`. Use these values as defaults wherever the per-screen spec is silent:

- `visualCharacter`, `audience`, `interactionMode`, and `contentDensity` →
  token, typography, tone, and surface constraints
- `firstViewport` → ordered composition, focal point, and primary-action
  placement; samples cannot move a region ahead of it
- `signatureMotifs` + `forbiddenDefaults` → required/forbidden components and
  hierarchy
- `Reference Contract` / brand primitives → binding reference or component
  details where present

Per-screen spec values always win only when they are compatible with the
experience contract. The contract is the fallback for fields the per-screen
spec does not pin.

If the plan lacks a Design section but the experience sidecars are present,
use the sidecars plus `mobile-design-philosophy.md` and `screen-templates.md`.
If the sidecars are missing, return `BLOCKED`; do not infer a dashboard or
industry-default design.
...
5. **## Product Experience Primitives** — implement the named signature motifs
  before generic List, Form, or Detail adornments. The primary screen must
  retain its composition kind and exact first-viewport order.

6. **## Motion** — follow the motion policy. If "Forbidden: spring bounces" → do not use spring animations.

**If it does NOT exist**, keep the experience contract and screen contract as
the product source of truth, then use generated semantic Tamagui aliases. Do not infer an industry preset or replace the primary composition.

**Priority order (highest wins):**
1. `brand/design-system.md` ## Negatives (absolute — never violated)
2. Binding Reference Contract (when present)
3. Experience contract primary composition, motifs, and forbidden defaults
4. `brand/design-system.md` ## Components, ## Product Experience Primitives, ## Palette, ## Typography
5. Per-screen spec values from the plan when contract-compatible
...
| atmospheric | Use the packed `ScreenShell` header mode and a spacious title treatment inside its content. Do not opt into automatic scroll insets because the route shell owns them |
| cinematic | Detail screens use packed `headerMode="back"` or `"close"` in `ScreenShell`; do not rebuild an inline back button or a second safe-area wrapper. Navigation chrome must not compete with content |
...
>   • Design tokens: <brand/design-system.md | Product Experience Contract | semantic defaults>"
...
38. **Mobile chrome from screenshots** — headers/titles never clip under the status area; use one `ScreenShell` around every loading/error/populated branch. FABs, snackbars, and sticky CTAs use `b={insets.bottom + 16}` or `b={tabBarHeight + insets.bottom + 16}` rather than a bare `b={16}`. `ScreenShell` owns `edges={['top', 'bottom']}`; do not nest another `SafeAreaView` inside it.
...
37. **`ScreenShell` owns the bottom edge whenever the route has bottom-anchored UI.** It uses `edges={['top', 'bottom']}` for forms with action rows, lists that clear the home indicator, and detail CTAs. Do not add a nested `SafeAreaView`; embedded tab bars reserve their own space.

  For full-screen capture/scanner modals, the header row (`Cancel`, title, close icon) MUST be inside the same `ScreenShell` as the preview/content. Never render modal header chrome above/outside the route shell.
...
40. **Tab-root List screens MUST use `headerLargeTitle` + `headerSearchBarOptions`.** Never build a custom Tamagui header with a manual `Input` search bar for Tab-root List screens. Use native iOS navigation chrome:
    ...
    The large title collapses on scroll (iOS Settings / Mail / App Store). On Android it degrades to a standard toolbar. This native-header recipe applies only when a screen pack explicitly selects native header ownership; otherwise ScreenShell owns the header and route insets. Detail and Form screens use their packed `ScreenShell` header mode rather than an ad hoc stack-header override.
