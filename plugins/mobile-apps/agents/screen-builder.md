...
- **Adapted-mode required reads.** If any `adapted_*` path is supplied, read every supplied companion before writing TSX. Source-of-truth order is: (1) typed skeleton imports/hooks in `target_file`; (2) this screen's compact spec in `native-app-plan.md`; (3) component contracts and exact instance bindings; (4) this screen's `adapted_behavior_shard`, which contains compact screen intent, semantic `controlIntents[]` (including approved PCF projections), exact core behavior, structured native intent hints, compact `workflowRefs[]`, and unmatched review formulas; (5) state placement; (6) server-side write guards; (7) localization/assets. **Never read/request global `behaviors.json`, `workflows.json`, `pcf-plan.json`, control coverage, or verbose screen/control formula files**—they are audit-only. A generic shell that ignores the shard is not a successful modernization.
...
Skip when `adapted_behavior_shard` is absent. The shard is already scoped to this screen; do not seek a global ledger. Its `actions`, `visibility`, `validations`, and `derivations` are exact load-bearing behavior selected by conservative backward dependency closure. Preserve each action's `controlFlow[]` frames and `sourceStatement`; a leaf nested under a branch, loop, error fallback, or concurrent group must not become unconditional. Its `intentHints[]` are disconnected Canvas UI plumbing converted to structured native outcomes—implement the outcome, not old temporary variables or choreography. Exclude every behavior listed by `workflowRefs[].coreBehaviorIds` from direct screen translation—the orchestrator-owned module implements and marks it. This screen owns the typed invocation and native UX/result intents around it.
...
- Use the hint's `guidance` and `nativeIntent`; do not invent source formulas that were deliberately omitted.
...
- If the hint appears in `workflowRefs[].intentHintIds`, implement it at the prewired workflow call site unless the orchestrator module already contains its marker.
