...
- **Finite kit only.** Compose from `@/components`. Do not create `InspectionRow.tsx`, `GroceryHero.tsx`, or any other app-specific widget. If `src/components/` already has a leftover entity file, prefer the kit (`EntityRow`, `ImageHero`) instead of extending it.
...
- **Required reading before writing any TSX.** Load only:
  - `${PLUGIN_ROOT}/shared/samples/src/components/index.tsx` — the finite kit you must compose from
  - `${PLUGIN_ROOT}/shared/references/screen-templates.md` — short kit API + Home stack
  - `${PLUGIN_ROOT}/shared/references/accessibility-checklist.md` — universal a11y rules
  - The matching sample for import/API idioms only (not layout):
  ...
  Do **not** read `mobile-design-philosophy.md`, `mobile-ui-patterns.md`, `tamagui-component-recipes.md`, `universal-patterns.md`, `typography-and-tone.md`, `color-palette-architecture.md`, or vibe brand-clone files. Tokens and `## Design Direction` already carry visual decisions.
...
**Functional fields:**
- **Domain layout decisions** — read this block FIRST (added by screen-planner). It answers: what data fields matter most, what is the visual emphasis, and which kit components + props make this screen this domain. These answers are binding — they override any generic "use the sample" instinct. If this block is absent, bind the Home stack or the matching archetype shell from the kit API before writing any JSX.
...
- **Kit bind** — required. Compose only the named `@/components`. If the spec still has legacy `Row style` / `Hero type` / `Operational pattern` keys, map them with the table in `screen-templates.md` and do not reopen the old prose files.
- Row bind (List screens) — if omitted, inherit `EntityRow` variant from `### Shared Conventions`; if no shared default exists, pick the variant that surfaces the entity's key fields. Do NOT invent a new row component.
- Hero bind (Detail screens) — if omitted, inherit `Hero` / `ImageHero` from `### Shared Conventions`; if no shared default exists, pick the hero that fits the entity.
...
**If no `## Design Direction` section exists**, reason from the brief and pick `saas` / `product` / `polished-inspection`, then use `brand/tokens.ts` plus the kit API. Do not block, do not search the internet, and do not invent a 25th component. Use a brand/design doc only if the user supplied one.
...
5. kit defaults in `screen-templates.md` + `brand/tokens.ts`
...
4. **Touch targets** are at ≥ 48pt (use `hitSlop` on small elements). Counts/qty/sets/kg use `NumericStepper`, not a keyboard, when the value is bounded.
...
14. **Anti-patterns** — no centered text in content flows, no unnecessary cards, no engineer-facing strings. Compose kit components; do not invent a new widget to avoid these.
15. **Typography** — if the plan specifies a font pairing, headings use `fontFamily="$heading"`, UI chrome uses `fontFamily="$body"`. Apply negative tracking (`letterSpacing` -0.5 to -1.0) on titles at `fontSize="$7"` and above.
16. **Copy tone** — if the plan specifies a tone profile, all UI copy (empty states, errors, buttons, confirmations) matches that tone. No exclamation marks, no emoji, no "Submit"/"OK".
...
21. **Dark mode quality** — dark mode is a designed inversion, not a raw swap. Background should be near-black with hue tint (not pure `#000`), text should be warm cream (not pure `#fff`), accent colors brighten, and shadows are replaced with surface elevation (`$color3` on `$color2`). Follow `brand/tokens.ts` and the plan's `## Design Direction`; do not open palette architecture docs.
...
33. **Domain differentiation** — could this screen belong to a different app unchanged? If yes, it is too generic. Bind domain into kit props (title, image URL, meter variant, CTA, row fields). An inspection `EntityRow variant="status"` looks different from a project `StatTile` + `EntityRow variant="sentence"` because the fields differ, not because a new component exists.
34. **Kit bind, not CRUD shells** — implement the spec's named `@/components`. Home uses the Home stack. Lists use `EntityRow`. Details use `Hero` or `ImageHero`. If a legacy plan still has `Operational pattern` / `Row style` / `Hero type` keys, map them with the table in `screen-templates.md` and stop. Never invent a 25th component.
...
38. **Mobile chrome from screenshots** — headers/titles never clip under the status area; use `SafeAreaView` or `pt={insets.top}` in every loading/error/populated branch. FABs, snackbars, and sticky CTAs use `b={insets.bottom + 16}` or `b={tabBarHeight + insets.bottom + 16}` rather than a bare `b={16}`. On tab-root screens (Home / Shop / Cart / You) that sit above an Expo `Tabs` bar, use `<SafeAreaView edges={['top']}>` plus `<BottomActionBar safeArea={false}>`. Tabs already own the home-indicator inset — adding it again on the bar creates a blank band. On stack/modal screens that cover the tab bar, keep `BottomActionBar`'s default `safeArea` (true) so the CTA clears the home indicator.
...
43. **Header and footer are required chrome.** Every screen file includes a title and a sticky next step. Title: `<ScreenHeader title="…">` on tab-root/list, `<ModalHeader>` on modal/form, or `<Stack.Screen options={{ headerShown: true, title }} />` on a pushed detail. **`ScreenHeader` sits above the scroll, never inside it** — render it as a sibling of `ScrollView`/`FlatList` so the title does not disappear. Footer: `<BottomActionBar>` with one labeled CTA (tab-root: `safeArea={false}`). Do not leave Sign out, Checkout, or Open cart as a button mid-scroll. Login/onboarding may keep template chrome. If `BottomActionBar` is present, omit `action=` on `Hero` / `ImageHero` and keep mid-screen buttons outlined or chromeless — two filled CTAs fail the one-focus rail. Header actions are for Back/Cancel or lightweight secondary only. Never add a Twitter-style `+` FAB over tabs.
44. **Bottom actions clear the tab bar and home indicator.** Use shared `BottomActionBar` for sticky form/review/sign-off actions. Tab-root: `<BottomActionBar safeArea={false}>` because `Tabs` already pads the home indicator. Modal/stack that covers tabs: default `safeArea` (true). If custom bottom UI is required, use `pb={insets.bottom + 20}` or `b={insets.bottom + 20}` on stack/modal screens only, and keep at least `$4` visual breathing room above tab/native chrome.
...
46. **Create actions use the right label shape.** On Home/dashboard/workflow screens, visible primary CTAs are labeled (`New inspection`, `Start walkaround`) and live in `BottomActionBar`. Do not use an icon-only `+` as the only Home CTA. **Never render `FloatingActionButton` on a tab-root screen** — it sits on the tab bar and steals taps. If a pushed list/queue (tab bar hidden or not present) needs a create affordance, an icon-only `FloatingActionButton` is acceptable only when it is the single obvious create action; it MUST still pass `label="New <entity>"` so the component can expose an `aria-label`. If there are multiple create-like actions or the entity is not obvious, use `extended` so the visible button reads `+ New <entity>`.
47. **Filter / sort chips are a horizontal scroller.** Use `FilterChipRow`. Chip count is unbounded — 2, 7, or 20 is fine; do not invent a 5-chip cap or wrap into a grid. Chips are controls, not cards: height 40pt, label `fontSize="$2"`, modest horizontal padding, no equal-width expansion. Include counts when available (`Ready 3`). Sort is another chip or `RowPick`, not a new widget. Photo carousels are a horizontal `ScrollView` of `EntityImage` / `ImageHero`.
47b. **Qty / line rows stay one grammar.** When a list row has a count plus a destructive action, wrap each line in a soft card (`YStack` `bg="$surface1"` `rounded="$4"` `borderWidth={1}` `borderColor="$borderColor"` `overflow="hidden"`). Gap between cards — no `ItemSeparatorComponent` hairline. Inside: `EntityRow variant="media"` (thumb, name, price right, meta under name) then a full-width `XStack justify="space-between"` with `NumericStepper` left and the text action right. No `flexWrap`. Warning text under that row. Do not create a 25th component for + / − / Remove / availability / Card — those are `YStack` + `NumericStepper` + chromeless text + `Callout` / caption.
47c. **Button shapes stay one grammar.** `BottomActionBar` primary: `rounded="$10"` pill. Secondary: chromeless text under it. `FilterChipRow` / `NumericStepper`: pill or circle with small type. Mid-screen filled actions: `rounded="$4"`. Destructive: text + 16pt icon. Never two pill primaries. Never a square primary next to a pill elsewhere.
47d. **Photo-led catalogs are tiles, not rows.** If the list is merch / food / look, render `numColumns={2}` tiles: `EntityImage` + overlay `StatusPill` + name + price. Bind existing kit only. Use `EntityRow` for queues, carts, and work lists.
...
54. **Audit/history screens look chronological.** Use `EntityRow variant="timeline"` with timestamp + actor + action, and optional grouping/toggle. Do not render audit as untitled generic cards.
...
37. **`SafeAreaView` bottom edge follows who owns the home indicator.** Tab-root screens (and nested stack screens that still show the `Tabs` bar) MUST use `edges={['top']}` plus `<BottomActionBar safeArea={false}>`. Do **not** add `edges={['top', 'bottom']}` there — that plus the tab bar plus a default `BottomActionBar` is the blank-band bug. Use `edges={['top', 'bottom']}` or default `BottomActionBar` only on stack/modal screens that cover the tab bar (checkout, capture, full-screen form). The default (`edges` omitted = all four) is acceptable only when no `Tabs` chrome is visible.
