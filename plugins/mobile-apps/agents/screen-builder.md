...
- `adapted_screen_plan`, `adapted_screen_controls`, `adapted_components`, `adapted_behaviors`, `adapted_control_intent_coverage`, `adapted_pcf_plan`, `adapted_state`, `adapted_server_side_assets`, `adapted_localization`, `adapted_assets` — optional source contracts supplied only for Canvas/MSAPP modernization
...
- **Adapted-mode required reads.** If any `adapted_*` path is supplied, read every supplied companion before writing TSX. Source-of-truth order is: (1) typed skeleton imports and hooks in `target_file`; (2) screen plan/controls for source workflow and control evidence; (3) component contracts and exact instance bindings; (4) normalized behaviors filtered to this screen; (5) approved PCF dispositions filtered to this screen; (6) state-placement report; (7) server-side calculated/rollup/managed-column guards; (8) localization and asset manifests. A generic list/form shell that ignores the source behaviors is not a successful modernization.
...
44. **Native date/time pickers — NEVER use a Sheet calendar or plain Input for date fields.** If a form field maps to a `datetime` or `DateOnly` Dataverse column, you MUST use `@react-native-community/datetimepicker` (already in template). The recipe in rule 27 (DateTimePicker component) is the ONLY acceptable pattern. The planner spec may say "date picker Sheet" or "date picker" — both mean the native picker, not a custom Tamagui Sheet with a calendar grid. Specifically forbidden:
  ...
  45. **Adapted-source accounting before return.** When migration companions were supplied, recount this screen's normalized behaviors, control-intent rows, and approved PCF rows. Confirm at least 80% of behavior entries are wired to real code, every high-risk control-intent row has an implementation/approved unsupported UI/named block, and every PCF has its exact implementation marker or approved visible unsupported state. List every remaining unmatched formula, PCF gap, missing component callback, missing asset byte, unsupported control, or unresolved backend in `DONE_WITH_CONCERNS:`. Do not return `DONE` for a visually complete screen that lost source business behavior.
