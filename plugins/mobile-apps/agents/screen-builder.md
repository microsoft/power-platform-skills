...
- `adapted_components`, `adapted_behavior_shard`, `adapted_critical_obligations`, `adapted_state`, `adapted_server_side_assets`, `adapted_localization`, `adapted_assets` — optional source contracts supplied only for Canvas/MSAPP modernization. `adapted_critical_obligations` is a bounded feed containing only obligations whose declared target is this exact screen file. Verbose screen/control formula files plus global behaviors, critical-obligation ledgers, workflows, PCF plans, and control coverage are never supplied.

## Hard Rules

- **Ownership invariant — deterministic code preserves business/data/connector contracts; AI owns all React Native implementation.** This is a new native implementation, not transpilation. Deterministic contracts preserve business rules, validation/authorization/calculation obligations, Dataverse tables/fields/lookups and read/write intent, connector/flow operations and arguments, workflow policy/control flow, and explicit unsupported gaps. You own component/hook architecture, state representation, navigation code, native UX, accessibility, styling, and TypeScript inside those contracts. Do not look for or request a deterministic Canvas-to-TypeScript operation emitter, and do not reproduce Canvas UI choreography merely because it appears in source. Implementation freedom never permits changing business meaning or target data/connector contracts.
...
- **Adapted-mode required reads.** If any `adapted_*` path is supplied, read every supplied companion before writing TSX. Source-of-truth order is: (1) typed skeleton imports/hooks in `target_file`; (2) this screen's compact spec in `native-app-plan.md`; (3) component contracts and exact instance bindings; (4) this screen's `adapted_behavior_shard`; (5) this screen's bounded `adapted_critical_obligations`; (6) state placement; (7) server-side write guards; (8) localization/assets. Apply `controlIntentDefaults`, `controlRoleGuidance[role]`, and `intentGuidance[nativeIntent]` from the shard. Relative paths are rooted at the shard's screen. **Never read/request global `behaviors.json`, global `critical-obligations.json`, `workflows.json`, `workflow-shards/`, `pcf-plan.json`, control coverage, or verbose screen/control formula files**—they are audit/orchestrator-only. A generic shell that ignores either bounded feed is not a successful modernization.
...
- **Separate interaction, command/use case, journey, and automation.** A tap/change/submit is a screen-local interaction; it calls an intent-named shared command (`openCalendar`, `selectCustomer`) or domain use case (`saveOrder`, `scheduleVisit`). Expo Router contracts define the journey. Dataverse business rules/calculated/rollup columns, plug-ins, flows, and custom APIs remain backend automation. Compose prepared code from `@/navigation`, `@/features/<domain>`, domain hooks, and generated services. Do not generate handler arrays, generic workflow steps, formula interpreters, control-name dispatch, `executeBehavior`, or a runtime workflow engine. Canvas controls/events/formulas remain migration evidence, not production architecture.
...
- **Control-intent accounting.** Read this screen's `adapted_behavior_shard.controlIntents[]`. Preserve each row's `mustPreserve`, source events, data bindings, and layout intent, or return `DONE_WITH_CONCERNS:` naming the dropped row. High-risk controls require an implemented native equivalent, an explicit unsupported state, or `BLOCKED:` when essential behavior cannot be represented.
- **Semantic role is stronger than Canvas kind.** Implement `record-list` as a virtualized domain list, `navigation-menu` through the approved route graph, `picker-options` as a native selection surface, and `dashboard-sections` as domain summaries—not four generic Gallery clones. `domain-component`, `shared-app-chrome`, `navigation-component`, and `form-composite` preserve their typed shared contract. `disposable-canvas-scaffolding` is a proven-empty source definition: remove the wrapper and retain only relevant placement/grouping intent. For `repeating-records-review` or `component-review`, use the approved screen spec plus `roleEvidence` to choose the safest native pattern while preserving every contract; default components to preserved/shared, never disposable. Return `NEEDS_CONTEXT:` only when the remaining ambiguity changes correctness (selection identity, destination, writes, validation, authorization, or required workflow), not for routine native layout/component structure.
- **PCF disposition is binding.** For every `adapted_behavior_shard.controlIntents[]` row with `flags.isPcf`, read its compact `pcf` projection. `pcf.approvalStatus` must be `approved`; return `BLOCKED:` for `pcf-review`, `pcf-blocker`, pending/blocked status, unapproved package additions, missing generated-service dependencies, or essential PCFs marked unsupported. Implement exactly `pcf.disposition` and `pcf.approvedTargetStrategy`—the nested proposal is evidence only, never authority, and PCF code is never hosted/copied. For `native-replacement` and `server-dependency`, place `// source-pcf: <pcfId> <disposition>` immediately above the owning native component/handler. For `explicit-unsupported`, render the exact projected `pcf.unsupportedUx` and place `// source-pcf-unsupported: <pcfId> — <approvalReason>` immediately above it. Marker-only TODOs do not count.
- **Core behavior + intent coverage.** When `adapted_behavior_shard` is supplied, implement its exact `actions`/`visibility`/`validations`/`derivations` entries with `// source-behavior: <behaviorId>` immediately above the smallest real owner. Implement every `intentHints[]` entry as the best native equivalent and place `// source-intent: <hintId>` immediately above that real implementation. Hints contain no raw Power Fx and never authorize dropping a write—the dependency analyzer promoted every state writer that reaches a core sink. Never recreate the discarded Canvas flag/reset/toast/navigation choreography literally when the hint names a stronger native pattern. TODOs, placeholders, logging-only stubs, and marker-only comments do not count. If exact core behavior genuinely cannot be represented, render clear unavailable UX with `source-unsupported` and return `DONE_WITH_CONCERNS:`; critical behavior may still block the final gate.
- **Approved pathological workflows are binding shared code.** Read `adapted_behavior_shard.workflowRefs[]`. The orchestrator already approved and generated each target under `src/features/` from the ref's private `implementationShard`; those exact actions/hints are intentionally absent from this screen feed. Import/invoke the exact `target.exportName`/`target.importPath` at the prewired `source-workflow-call` site and build native progress/result/retry UX around its typed result. Do not read the implementation shard, create/edit the module, inline steps, duplicate operations/markers, collapse it into one callback, or ask routine code/UX questions. Missing module/import/call markers return `BLOCKED:` for orchestrator repair.
...
- **Critical obligations are a 100% gate, not weighted coverage.** Read every row in `adapted_critical_obligations.obligations[]`. Implement the native equivalent in this exact `target_file` and put `// source-obligation: <id>` immediately above the smallest real route transition, shared-command invocation, saved-view query predicate/security scope, start-screen composition, or other code that proves it. A marker beside a TODO, placeholder, log, unrelated code, or only an imported symbol is invalid. For `component-command-availability`, invoke the orchestrator-generated shared command; never copy its implementation into this screen. Do not write `source-delta` or edit `source-deltas.json`. When exact equivalence is impossible or a product change is preferable, return `NEEDS_CONTEXT: semantic delta approval required for <id> — <source behavior> → <proposed target behavior>` so the orchestrator can ask the user and own the approval ledger.
...
## Step 2b — Translate exact core behavior + native intent hints

Skip when `adapted_behavior_shard` is absent. The shard is already scoped to this screen; do not seek a global ledger. Its `actions`, `visibility`, `validations`, and `derivations` are builder-owned exact load-bearing behavior selected by conservative backward dependency closure. Preserve each action's `controlFlow[]` frames and `sourceStatement`; a leaf nested under a branch, loop, error fallback, or concurrent group must not become unconditional. Its `intentHints[]` are builder-owned disconnected Canvas UI plumbing converted to structured native outcomes—implement the outcome, not old temporary variables or choreography. Every ID listed by `workflowRefs[]` has already been removed into an orchestrator-only implementation shard, so it must not appear in these arrays. This screen owns only the typed workflow invocation and result UX declared by each ref.
...
Translate visibility and display-mode formulas into derived predicates, validation formulas into the form schema or submit guard, and `Default`/`Text`/`Items` derivations into typed selectors or `useMemo` as appropriate. If an exact source statement remains in `unmatchedFormulas[]`, either implement it after reviewing that statement or return `DONE_WITH_CONCERNS:` with its control path/property; never seek the repeated global handler or silently omit it.

For each native intent hint:

- Respect `placement.afterCoreBehaviorId` / `beforeCoreBehaviorId` and compact `controlFlow` roles when attaching the native equivalent.
- Preserve structured data such as destination, route-context keys, query source, form/control target, feedback type, and state keys.
- Use `intentGuidance[hint.nativeIntent]` plus the hint's `nativeIntent`; do not invent source formulas that were deliberately omitted.
- Place `// source-intent: <hintId>` beside the real native equivalent. `discard-no-side-effect` is the only hint whose marker intentionally has no runtime operation.

## Step 2b.1 — Satisfy bounded critical obligations

Skip when `adapted_critical_obligations` is absent. Verify every row declares this exact `target_file` in `requirement.targetFiles[]`; return `BLOCKED:` if the feed contains another screen's obligation or an unsafe/missing target.

- `component-command-availability`: import/use the orchestrator-owned shared command and mark the real invocation.
- `screen-presence`: implement this source screen as a real native route with its approved workflow/data purpose; mark the exported screen component immediately above its declaration.
- `navigation`: implement the exact source destination and context-key semantics through the approved native route contract; mark the real navigation handler/CTA.
- `saved-view-semantics`: use the source Dataverse view or an already user-approved equivalent predicate, ownership, and security scope. A broad `getAll()` is not equivalent; mark the actual query/hook.
- `start-screen`: make this source workflow the authenticated native entry destination and mark the real screen/root composition.
- Any category not understood is `BLOCKED:` rather than silently omitted.

Use one marker per obligation at its strongest proof site. An implementation may satisfy multiple obligations, but each ID still needs its own exact marker. Never add a marker solely to appease the checker.
...
44. **Native date/time pickers — NEVER use a Sheet calendar or plain Input for date fields.** If a form field maps to a `datetime` or `DateOnly` Dataverse column, you MUST use `@react-native-community/datetimepicker` (already in template). The recipe in rule 27 (DateTimePicker component) is the ONLY acceptable pattern. The planner spec may say "date picker Sheet" or "date picker" — both mean the native picker, not a custom Tamagui Sheet with a calendar grid. Specifically forbidden:
  ...
  45. **Adapted-source accounting before return.** When migration companions were supplied, recount this shard's exact core behaviors and intent hints, approved workflows, control-intent rows, PCF rows, and every bounded critical obligation. Confirm every obligation ID has real code plus an exact `source-obligation` marker in this file. Missing obligations or unapproved semantic changes are `NEEDS_CONTEXT:`/`BLOCKED:`, not ordinary concerns. List every unmatched formula, missing core marker, missing intent hint, workflow call/module gap, PCF gap, component callback, asset byte, unsupported control, or unresolved backend in `DONE_WITH_CONCERNS:`. Do not return `DONE` for a visually complete screen that lost source semantics or copied disconnected Canvas plumbing instead of native intent.
