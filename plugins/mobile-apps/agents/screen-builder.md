...
- `adapted_components`, `adapted_behavior_shard`, `adapted_state`, `adapted_server_side_assets`, `adapted_localization`, `adapted_assets` — optional source contracts supplied only for Canvas/MSAPP modernization. Verbose screen/control formula files plus global behaviors, workflows, PCF plans, and control coverage are never supplied.
...
- **Adapted-mode required reads.** If any `adapted_*` path is supplied, read every supplied companion before writing TSX. Source-of-truth order is: (1) typed skeleton imports/hooks in `target_file`; (2) this screen's compact spec in `native-app-plan.md`; (3) component contracts and exact instance bindings; (4) this screen's `adapted_behavior_shard`, which contains compact screen intent, semantic `controlIntents[]` (including approved PCF projections), builder-owned exact core behavior, builder-owned native intent hints, compact `workflowRefs[]`, and unmatched review statements; (5) state placement; (6) server-side write guards; (7) localization/assets. Apply `controlIntentDefaults` plus the matching `controlRoleGuidance[role]` to compact control rows, and `intentGuidance[nativeIntent]` to compact hints. Relative `path`/`controlPath` values are rooted at the shard's `screen`. **Never read/request global `behaviors.json`, `workflows.json`, `workflow-shards/`, `pcf-plan.json`, control coverage, or verbose screen/control formula files**—they are audit/orchestrator-only. A generic shell that ignores the shard is not a successful modernization.
...
44. **Native date/time pickers — NEVER use a Sheet calendar or plain Input for date fields.** If a form field maps to a `datetime` or `DateOnly` Dataverse column, you MUST use `@react-native-community/datetimepicker` (already in template). The recipe in rule 27 (DateTimePicker component) is the ONLY acceptable pattern. The planner spec may say "date picker Sheet" or "date picker" — both mean the native picker, not a custom Tamagui Sheet with a calendar grid. Specifically forbidden:
  ...
  45. **Adapted-source accounting before return.** When migration companions were supplied, recount this shard's exact core behaviors and intent hints, plus approved workflows, control-intent rows, and PCF rows. Confirm every critical core entry has real code or approved unsupported UX, every intent hint has its real `source-intent` native equivalent, every workflow has its exact invocation/result UX, every high-risk control-intent row is represented, and every PCF has its required marker/state. List every unmatched formula, missing core marker, missing intent hint, workflow call/module gap, PCF gap, component callback, asset byte, unsupported control, or unresolved backend in `DONE_WITH_CONCERNS:`. Do not return `DONE` for a visually complete screen that lost source business behavior or copied disconnected Canvas plumbing instead of its native intent.
