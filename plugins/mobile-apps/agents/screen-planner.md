...
| `/(app)/inspections/[id]` | `inspection-detail` | `id: string` | — | `inspectionId: guid` | yes | `push` | `router.back()`; parent re-fetches via useFocusEffect |
| `/(app)/inspections/form` | — | — | `editId?: string` (omit for create, set for edit) | — | yes | `navigate` | `router.back()`; parent re-fetches via useFocusEffect |
| `/(app)/inspections/[id]/photo` | — | `id: string` | — | — | yes | `push` | `router.back()` after photo saved |
| `/(app)/inspections/[id]/defect` | — | `id: string` | `zone?: string` (from walkaround), `editId?: string` (edit existing), `defectId?: string` (alt edit form) | — | yes | `push` | `router.back()`; walkaround re-fetches via useFocusEffect |
| `/(app)/profile` | `profile` | — | — | — | yes | `navigate` | (tab root) |
