...
- child records, stat tiles, images, and product cards when present.
...
- **Home hero keys** (Home/dashboard tab roots): `state-hero` · `metric-hero` · `media-hero` · `queue-hero`
...
- **Hero** (REQUIRED on every Home/dashboard Tab-root): choose `state-hero` for one dominant computed state, `metric-hero` for progress toward a target, `media-hero` only when the schema supplies the selected record's own hero-eligible image, or `queue-hero` when waiting work is the story. Emit the exact key. The largest headline names a seed-backed record and every hero includes a numeric metric object plus a boundary-crossing nested element.
...
- **Cardinality** — REQUIRED when the screen has filters, choices, rows, child records, actions, stats, images, or product cards. Emit one semicolon-separated machine-readable line, for example: `filters=6 -> chips-overflow; choice-cr_status=6 -> inline-radio-list; listRows=12 -> plain-list; actions=3 -> button-row; images=1 -> image-hero; productCards=4 -> product-card-row`. Counts come from Step 0c. Use the exact pattern keys from `screen-templates.md`; append `because <domain reason>` only for an intentional override.
