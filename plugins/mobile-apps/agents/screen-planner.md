...
- **Role / active scope** (omit if open to all signed-in users and unscoped) —
  only when the screen is role-gated or depends on the active
  store/site/location. Reference the approved Shared Operational Context
  values and require `useOperationalContext`; never define a per-screen source.
...
- **Data** — which generated services it calls, with method names (e.g., bounded lookup: `AccountsService.getAll({ top: 50, orderBy: ['name asc'], select: ['name'] })`; cursor list: `InspectionsService.getAll({ maxPageSize: 50, orderBy: ['scheduledDate asc', 'inspectionid asc'], select: [...] })` plus `skipToken` continuation support). Any user-entered search/filter value MUST use `containsFilter('<logical column>', query)` or `odataString(value)` from `@/utils`. Never emit raw OData interpolation such as ``contains(name, '${query}')`` or `name eq '<q>'`; apostrophes and other literal content must be escaped by the helper.
...
- **Data:** `useAuth()` for identity/sign-out, plus
  `useOperationalContext()` when Profile shows role/team/site and generated
  services only when the approved context contract requires them.
