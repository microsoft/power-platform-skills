...
- `<working_dir>/package.json` — installed dependencies. Native modules must already be present, except controls from [Microsoft OOB controls](${PLUGIN_ROOT}/skills/add-native/references/oob-controls.md) approved in `## Native Capabilities`; their `/add-native` helpers install missing app dependencies before screen generation. Follow the reference's use cases, including the explicit-only Microsoft scanner route. Do not install them here or misclassify them as JS-only. A verified pure-JavaScript library may instead be proposed with an exact version under `## Screens → ### JavaScript Dependencies` for the orchestrator to install.
