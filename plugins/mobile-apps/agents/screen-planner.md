...
- **Native capabilities** — which native modules/wrappers it uses, and which iOS/Android platforms or permission states need fallback handling. For PDF/pen screens, be precise: `document-picker` (`expo-document-picker`) for user-picked files; `pdf-report` (`expo-print`, plus `expo-sharing` only when present and local share/open is required) for generated local PDFs; `native-pdf-viewer` (`@microsoft/extension-native-pdf-viewer`) for HTTPS PDF URLs only; `pen-input` (`@microsoft/extension-pen-input`) for signature/ink capture. For location screens, distinguish `geolocation` (`@microsoft/powerapps-geolocation-control`) — continuous/background tracking with native Dataverse sync, needs start/stop/tracking-status UI plus a permission-denied state — from one-shot `location` (`expo-location`) for a single foreground coordinate read.
