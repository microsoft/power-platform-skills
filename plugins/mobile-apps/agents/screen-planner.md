...
|  | Capabilities: `expo-camera`, `expo-document-picker`, `expo-print`, `expo-secure-store`, `expo-file-system`, `expo-sharing`, `@powerapps/extension-native-pdf-viewer`, `@powerapps/extension-pen-input` when allowlisted by the plan (see AGENTS.md §2) |
...
- **Native capabilities** — which native modules/wrappers it uses, and which iOS/Android platforms or permission states need fallback handling. For PDF/pen screens, be precise: `document-picker` (`expo-document-picker`) for user-picked files; `pdf-report` (`expo-print`, plus `expo-sharing` only when present and local share/open is required) for generated local PDFs; `native-pdf-viewer` (`@powerapps/extension-native-pdf-viewer`) for HTTPS PDF URLs only; `pen-input` (`@powerapps/extension-pen-input`) for signature/ink capture.
