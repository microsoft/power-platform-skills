...
- **Read-only.** You MUST NOT POST to `/mobileofflineprofiles`, PUT EntityMetadata, or call `npx --no-install pa app add data-source --non-interactive`. Mutation happens later in `/setup-offline-profile` after user approval through 3 gates.
