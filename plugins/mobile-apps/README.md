...
| `/check-updates` | ✅ v0 | Standalone dependency maintenance — checks for a plugin update and restart first, updates the native host with its template migrations, then presents, approves, updates, and validates the remaining direct packages one at a time. |
