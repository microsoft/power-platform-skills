...
| `/create-mobile-prototype` | ✅ v0 | Prototype orchestrator — starts from the same fresh installed template, resolves evidence-bound context, defaults to one environment-free review (with an explicit four-section mode), writes a neutral domain contract, generates deterministic local repositories/TanStack Query hooks/fixtures under `src/data`, builds screens from one immutable pack via in-memory work orders, starts Metro without Power Platform provisioning, and records a statically validated prototype. |
...
| `/sync-from-plan` | ✅ v0 | Reconciles an existing prototype or real app from the readable plan plus machine sidecars; refreshes domain/repository mappings, routes, shared code, affected screen work orders, quality gates, preview, and lifecycle hashes. |
