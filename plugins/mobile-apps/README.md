...
The skill deterministically extracts screens, controls, Power Fx behavior, Dataverse schema, connectors, flows, components, assets, localization, and PCF public contracts into a local migration package. It presents behavior/control coverage and unsupported items before changing the target. The full behavior ledger remains lossless for deterministic auditing, while backward dependency closure gives each builder only its screen's exact load-bearing behavior plus structured native intent hints for disconnected Canvas UI plumbing; ambiguous state stays exact. Every PCF receives an explicit user-approved native replacement, server dependency, optional visible unsupported state, or hard blocker before screen generation. Pathological single-event formulas are decomposed into approved named-step native workflows instead of one giant callback; AI owns routine code/UX structure, while the user is asked only when transaction, partial-failure, retry/idempotency, batch, async-completion, or unclassified critical behavior cannot be proven from source. On approval, it delegates to `/create-mobile-app --adapted-from` so the current public template, app registration, offline, native capability, screen-generation, debug, and `/edit-app` workflows remain unchanged.
