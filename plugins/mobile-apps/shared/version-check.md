...
- **Deploy** = `npm run build` + `npx --no-install pa app push --non-interactive`. No local Xcode / Android Studio involvement.
...
Result: the only required tooling is what Node/npm, `npx --no-install pa`, and the relevant helper scripts need. Xcode/JDK/Android Studio are explicitly out of scope.
...
| `@microsoft/power-apps-native-host` | `^0.4.0` | native runtime host |
