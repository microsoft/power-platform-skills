...
- industry context and product archetype, with classification evidence and confidence;
- workflow capabilities and operating context;
- supplied visual-reference sources, if any;
- environment;
- online/offline intent;
- native capability and connector implications;
- rough table/screen/time estimates.

The product archetype describes the primary repeated loop. Inspection, scanning,
approval, or scheduling that supports another loop belongs under workflow
capabilities. Low-confidence classification and its top alternatives are shown
inside this gate and revised with `Edit`; they never create a separate prompt.
...
4. validate the Gate 1 product archetype/workflow classification against the
  architecture without changing approved scope silently;
5. classify every cross-entity field using the resolution ladder in
   `data-performance.md`;
6. produce complete architecture and screen-plan sections;
7. render `mobile-app-plan.html`.
...
- visual personality and ambition, independently of product archetype;
- Home composition and cross-tab silhouettes;
- first-viewport geometry and signature component contracts;
- media source/fallback strategy;
- reference fidelity, required motifs, and forbidden drift when visual input exists;
...
Static phone frames are explicitly labeled approximations. They must show the
approved composition rather than restyling one universal dashboard layout.
...
- required runtime visual-QA scope for `premium`, `bespoke`, `high`, or
  `strict-structural` experience contracts;
- readiness blockers.
...
When reference input exists, show its extracted structural contract beside the
planned composition; do not imply pixel parity.
