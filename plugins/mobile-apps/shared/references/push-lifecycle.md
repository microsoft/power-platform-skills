...
| 2. Platform credentials and capabilities | Platform-specific credentials, identifiers, entitlements, and capabilities required before wrapping are configured for the exact app identity. iOS requires the `/setup-apple-ios` provisioning handoff and `/setup-apns` manual-upload handoff; that APNs handoff remains pending physical verification. Android may report `not applicable` when the selected runtime requires no separate external handoff. | Current iOS owners are `/setup-apple-ios` and `/setup-apns`. Do not invent an Android credential workflow when no owner is bundled. |
| 3. Runtime integration | The selected platform runtime contains the required native modules, and the app implements consent/permissions, registration-token lifecycle, exact topic transitions, foreground/background/response listeners, and validated deep links. | `/add-push-notifications` |
...
| 6. Wrapped build | A fresh platform artifact is created from the current app and push inputs and recorded by the platform build owner. Artifact creation does not include installation or delivery proof. | iOS: `/build-ios`. Android: `/build-android`. |
| 7. Physical delivery | The exact current wrapped artifact is installed on a supported physical device and end-to-end delivery is correlated through the live flows, provider, device states, topic transitions, and validated deep links. | iOS: `/verify-ios-push`. Android: `/verify-android-push`. |
