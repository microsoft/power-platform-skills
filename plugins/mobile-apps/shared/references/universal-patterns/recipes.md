# Universal Pattern Recipes

On-demand examples reached through [the index](../universal-patterns.md). Locate the selected heading and read that section only; never load this full library by default. Examples illustrate composition, not production-ready data/auth/persistence contracts. Follow the approved task, current host APIs, accessible controls, and [native allowlist](../../../skills/add-native/SKILL.md); no recipe authorizes an additional capability.

Use actual data and theme values, preserve keyboard/safe-area behavior, label controls, and respect reduced motion. A state illustration is not proof that a backend commit or runtime capability exists.
...
Used when a user's retrieval/triage task benefits from search and multiple filters. Row count alone does not justify a filter drawer. Use server-side search and cursor pagination for unbounded data; do not search only a capped page.
...
Use only for an explicitly required reveal policy with allowlisted local authentication. Local reveal is not server authorization; retain the host's authenticated service boundary. Handle cancellation, missing enrollment, and failed authentication without exposing the value.
...
Use only when the real auth/session policy supports expiry detection and renewal. Do not invent a countdown or extend-session action from an industry label. Host-expiry redirects preserve work according to the approved auth contract.
  ...
  <AlertDialog.Portal>
    ...
    <AlertDialog.Content p="$5" gap="$4" items="center">
      ...
      <XStack gap="$3" width="100%">
        <Button flex={1} size="$4" onPress={openProfile}>Account options</Button>
...
Requires an explicitly needed and allowlisted voice-input implementation. Do not infer it from field work or draw an enabled button without a working capability; keyboard input remains available.
...
Only when spatial selection/tracking is the actual job and the native module is allowlisted. A map example does not authorize adding `react-native-maps` to the native binary. Provide accessible non-map context/actions and permission/error recovery.
...
The gesture example below is only visual wiring. For actual destructive actions, use [mobile-gesture-recipes.md](../mobile-gesture-recipes.md): confirm before irreversible commits, retain/restore the row on failure, prevent duplicates, and provide a visible accessible action. `onDelete`/`onArchive` must implement those service semantics; moving a row off-screen is not success.
...
- Offer undo only if the backend supports restoration; otherwise confirm before committing. Do not advertise a no-op Undo.
...
Select a named interaction from [the index](../universal-patterns.md) because it supports the approved journey. Industry supplies vocabulary/context, not permission to force a visual composition, connector, authentication gate, or device capability.
