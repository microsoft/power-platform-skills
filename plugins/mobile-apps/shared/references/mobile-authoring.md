...
If the VS Code request supplies a command wrapper, use it for every command
shown here and every nested plugin command. The VS Code process itself may
not have callback variables; the wrapper supplies the existing scoped context.
Do not read or copy its private session file. Reuse its verified result.
...
The isolated CLI has no interactive question tool; attended VS Code retains
its desktop permissions. Neither host's ordinary question UI is the Player
question channel. The helper
...
and decisions; existing bounded workers retain their assigned scopes.

VS Code's desktop/tool permissions remain separate from Player product
questions. Use app/build/update wording, not internal prototype terminology.
...
File checks do not constitute a sandbox. Isolated CLI mode enforces its
constrained runner policy. Attended VS Code is an explicit trusted-desktop mode,
not security-equivalent isolation or an automatic fallback. Both retain scoped
callbacks, protected descriptors, candidate separation and signed decisions.
An initial local-data app operation cannot mutate Dataverse.
...
A signed receipt establishes the bridge's decision, not that the maker saw a
particular UI or personally tapped it. If the maker disputes seeing an approval,
pause further mutations and investigate the Player UI; do not infer auto-approval
or describe the disputed click as observed.
...
## Phone app planning and integration gates

The explicit phone profile uses real persistent app-owned local repositories
before a separately approved data connection. It never creates fake Power
Platform services. First deliver one complete 2-3-screen journey; preserve
the remaining scope for Build more screens. Main's design and quality guidance
still applies, but HTML companions and new client-registration wizards do not
block this phone flow. The bridge owns native preview publication and Metro.

The main-based phone creation profile uses its combined `phone-app-plan`
question, binding the human plan and complete input artifacts with `--bind`.
It does not invoke the old four-gate Product Experience planner. Follow
[the phone profile](../../skills/create-mobile-app/references/phone-app-profile.md).

Individual native/connector additions still use `--record-gate 1` after their
separate edit proposal. For a verified phone plan, this binds its current
screen/navigation, ownership, domain and relevant design/schema artifacts.
The persisted signed receipt is consumed directly; no legacy creation receipt
is written over main's independent Dataverse schema approval file.
Dataverse conversion retains separate discovery, schema and Apply decisions.

## Compatibility gates for older profiles
