...
- Record, at most, this note after the connector table: `Application Insights telemetry is configured separately by the /setup-app-insights skill through PowerAppsProvider; it is not a Power Platform data connector.`
...
`/add-connector` owns the `npx pa app add data-source --non-interactive` call for its connector. The orchestrator never calls `npx pa app add data-source --non-interactive` directly.
