...
Inventory the selected Entra app/service principal, credential metadata, Key
Vault secret metadata, runtime RBAC, pool/provider, sender service account and
its keys/IAM policy, Firebase project IAM policy, and custom FCM role. The
`create-dedicated-registration` route may begin without a client ID only while
inventorying the exact pinned display name and proving absence rather than
inventing an ID. Acquire a
...
Show the discovered state, route, exact
...
For a genuinely absent explicitly requested new Entra identity, first present
only the narrow identity/credential and secret-safe Key Vault bootstrap plan.
After its separate approval and execution, reread the generated identity and
present a fresh complete plan for the remaining Google/API/RBAC work. Any
discovered addition, route change, execution-mode change, resource change, API
enablement, replacement, or rotation requires a new explicit approval; never
treat orchestration or first-stage approval as blanket approval.
...
- **Provision:** required resources do not exist. For either
  ...
  For `create-dedicated-registration`, split a missing identity into the narrow
  bootstrap stage; only its fresh post-bootstrap plan may classify and propose
  the remaining Google, API, IAM, runtime RBAC, proof, and handoff work. If an expected ID exists,
...
or substitute a broader FCM role.
...
For a truly absent create-new identity, use an explicit serial identity
bootstrap stage. Freshly reread absence before the first mutation, execute only
the approved bootstrap list, and retain only the safe server-generated
client/app/service-principal IDs, application ID URI, credential expiry, and
Key Vault URI/name. The credential value must flow
...
Credentials, IAM, or FCM disabled, show that exact API and obtain approval.
Then issue one explicit `services enable` and reread the affected resource.
