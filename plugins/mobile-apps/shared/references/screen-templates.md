...
| `scan-geofence-gate` | A dedicated workflow route or full-screen modal requires identity, QR/barcode, NFC, camera, or location verification; never use this pattern on Home | Scan/camera target or manual fallback, location confidence state, retry/override path, audit note for override; Home may only link to this surface with a visible CTA |
