...
`power.config.json` is intentionally NOT touched by the offline skill — it stays owned by `npx pa app init --app-type MobileApp --non-interactive`.
