...
- Power Apps CLI: `npx pa` command flags and Code Apps behavior; Power Platform environment / connection commands
...
1. Track changed files by writer: the skill/subagents and preparation helpers versus trusted generators. Use a helper's returned `writtenFiles` when available; track removals separately, not as files to validate. Output from `npx pa app init --app-type MobileApp --non-interactive` or `npx pa app add data-source --non-interactive` is excluded only when produced by that command and not modified afterward by the skill or its subagents. Newly generated does not mean manually written.
...
- MUST NOT edit any file under `src/generated/` unless the step explicitly calls for it. These files are regenerated by `npx pa app add data-source --non-interactive`.
...
Use direct `npx pa`, `node`, and `az` commands for the mobile-app plugin flow.
...
**Power Apps CLI required-argument rule:** when a skill invokes `npx pa`, pass every value the skill already knows and run app-root verbs from the directory that contains `power.config.json`. In practice:
...
**Standalone `npx pa` auth:** the CLI uses its own MSAL cache at `~/.powerapps-cli/cache/auth/msal_cache.json`; `az login` / `az account set` will not switch the account used by `npx pa`. Auth commands do **not** require `--environment-id`. Use this triage order when auth fails or the wrong user is active:

| Step | Command | When to use |
|---|---|---|
| 1. Check state | `npx pa auth status --non-interactive` or `npx pa auth status --json --non-interactive` | Always — see which accounts are cached and which is active (marked `*`) |
| 2. Switch account | `npx pa auth switch --account <email-or-homeAccountId> --non-interactive` | Right user is already cached — no browser re-auth needed |
| 3. Add account | `npx pa auth login --non-interactive` or `npx pa auth login --account <email> --non-interactive` | Right user is NOT in cache — unattended mode fails fast instead of opening or waiting for browser authentication (`--account` identifies the email) |
| 4. Clear cache | `npx pa auth logout --non-interactive` | Last resort — removes every cached account; a later authenticated command will require fresh sign-in |
...
**Failure refresh policy (global):** if any `npx pa *` command exits non-zero, run `npx pa auth status --json --non-interactive` to confirm the active account is correct. If the account needs to change, use `auth switch`; if no account is cached, use `auth login`. Only run `npx pa auth logout --non-interactive` when the cache itself is corrupt or you want to remove all accounts. After correcting auth state, retry the same command once before further triage.
...
### `npx pa *` failures (all commands)

1. Run `npx pa auth status --json --non-interactive` to verify the active account.
2. If the wrong account is active and the right one is cached, run `npx pa auth switch --account <email> --non-interactive`.
3. If no account is cached or the right account is missing, run `npx pa auth login [--account <email>] --non-interactive`; unattended mode will fail fast rather than open or wait for a browser.
4. Re-run the same `npx pa *` command once with the same arguments.
...
### `npx pa app add data-source --non-interactive` failures
...
- Generated files that a tool may have rewritten (e.g. `npx pa app add data-source --non-interactive` regenerating `connectorSchemas.ts` between your edits).
