...
If the bank doesn't exist yet, `/create-mobile-app` is responsible for copying the template (`${PLUGIN_ROOT}/shared/memory-bank.md`) into the working directory at Step 6 (right after `npx --no-install pa app init --app-type MobileApp --non-interactive` succeeds).
...
- Power Apps CLI: `npx --no-install pa` command flags and Code Apps behavior; Power Platform environment / connection commands
...
All non-Dataverse connectors require a connection ID or connection reference before `npx --no-install pa app add data-source --non-interactive`. Read this before any `/add-*` connector skill. Always run `/list-connections` first to create a supported connection, reuse a caller-provided connection ID, or resolve a solution connection reference.
...
1. Track changed files by writer: the skill/subagents and preparation helpers versus trusted generators. Use a helper's returned `writtenFiles` when available; track removals separately, not as files to validate. Output from `npx --no-install pa app init --app-type MobileApp --non-interactive` or `npx --no-install pa app add data-source --non-interactive` is excluded only when produced by that command and not modified afterward by the skill or its subagents. Newly generated does not mean manually written.
...
- MUST NOT edit any file under `src/generated/` unless the step explicitly calls for it. These files are regenerated by `npx --no-install pa app add data-source --non-interactive`.
...
Use direct `npx --no-install pa`, `node`, and `az` commands for the mobile-app plugin flow.

**Local CLI gate (before any Power Apps CLI command, including auth):** run
from the approved project root containing `package.json`, even before
`power.config.json` exists. Check for the installed grouped shim without
executing it or probing its version:

```bash
node -e "const fs = require('node:fs');
const shims = ['node_modules/.bin/pa', 'node_modules/.bin/pa.cmd'];
if (!shims.some(shim => fs.existsSync(shim) && fs.statSync(shim).isFile())) {
  console.error('Local grouped Power Apps CLI is missing. Restore the project dependencies or migrate an older template before retrying.');
  process.exit(1);
}"
```

If the gate fails, **STOP before invoking `npx`** and surface the diagnostic.
Do not treat a missing shim as an authentication failure, install the unrelated
unscoped `pa` package, remove `--no-install`, or silently install/upgrade
dependencies. Use the project's approved dependency installation/recovery flow;
an older project with only the flat `power-apps` shim must migrate to the supported
template before using these grouped commands. No automatic flat-command fallback.
Recheck after changing project roots, restoring dependencies, or a binary-resolution
failure. A successful check applies only to that project and dependency installation.

**Always keep `--no-install`.** The presence check requires a project-local shim;
the flag prevents npm from downloading a registry package if resolution fails.
`--non-interactive` controls CLI prompts, not npm's package resolution, so both
flags are required.

Typical commands:

```bash
npx --no-install pa app init --app-type MobileApp --display-name '<name>' --environment-id <id> --non-interactive
npx --no-install pa app add data-source --connector <api> --connection-id <connection-id> --non-interactive
npx --no-install pa connection create --connector <api> --json --non-interactive
npx --no-install pa connection list-references --solution-id <solution-id> --non-interactive
node scripts/resolve-environment.js [environment-id-or-url]
```

**Power Apps CLI required-argument rule:** when a skill invokes `npx --no-install pa`, pass every value the skill already knows and run app-root verbs from the directory that contains `power.config.json`. In practice:

- `app init` can use `--environment-id` because there is no `power.config.json` yet.
- After `power.config.json` exists, do **not** pass `--environment-id` to grouped app or connection commands. The CLI reads the environment and region from `power.config.json`; extra unregistered flags can fail command parsing.
- Every `npx --no-install pa` invocation MUST include `--non-interactive` to prevent an unattended terminal from waiting for input. This is a plugin fail-fast policy even where the current CLI reference does not list the flag for that command. Keep `--force` as well for data-source and flow removal because it controls their confirmation prompts.
- Use `--json` only where the grouped command documents it: `auth status`, `connection create`, `connection list`, `connector list`, and `app find-dataverse-api`. Parse the documented tabular output from other list commands.
- For Dataverse table generation, pass `--connector dataverse`, `--org-url <environment-url>`, and `--table <table-logical-name>`. The template-pinned CLI still requires the resolved Dataverse URL in non-interactive mode even when the environment ID is present in `power.config.json`; resolve both against the same approved environment.
...
**Standalone `npx --no-install pa` auth:** the CLI uses its own MSAL cache at `~/.powerapps-cli/cache/auth/msal_cache.json`; `az login` / `az account set` will not switch the account used by `npx --no-install pa`. Auth commands do **not** require `--environment-id`. Use this triage order when auth fails or the wrong user is active:

| Step | Command | When to use |
|---|---|---|
| 1. Check state | `npx --no-install pa auth status --non-interactive` or `npx --no-install pa auth status --json --non-interactive` | Always — see which accounts are cached and which is active (marked `*`) |
| 2. Switch account | `npx --no-install pa auth switch --account <email-or-homeAccountId> --non-interactive` | Right user is already cached — no browser re-auth needed |
| 3. Add account | `npx --no-install pa auth login --non-interactive` or `npx --no-install pa auth login --account <email> --non-interactive` | Right user is NOT in cache — unattended mode fails fast instead of opening or waiting for browser authentication (`--account` identifies the email) |
| 4. Clear cache | `npx --no-install pa auth logout --non-interactive` | Last resort — removes every cached account; a later authenticated command will require fresh sign-in |

In CI or other unattended execution, `auth switch` requires `--account <email>` when more than one account is cached; it will fail with an error listing the cached accounts if omitted.

**Failure refresh policy (global):** after the local CLI gate succeeds and the CLI starts, if a `npx --no-install pa *` command exits non-zero, run `npx --no-install pa auth status --json --non-interactive` to confirm the active account is correct. Binary-resolution failures return to the local CLI gate instead. If the account needs to change, use `auth switch`; if no account is cached, use `auth login`. Only run `npx --no-install pa auth logout --non-interactive` when the cache itself is corrupt or you want to remove all accounts. After correcting auth state, retry the same command once before further triage.
...
### `npx --no-install pa *` failures (all commands)

If the binary cannot be resolved, return to the local CLI gate above and STOP;
do not run further CLI/auth commands until the local installation is restored.

1. Run `npx --no-install pa auth status --json --non-interactive` to verify the active account.
2. If the wrong account is active and the right one is cached, run `npx --no-install pa auth switch --account <email> --non-interactive`.
3. If no account is cached or the right account is missing, run `npx --no-install pa auth login [--account <email>] --non-interactive`; unattended mode will fail fast rather than open or wait for a browser.
4. Re-run the same `npx --no-install pa *` command once with the same arguments.
...
### `npx --no-install pa app add data-source --non-interactive` failures

| Condition | Action |
| --- | --- |
| Wrong Power Apps CLI user, `Multiple accounts found`, or standalone CLI auth loop | Run `npx --no-install pa auth status --json --non-interactive` to see cached accounts. If the right account is cached, run `npx --no-install pa auth switch --account <email> --non-interactive`. If not cached, run `npx --no-install pa auth login [--account <email>] --non-interactive`; unattended mode fails fast rather than waiting for browser authentication. Do not use `az account set` to switch this CLI. |
| `connectionId not found` or empty `-c` | Create a connection with `npx --no-install pa connection create --connector <connector-id> --json --non-interactive`, use a caller-provided existing connection ID, or use `npx --no-install pa connection list-references --solution-id <solution-id> --non-interactive` and retry with `--connection-ref`. |
...
| `environment not set` | Confirm `power.config.json` has `environmentId`; if missing, rerun `npx --no-install pa app init --app-type MobileApp --display-name '<name>' --environment-id <id> --non-interactive`. |
...
- Generated files that a tool may have rewritten (e.g. `npx --no-install pa app add data-source --non-interactive` regenerating `connectorSchemas.ts` between your edits).
