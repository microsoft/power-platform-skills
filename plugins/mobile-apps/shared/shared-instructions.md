...
- MUST NOT add packages with native source, podspecs, codegen configuration, Expo modules/config plugins, or platform projects unless they already exist in the template `package.json`, except the allowlisted Microsoft OOB control packages in [Microsoft control dependency setup](${PLUGIN_ROOT}/skills/add-native/references/oob-controls.md). Only their `/add-native` helpers may add the requested app dependency and update its lockfile; this does not install native code into the wrapped binary or permit arbitrary Microsoft/Expo packages.
...
| `TS2305` / `TS2307` (missing export / module not found) | For a requested Microsoft OOB control, route to its `/add-native` helper's Microsoft control dependency setup. For other native code/config, STOP unless it already exists in the template `package.json`. If an approved plan names a pure-JavaScript dependency, run `npm install --save-exact <package>@<approved-version>` and retry. Do not install an unplanned package merely to silence an import error. |
