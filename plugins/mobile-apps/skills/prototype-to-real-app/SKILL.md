...
description: Connect an existing mobile app to approved Dataverse tables while preserving its screens, local data model and rules. Uses bounded discovery, reviewed mappings, official services and a separately reviewed app update. Does not deploy or import sample records automatically.
user-invocable: true
allowed-tools: Read, Write, Edit, Bash, Glob, Grep, AskUserQuestion, Task
model: opus
---

# Connect an app to Dataverse

Read [shared instructions](${PLUGIN_ROOT}/shared/shared-instructions.md) and
...
In the phone flow, reuse the existing desktop and Player sign-in and configured
host registration. Do not start a new client-registration wizard or an HTML
preview. Missing real authentication is a visible prerequisite, never a reason
to fabricate IDs, skip auth checks or fall back silently to local records.
...
- Reuse the current foreground bounded snapshot/evidence/proposal/compiler
  pipeline and the bounded `mobile-app:data-model-architect`.
...
**Telemetry checkpoint: `connect_app_to_dataverse`**

Read `.tmp/prototype-profile.json`, domain/bindings/rules, the app-owned registry,
the main human plan, phone screen projection, persistence, canonical facts,
screen packs, and the current source-bound authoring context. Retain any older
planning artifacts already present, but do not recreate that older workflow.
Verify the active
...
Configure the real registration using the ordinary
[authentication phase](${PLUGIN_ROOT}/skills/create-mobile-app/references/phase-05-auth.md) when required.
...
Read and execute main's
[bounded planning snapshot](${PLUGIN_ROOT}/skills/create-mobile-app/references/planning-snapshot.md)
and the required-data branch of
[planning](${PLUGIN_ROOT}/skills/create-mobile-app/references/phase-02-planning.md).
Use the current publisher detection, bounded foreground snapshot, compact
evidence, normalized schema contract and foreground approval receipt.
...
Keep the phone screen projection, existing routes, scenario values and rules.
Rebind existing data ownership without adding the older Journey/usage workflow.
Capture the model/mapping approval in Player, then have the foreground
initialize main's [schema approval receipt](${PLUGIN_ROOT}/skills/create-mobile-app/references/approval-receipt.md)
from that actual acceptance. No local self-hash or compiler flag is consent.
Review this conversion's own complete plan with `--record-gate 4`; phone gate
binding captures the current main plan, model and execution artifacts without
requiring HTML or the older four-stage creation planner. Reuse the current
design, rather than running another design intake or rebuilding the app.

## 4. Bound execution copy and sequential remote phases

Read
[main data execution](${PLUGIN_ROOT}/skills/create-mobile-app/references/phase-06-data.md)
