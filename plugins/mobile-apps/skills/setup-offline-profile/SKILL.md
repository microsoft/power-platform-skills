...
NOT modify `power.config.json`** (that file is owned by `npx pa app init --app-type MobileApp --non-interactive`
...
Note: power.config.json was NOT modified (owned by npx pa app init --app-type MobileApp --non-interactive).
