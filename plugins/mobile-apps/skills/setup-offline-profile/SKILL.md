...
NOT modify `power.config.json`** (that file is owned by `npx --no-install pa app init --app-type MobileApp --non-interactive`
...
> **Important: this skill must NOT modify `power.config.json`.** That file is owned by `npx --no-install pa app init --app-type MobileApp --non-interactive` and its schema is controlled by the upstream tool — adding custom fields there risks being overwritten on re-init and breaks when the upstream schema changes. All app-level offline config goes into `offline-profile.json` (which we own) under an `appConfig` block.
...
Note: power.config.json was NOT modified (owned by npx --no-install pa app init --app-type MobileApp --non-interactive).
