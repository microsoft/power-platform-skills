...
description: Use when a user chooses offline support for a Dataverse-backed Power Apps mobile app or explicitly invokes profile authoring for an existing project; designs per-table row scope, relationships, columns, and sync frequency through one consolidated configuration review.
user-invocable: true
...
- **Orchestrated `/create-mobile-app`:** the caller passes
  `--orchestrated-create` only after the user makes an affirmative offline
  choice in Step 8.85. Treat the flag as proof of that answer, not as a planning
  contract. Do not ask a selection or opt-out question here. Profile
  configuration review remains required because it controls Dataverse
  mutations.
- **Standalone existing project:** a user may still explicitly invoke this
  skill. The explicit invocation is the affirmative offline choice; continue
  with the manifest/auth/profile checks below and do not ask a selection or
  opt-out question.
...
- Environment resolution failure → standard auth recovery (`az login --tenant <env-tenant>` or provide environment URL directly; see [shared-instructions-core.md](${PLUGIN_ROOT}/shared/shared-instructions-core.md)).
...
| `status: not-applicable` | The current explicit invocation supersedes the marker; clear it and continue. Do not treat it as a current selection question. |
