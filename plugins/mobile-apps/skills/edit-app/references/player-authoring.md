...
The one-screen builder keeps the main create flow's bounded context, typed
skeleton, assigned-file ownership and retry conventions. The edit transaction
still seals the complete baseline and exact allowed effects.
...
For a main-based phone app, read `.tmp/phone-app-plan.json` and the relevant
`native-app-plan.md` sections. **Build more screens** asks the maker which
deferred/useful next screens to add, then uses a `screen` proposal with
`newScreens`; it does not regenerate existing screens or create a new app.
Basic visual changes include app background and a selected list/card treatment.
Basic teaching is a bounded condition/validation rule on an existing action,
not model training or automatic schema mutation.
...
Keep the complete app screen inventory intact. Use `screen` events for
per-screen progress, not a replacement `plan` containing only changed screens.
A matching affected-screen report does not remove unchanged destinations or
invalidate approval. New routes or changed screen contracts still require a
current plan review; `approval-required` is not a revoked credential.

1. Update only the approved main plan delta and affected phone execution
   projection. For phone apps, `compile-screen-build-pack.js` derives screen
   and navigation metadata from `.tmp/phone-app-plan.json`; do not reintroduce
   Product Experience/Scope/Journey planning or unrelated screen work.
...
4. Use the main one-screen builder contract for each affected screen with a
   real compiling skeleton and exact source assignment. A child never expands
   file scope or answers maker questions.
...
readiness checks, and sends the exact source revision to the bridge. A final
edit or teaching candidate revalidates **every compiled screen**, including
unchanged destinations, before publishing their readiness. For
