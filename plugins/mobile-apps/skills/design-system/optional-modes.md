...
Do not preload the reference library. The normal automatic prototype path
below reads no optional reference files; its strict schema and implementation
live in deterministic scripts. Load only the reference selected by explicit
maker intent:

- design spec or standalone authoring: `references/design-system-schema.md`;
- gallery: `references/preview-template.md`;
- refresh/reskin: `references/refresh-flow.md`;
- brand/Figma/sibling import: `references/input-modes.md`;
- screenshot/design intake: `references/reference-intake.md` and the shared
  reference-fidelity contract;
- style comparison: `references/vibe/style-picker.md` and only the selected
  direction file.
...
### Automatic PR1 prototype fast path

When `CODE_APPS_NATIVE_ORCHESTRATING=1`,
`.tmp/prototype-semantic-plan.json` exists, and the maker did not explicitly
select an optional design mode, this path is mandatory and terminal for this
skill invocation. Do not continue into brand handling, a style/cost picker,
preview generation, or optional reference loading.

Read only the approved machine contracts consumed by the compiler, then run:
...
The compiler writes one strict recipe, byte-stable tokens, a compact signature
registry, the exact Foundation/signature component sources, their export
index, deterministic platform font/theme mapping inside the fresh template's
owned `tamagui.config.ts` customization block, a human-readable companion, and
an ownership manifest. The validator
checks PR1 path preservation, contrast, hierarchy, media/offline policy,
Navigation compatibility, accessibility, and deterministic output. Any
missing AI-owned decision is `BLOCKED`; do not synthesize a fallback. Return
`DONE` only after validation succeeds.
