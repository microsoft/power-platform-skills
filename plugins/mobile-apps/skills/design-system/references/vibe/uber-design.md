...
- **Pressed Gray** (`#e2e2e2`): White button pressed state -- a clean, cool light gray that provides clear feedback without warmth.
- **Pressed Light** (`#f3f3f3`): Subtle pressed for elevated white buttons -- barely-there gray for gentle interaction feedback.
...
- Padding: 10px 12px
- Radius: 999px (full pill)
- Pressed: background shifts to Pressed Gray (`#e2e2e2`)
- Focus: background shifts to Pressed Gray, inset ring appears
...
- Pressed: background shifts to `#f3f3f3`
...
- Focus: no extracted custom focus state -- relies on default focus ring
...
| Large Tablet | 1120px | Large Screen grid activates, horizontal nav pills |
| Large Screen | 1136px | Full larger surface layout, maximum container width, split hero |
...
- Pressed State: "Pressed Gray (#e2e2e2)"
...
- "Create a dark footer on Uber Black (#000000) with Pure White (#ffffff) heading text in UberMove at 20px weight 700. Footer links in Muted Gray (#afafaf) at 14px UberMoveText. Links pressed to Pure White. Multi-column grid layout."
- "Design a floating action button with Pure White (#ffffff) background, 999px radius, 14px padding, and shadow rgba(0,0,0,0.16) 0px 2px 8px. Pressed shifts background to #f3f3f3. Use for scroll-to-top or map controls."
