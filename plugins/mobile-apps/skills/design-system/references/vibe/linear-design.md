...
- **Lavender Pressed** ({colors.primary-pressed}): Lighter lavender (#828fff) — pressed state of the primary CTA.
...
- **Surface 2** ({colors.surface-2}): Two steps above — featured pricing card, pressed cards.
...
- Card grids are 3-up at larger surface, 2-up at tablet, 1-up at mobile.
...
| 2 (surface-2 lift) | `{colors.surface-2}` background, 1px `{colors.hairline-strong}` | Featured pricing card, pressed cards |
...
- Pressed state lives in `button-primary-pressed` (background shifts to `{colors.primary-pressed}` lighter lavender).
...
|---|---|---|
| Large Tablet | 1440px | Default larger surface layout |
| Large Screen | 1280px | Card grid 3-up maintained |
