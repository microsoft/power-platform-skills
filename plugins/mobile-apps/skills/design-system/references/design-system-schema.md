...
Generated: {{ISO 8601 timestamp}} | Personality: {{utility | polished-operational | premium-brand-forward | editorial | immersive | playful-consumer | reference-driven}}
...
- Contract version: 1
- Product archetype: {{slug from native-app-plan.md}}
- Workflow capabilities: {{comma-separated slugs}}
- Operating context: {{comma-separated slugs}}
...
## Navigation
- Mood: {{functional | atmospheric | cinematic | reference-driven}}
...
- Materialization preset: {{legacy preset name or none}}
...
## Legacy materialization presets

These presets are compatibility starting points, not product archetypes. They
must never be selected directly from industry. `inspection` maps to the
`utility` personality, `saas` maps to `polished-operational`, and `product`
maps to `premium-brand-forward`. The approved Product Experience and Reference
Contract can override any preset field.
...
| Role     | Family | Size | Weight | Line | Tracking |
|----------|--------|------|--------|------|----------|
| Display  | Inter  | 28   | 700    | 1.2  | 0        |
| Heading  | Inter  | 22   | 600    | 1.25 | 0        |
...
- ✗ No equal-weight metric/card grid on a composition that requires one dominant object or media region
...
2. **Structural priority** — `## Product Experience Link`, `## Composition`, `## Media`, `## Navigation`, and `## Signature Components` are binding and outrank generic samples/presets.
