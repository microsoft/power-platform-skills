...
| `pdf-viewer`, `native-pdf-viewer`, `pdf-control`, `open-pdf`, `@powerapps/extension-native-pdf-viewer` | [`add-pdf-viewer`](add-pdf-viewer/SKILL.md) internal helper | Enforces HTTPS-only viewer URLs and native viewer result handling |
| `pen-input`, `signature`, `ink`, `draw`, `@powerapps/extension-pen-input` | [`add-pen-input`](add-pen-input/SKILL.md) internal helper | Captures PNG data URI and documents Dataverse Image/File persistence |
...
| Open/preview an existing PDF URL | `/add-native pdf-viewer` | `@powerapps/extension-native-pdf-viewer` present and URL is `https://` | Do not pass `file://`, `content://`, `blob:`, `http://`, or generated local URIs to the viewer |
| Capture signature, ink, drawing, or sign-off | `/add-native pen-input` | `@powerapps/extension-pen-input` present | If persisted, plan Dataverse Image/File/child Evidence target first |
...
| `pdf-viewer`, `native-pdf-viewer`, `pdf-control`, `open-pdf`, `@powerapps/extension-native-pdf-viewer` | `@powerapps/extension-native-pdf-viewer` | `src/native/pdfViewer.ts` | `/add-native` routes internally to `add-pdf-viewer`; opens HTTPS PDF URLs only |
...
| `pen-input`, `signature`, `ink`, `draw`, `@powerapps/extension-pen-input` | `@powerapps/extension-pen-input` | `src/native/penInput.ts` | `/add-native` routes internally to `add-pen-input`; captures PNG data URI |
...
| `location` | `expo-location` | `src/native/location.ts` | Use only when the current template package contains `expo-location` |
...
- Use `native-pdf-viewer` / `pdf-control` only when the app opens/previews an HTTPS PDF URL with `@powerapps/extension-native-pdf-viewer`. Local `file://`, `content://`, and `blob:` URIs are unsupported by the native viewer path.
...
- Use `pen-input` only for signatures, drawn approvals, ink notes, sketches, and handwritten sign-off with `@powerapps/extension-pen-input`.
...
For normalized `camera`, `image-picker`, `barcode-scanner`, `qr-scanner`, `pdf-report`, `pdf-viewer`, or `pen-input`, do not fall through to the generic wrapper flow and do not tell the user to run another slash command. Read the nested helper and follow its steps inside this `/add-native` invocation:
