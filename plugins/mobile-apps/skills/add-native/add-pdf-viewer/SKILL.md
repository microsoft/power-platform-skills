---
name: add-pdf-viewer
description: Internal implementation skill invoked by /add-native for native PDF control workflows. Handles HTTPS-only PDF viewing with @powerapps/extension-native-pdf-viewer.
...
**Internal helper.** Users should invoke `/add-native pdf-viewer`, `/add-native pdf-control`, or `/add-native @powerapps/extension-native-pdf-viewer`; `/add-native` routes here after resolving the capability.
...
import { NativePdfViewer } from '@powerapps/extension-native-pdf-viewer';
...
- Use `@powerapps/extension-native-pdf-viewer` only for this native HTTPS PDF viewing use case.
...
```ts
import NativePdfViewerExtension from "@powerapps/extension-native-pdf-viewer";
```
...
```text
PDF viewer added
Package present   : @powerapps/extension-native-pdf-viewer
...
```text
- PDF viewer added — @powerapps/extension-native-pdf-viewer (<ISO date>)
```
