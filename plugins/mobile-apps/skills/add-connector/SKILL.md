...
Real existing auth client/tenant setup is required. In the phone profile,
reuse the configured Player/template registration and verify sign-in; do not
start a new client-registration wizard. Missing existing authentication is a
visible prerequisite, not permission to fabricate IDs or bypass sign-in.
Ordinary standalone setup retains its existing approved auth flow. This step is not the Dataverse conversion generator.
