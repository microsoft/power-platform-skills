...
When the project contains `src/data/contracts.ts`, generated service methods are
adapter implementation details. Do not help a screen import them directly.
Implement every approved connector repository method in
`src/data/repositories/connectorRepositories.ts` with this exact first line:

```ts
// Generated by connector repository adapter
```

The file exports `createConnectorRepositories(): ConnectorRepositoryMap`, calls
only exact approved generated methods, maps their inputs/outputs into neutral
operation shapes, and contains no screen/UI behavior. Existing screens and
`src/data/hooks/*` remain unchanged. A missing/ambiguous signature is blocking;
do not cast through it or choose a nearby operation.
...
node "${CLAUDE_SKILL_DIR}/../../scripts/validate-mobile-app.js" \
	--project-root "<working_dir>" --scope domain
```
...
The native host runtime handles all connector routing automatically via `PowerAppsProvider` in `app/_layout.tsx`. When a domain repository adapter calls a generated service method:
