...
Finds or creates a Power Platform connection with the Power Apps CLI. Returns the **Connection ID** or **Connection Reference** that callers feed into `npx --no-install pa app add data-source --non-interactive`.
...
```bash
npx --no-install pa connection create --connector <api-id> --json --non-interactive
```

Use the returned `connectionId` for `--connection-id <connectionId>`. Optional display names are supported:

```bash
npx --no-install pa connection create --connector <api-id> --display-name '<display-name>' --json --non-interactive
```
...
If `npx --no-install pa connection create --non-interactive` or `npx --no-install pa connection list-references --non-interactive` fails because of auth, wrong user, multiple accounts, no output, or timeout, follow shared-instructions command-failure handling and retry once.
