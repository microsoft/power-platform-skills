...
> **`power.config.json` is intentionally NOT consulted here.** That file is owned by `npx --no-install pa app init --app-type MobileApp --non-interactive`. The profile ID lives in `offline-profile.json` only.
