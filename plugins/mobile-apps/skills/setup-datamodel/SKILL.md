...
| "view PDF" | Store or reference an HTTPS URL if the app has a durable source. Native PDF viewer also supports local `file://` URIs; `content://`, `blob:`, and `http://` remain unsupported. |
