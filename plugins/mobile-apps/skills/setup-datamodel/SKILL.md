...
`/add-dataverse` creates tables in tier order, runs `npx pa app add data-source --connector dataverse --org-url <envUrl> --table <name> --non-interactive` per table from the app root, publishes customizations, writes `.datamodel-manifest.json`, and type-checks. Wait for it to return before Phase 6.
