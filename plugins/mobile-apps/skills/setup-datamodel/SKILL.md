...
| "view PDF" | Store or reference an HTTPS URL if the app has a durable source. Local `file://` viewing with native PDF viewer 0.2.9+ needs no Dataverse column; `content://` and `blob:` remain unsupported. |
