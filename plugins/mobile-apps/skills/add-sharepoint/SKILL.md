...
```bash
npx --no-install pa connection create --connector shared_sharepointonline --json --non-interactive
```
...
```bash
npx --no-install pa connection list-datasets --connector <apiId-from-list> --connection-id <connection-id> --non-interactive
```
...
**If `npx --no-install pa connection list-datasets --non-interactive` fails or returns no results:**
...
```bash
npx --no-install pa connection list-tables --connector <apiId-from-list> --connection-id <connection-id> --dataset '<site-url>' --non-interactive
```
...
**Print before starting:**
> "→ Running `npx --no-install pa app add data-source --non-interactive` per list (sequential, ~10–20 seconds each)."

SharePoint is a tabular datasource — requires `--connection-id`, `--dataset`, and `--table`:

```bash
npx --no-install pa app add data-source --connector <apiId-from-list> --connection-id <connectionId-from-list> --dataset '<site-url>' --table '<table-name>' --non-interactive
```
...
`npx --no-install pa app add data-source --non-interactive` wrote new files into `.power/schemas/sharepointonline/`. Regenerate `connectorSchemas.ts` before type-checking so the new list is wired into the runtime schema map:
