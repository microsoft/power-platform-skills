...
When `src/data/contracts.ts` exists, the sample below belongs only inside
`src/data/repositories/connectorRepositories.ts`. Give that file the marker
`// Generated by connector repository adapter`, implement the approved
`ConnectorRepositoryMap`, and map SharePoint inputs/outputs into neutral domain
shapes. Screens continue to call `@/data` hooks and must never import
`SharePointOnlineService` directly.
