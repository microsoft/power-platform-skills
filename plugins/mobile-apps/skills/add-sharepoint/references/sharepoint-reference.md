...
| `"The list 'X' does not exist"`                   | List name doesn't match exactly              | Verify list display name via `npx pa connection list-tables --connector <apiId> --connection-id <connection-id> --dataset '<site-url>' --non-interactive`; names are case-sensitive |
