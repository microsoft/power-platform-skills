...
| `SyntaxError` in `src/generated/` | Cited file is under `src/generated/` | **Do not edit.** For `connectorSchemas.ts`, regenerate the runtime map. For services/models, hand off to `/add-connector` or `/add-dataverse` for targeted CLI refresh and validation. Check the approved plan first: a removed source needs consumer cleanup through `/edit-app`, not re-registration. |
