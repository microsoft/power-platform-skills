...
| iOS notification runtime/delivery failure in a wrapped physical-device build (missing foreground/background receipt, notification tap/cold-start route failure, topic transition, opt-out, or re-registration recovery) | **Hand-off:** route user to `/verify-ios-push`. Preserve Metro findings as diagnostic context, but do not claim end-to-end push verification or recreate its physical-device/flow evidence matrix here. Suspected Apple/Xcode setup drift routes to the manual `/setup-apple-ios` guidance, then the user-managed `/build-ios` path; do not inspect signing assets or device identifiers. |
