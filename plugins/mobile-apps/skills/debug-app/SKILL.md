...
| iOS notification runtime/delivery failure in a wrapped physical-device build (missing foreground/background receipt, notification tap/cold-start route failure, topic transition, opt-out, or re-registration recovery) | **Hand-off:** route user to `/verify-ios-push`. Preserve Metro findings as diagnostic context, but do not claim end-to-end push verification or recreate its physical-device/flow evidence matrix here. Stale or mismatched Apple signing/profile proof routes to `/setup-apple-ios`, then `/build-ios`; never ask for certificate names, profile UUIDs, or device UDIDs. |
