...
- **Existing tables only** — skip to Step 5 (just runs `npx --no-install pa app add data-source --non-interactive` per table)
...
`npx --no-install pa` and `az` authenticate independently — they can point to different accounts. Verify `power.config.json` resolves and `az` can token for the target tenant before making any Dataverse API calls:
...
**Print before starting:**
> "→ Generating TypeScript services for <N> tables via `npx --no-install pa app add data-source --non-interactive` (sequential). Print '✓ <table>Service.ts' after each."
...
```bash
npx --no-install pa app add data-source --connector dataverse --org-url <envUrl> --table <table-logical-name> --non-interactive
```
...
`npx --no-install pa app add data-source --non-interactive` rather than guessing a JSON path or service
...
- Don't edit files under `src/generated/` — they are regenerated on every `npx --no-install pa app add data-source --non-interactive`.
