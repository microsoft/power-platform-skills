...
Explicit prototype uses the same design and Gates 3–4, through the shared
decision adapter in Player mode. The profile-specific scaffold is implemented
by `prepare-prototype.js`; never create fake live config/schema inputs.
...
**Prototype:** not applicable; keep all environment variables unset and proceed
to Step 5. The following resolver is the real-profile branch only.
...
**Prototype branch:** instead of the real-profile command below, run
`prepare-prototype.js` with `--project-root`, `--display-name`, `--slug`, and
the approved static `--entry-route`, exactly as in
[prototype-profile.md](./prototype-profile.md). It composes the existing
preparer with app-owned local generation, saves connected startup outside the
source graph, and preserves all official generated bytes. Do not run both
preparation commands; continue directly to Step 6.5 when it succeeds.
...
**Prototype:** skip this entire step. `power-apps init`, environment resolution,
and a placeholder `power.config.json` are forbidden. Proceed to Step 6.5.
The initialization command below remains unchanged for real apps.
...
**Prototype:** run `generate-prototype.js --project-root "<working_dir>"
--check`; verify the active root uses `PrototypeProvider` and no active route
imports absent live config, generated schemas, or host auth. Keep the installed
host config factories. In Player mode apply the shared authoring runtime
instrumentation, which configures the local data namespace before children
mount; the bridge owns the reserved runtime stamp. Do not reintroduce
`PowerAppsProvider` merely to satisfy the real-profile checks below.
...
Honor `MOBILE_APP_HTML_COMPANIONS` throughout this phase. With `0`, keep design
materialization, ownership, canonical validation and all approvals below, but
skip HTML generation/repair/opening. The existing preview validator validates
and retains the full design contract without requiring HTML. Gate 3 shows the
materialized design and journey in Player/host; Gate 4 says "native preview
after implementation" instead of supplying an HTML path.
...
and the validated `.tmp/product-experience-final-preview-contract.json`.
Require `_plan_preview.html` only when HTML companions are enabled. Missing
...
if [ "${MOBILE_APP_HTML_COMPANIONS:-1}" = "1" ]; then
  PREVIEW_ARTIFACT=(--artifact "preview=_plan_preview.html")
fi
node "${PLUGIN_ROOT}/scripts/mobile-pipeline-state.js" \
  ...
  --artifact "design-system=brand/design-system.md" \
  --artifact "design-tokens=brand/tokens.ts" \
  --artifact "signature-components=brand/signature-components.ts" \
  --artifact "design-contract=.tmp/product-experience-final-preview-contract.json" \
  "${PREVIEW_ARTIFACT[@]}"
```
