...
After the join, this step is template-only and foreground-only. Do not
clone/copy templates, start another scaffold job, or use a legacy fallback
path.
...
> "This folder contains generated app state and is not an approved resume target. Choose a new empty target or resume the existing app; refusing to overwrite it."
...
shell-safe quoting; do not ask for either value again.

On an approved resume, if `power.config.json` already has a non-empty
`environmentId`, verify that its environment and app display name match the
approved values. If both match, skip `init` and continue to the postcondition;
if either differs, STOP rather than overwriting it.

For every other invocation, Step 5 must leave `power.config.json` absent.
If a populated file remains, STOP and report its environment instead of
overwriting it or running `init` again.

When initialization is required, run:
...
If `npx power-apps init` fails, report the exact error and STOP—do not proceed.

Verify `power.config.json` exists and both its `environmentId` and
`appDisplayName` match the approved Step 2/Step 4 values, then update `Create
workflow checkpoint` in `memory-bank.md` to `Step 6 complete`.
