...
- **Scenario-aware rows.** Read `native-app-plan.md`, especially `### Shared
  Conventions`, each screen's workflow description, and its first-viewport
  requirements. Seed rows should exercise the app's actual workflow: statuses,
  dates, relationships, priority/severity, media metadata, and edge cases that
  make the planned first viewport light up.
