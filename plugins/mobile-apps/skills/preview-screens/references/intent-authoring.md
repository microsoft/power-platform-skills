...
the next action relates to its prerequisites. Preserve those choices in the authored HTML.
...
empty or later-stage preview is valid: label it and separately describe the app's first-entry
scope from Data/Navigation. Do not switch to All or add records to make the image look fuller.
Implement every offered filter and search with matching and no-match states; check
record identities and count labels in the source. A loaded-page count must not
...
Model two distinct records when a collection has them, including a read-only/completed variant
...
dimensions and implement loading/error fallback; download locally only when the delivery/offline
...
Inspect bindings statically before handoff:
...
- Trace each applicable scope/filter/search through its declared data source and affected
...
- Include compound filters, different selected records, invalid input and reset after edits
  in the authored handlers, not just the happy path. Handler presence is not evidence that
  the interaction was executed in a browser.
...
- Design phone previews for desktop and 320px widths without horizontal overflow of essential
  content/actions. Check unique HTML IDs, destination references, dialog focus/close/recovery,
  only declared verified image-media requests (no remote scripts or live data calls), and no
  unescaped user text.
- Review container nesting and information density in the authored layout. Simplify
  ...
  Review every selected screen's source, not just Home, including responsive rules, full
  fixed-height viewport, internal scrolling and bottom-action clearance.

These are source checks, not browser observations. Do not automatically run interactions,
capture screenshots, resize across a matrix or collect evidence records after authoring.
Deliver the HTML for user review; browser testing requires a separate explicit request.

## Authoring review

Check the proposed experience against approved requirements in source before foreground
design approval. This is not a screenshot-based test, beauty score or additional user gate.
Keep authored hierarchy, media, responsive layout and actions coherent without claiming that
their rendered appearance or native behavior has been verified.

| Check | Inspect in authored content and styles | Repair when |
|---|---|---|
...
| Media proportions (when relevant) | Asset choice, intended crop and reserved dimensions support recognition or reading | Missing assets or unsuitable crop settings undermine the intended evidence |
| Usable first viewport | Content order, responsive rules and scrolling preserve the task | Chrome, introduction or empty padding displaces the task in the proposed layout |
| Action placement | Labels, destinations, event handlers, focus behavior and scroll clearance support intended access | Source omits an action, recovery path or space for normal access |
...
| Density and typography | Text roles, target dimensions and wrapping rules support populated/long-content states | Tiny labels or controls make a dense layout fit, or excess chrome displaces task facts |
...
Preserve the target geometry and usable text/controls instead of padding or shrinking to fit
a screenshot. No fixed hero, image, palette, card count, density or similarity target applies.
Long content and large text may scroll; do not force every action above the fold.
Put preview limitations in compact review chrome, not repeated pinned explanations.
Static checks cannot establish effective rendered hit areas, crop quality or browser reachability.

Repair source inconsistencies before delivery. Fixed behavior/schema/route or explicit brand
changes return `NEEDS_CONTEXT` to foreground. Keep the normal required static gates; absence
of browser testing does not block completion or require an evidence file. Respond to user
feedback on the delivered preview without silently changing approved business scope.
...
Report static findings separately from any testing explicitly requested by the user.
