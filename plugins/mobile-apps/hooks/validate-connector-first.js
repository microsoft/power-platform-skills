...
const VIOLATIONS = [
  ...
  },
  {
    ...
    reason: 'Direct Microsoft Graph fetch is forbidden. Use the Office 365 / Graph connector via `npx --no-install pa app add data-source --non-interactive` and import from `src/generated/`.',
  },
  {
  ...
  },
  {
    ...
    reason: 'Direct Dataverse REST fetch is forbidden. Use the Dataverse generated service via `npx --no-install pa app add data-source --connector dataverse --org-url <environment-url> --table <table-logical-name> --non-interactive` from the app root.',
  },
];
...
process.stdin.on('end', () => {
  ...
  lines.push('');
  lines.push('Power Platform data MUST go through generated connector services in `src/generated/`. Add the connector via `npx --no-install pa app add data-source --non-interactive`, then import the typed client.');
