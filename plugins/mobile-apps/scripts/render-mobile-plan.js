  ...
  return `<div class="diagram er-diagram">
    <div class="er-toolbar">
      ...
      <div>
        ...
        <button type="button" class="er-small er-edit-controls" data-er-submit>Submit revision</button>
...
function renderPlanningProgress(status) {
  ...
  const items = stages.map(([label, detail], index) => {
    ...
    return `<article class="outcome ${state}" data-planning-stage="${index}"><div><strong>${escapeHtml(label)}</strong><span data-planning-state>${escapeHtml(state)}</span></div><p data-planning-detail>${escapeHtml(liveDetail)}</p></article>`;
  }).join('');
...
.concern{border-left:4px solid #dc2626;background:#fef2f2;color:#7f1d1d;padding:10px 12px;border-radius:6px}.verification-note{padding:10px 12px;border-radius:8px;margin-bottom:14px;background:#eff6ff;color:#1e3a8a}
.concept-banner{max-width:1356px;margin:14px auto 0;padding:18px 22px;border:3px solid #f97316;border-radius:12px;background:#fff7ed;color:#7c2d12;text-align:center;box-shadow:0 6px 18px #9a341226}.concept-banner strong{display:block;font-size:clamp(20px,3vw,30px);line-height:1.1;letter-spacing:.035em}.concept-banner span{display:block;margin-top:7px;font-size:14px;font-weight:650}
...
.notice{font-size:12px;color:#cbd5e1;margin-top:8px}.live-update-notice{font-size:12px;color:#fde68a;margin-top:7px;font-weight:700}@media(max-width:800px){.summary{grid-template-columns:repeat(2,1fr)}.layout{grid-template-columns:1fr}nav{position:static;display:flex;overflow:auto}}
...
<div class="meta"><span>Phase: <b data-live-phase>${escapeHtml(status.phase || 'planning')}</b></span><span data-live-message>${escapeHtml(status.message || 'Review the approved architecture and experience')}</span><span><b data-live-percent>${progress}%</b> complete</span></div>
<div class="bar"><span data-live-progress></span></div><div class="notice">Plan preview only — implementation has not started unless the status says otherwise.</div><div class="live-update-notice" data-live-notice aria-live="polite"></div></header>
<div class="input-banner" data-live-input${status.awaitingInput ? '' : ' hidden'}>${status.awaitingInput ? `Input required — ${escapeHtml(status.inputPrompt || 'return to the terminal to continue.')}` : ''}</div><div class="view-tabs"><button class="active" data-filter="architecture">Architecture</button><button data-filter="experience">Experience concept</button><button data-filter="implementation">Implementation status</button><button data-filter="all">All</button></div>
<div class="concept-banner" data-view-banner="experience" hidden><strong>DESIGN CONCEPT — NOT THE GENERATED APP</strong><span>This shows planned structure and visual direction with representative content. Generated TSX and the live device are authoritative.</span></div>
...
<script>
document.querySelectorAll('[data-filter]').forEach(function(button){button.addEventListener('click',function(){var filter=button.dataset.filter;document.querySelectorAll('[data-filter]').forEach(function(item){item.classList.toggle('active',item===button)});document.querySelectorAll('section[data-view],nav a[data-view]').forEach(function(item){item.hidden=filter!=='all'&&item.dataset.view!==filter});document.querySelectorAll('[data-view-banner]').forEach(function(item){item.hidden=filter!=='all'&&item.dataset.viewBanner!==filter});});});
...
function planProgress(status){if(Array.isArray(status.outcomes)&&status.outcomes.length){var total=Math.max(status.outcomes.length,Number(status.outcomeTotal||0));var complete=status.outcomes.filter(function(item){return item.state==='completed'}).length;return total?Math.round(complete/total*100):0}return status.total?Math.min(100,Math.round(Number(status.completed||0)/Number(status.total)*100)):0}
function applyLiveStatus(status){var progress=planProgress(status);var phase=document.querySelector('[data-live-phase]');var message=document.querySelector('[data-live-message]');var percent=document.querySelector('[data-live-percent]');var bar=document.querySelector('[data-live-progress]');var input=document.querySelector('[data-live-input]');if(phase)phase.textContent=status.phase||'planning';if(message)message.textContent=status.message||'Planning your mobile app';if(percent)percent.textContent=progress+'%';if(bar)bar.style.width=progress+'%';if(input){input.hidden=!status.awaitingInput;input.textContent=status.awaitingInput?'Input required — '+(status.inputPrompt||'return to the terminal to continue.'):''}document.querySelectorAll('[data-planning-stage]').forEach(function(card){var index=Number(card.dataset.planningStage);var completed=Math.max(0,Number(status.completed||0));var state=index<completed?'completed':index===completed&&status.state!=='blocked'?'running':index===completed&&status.state==='blocked'?'blocked':'pending';card.className='outcome '+state;var stateNode=card.querySelector('[data-planning-state]');if(stateNode)stateNode.textContent=state;if(state==='running'&&status.message){var detail=card.querySelector('[data-planning-detail]');if(detail)detail.textContent=status.message}})}
function savePlanViewState(){try{var active=document.querySelector('[data-filter].active');var open=Array.from(document.querySelectorAll('details.plan-subsection')).map(function(item,index){return item.open?index:-1}).filter(function(index){return index>=0});sessionStorage.setItem('mobile-plan-view',JSON.stringify({filter:active&&active.dataset.filter,open:open,scrollY:window.scrollY}))}catch{}}
function restorePlanViewState(){try{var value=JSON.parse(sessionStorage.getItem('mobile-plan-view')||'null');if(!value)return;if(value.filter){var button=document.querySelector('[data-filter="'+value.filter+'"]');if(button)button.click()}document.querySelectorAll('details.plan-subsection').forEach(function(item,index){item.open=value.open&&value.open.includes(index)});window.scrollTo(0,Number(value.scrollY||0));sessionStorage.removeItem('mobile-plan-view')}catch{}}
if(location.protocol==='http:'||location.protocol==='https:'){var token=new URLSearchParams(location.search).get('token');if(token){var events=new EventSource('/events?token='+encodeURIComponent(token));events.addEventListener('status',function(event){try{applyLiveStatus(JSON.parse(event.data))}catch{}});events.addEventListener('plan',function(){var editing=document.querySelector('.er-diagram.editing');var notice=document.querySelector('[data-live-notice]');if(editing){if(notice)notice.textContent='A newer plan is ready. Finish or reset the ER edit before refreshing.';return}savePlanViewState();location.reload()});events.addEventListener('revision',function(){var notice=document.querySelector('[data-live-notice]');if(notice)notice.textContent='ER revision saved. Choose “Apply browser revision” in the Gate 2 terminal prompt.'});events.addEventListener('server-error',function(event){var notice=document.querySelector('[data-live-notice]');if(notice)notice.textContent='Planning companion warning: '+event.data});events.onerror=function(){var notice=document.querySelector('[data-live-notice]');if(notice)notice.textContent='Live updates disconnected. The static Plan HTML remains available.'}}}
document.querySelectorAll('.er-diagram').forEach(function(diagram){
  ...
  diagram.addEventListener('click',function(event){var target=event.target;
    ...
    if(target.closest('[data-er-submit]')){var token=new URLSearchParams(location.search).get('token');if(location.protocol!=='http:'&&location.protocol!=='https:'){status.textContent='Live submission requires the localhost planning companion. Use Copy revision instead.';return}fetch('/api/gate-2/revision?token='+encodeURIComponent(token||''),{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(model())}).then(function(response){return response.json().then(function(value){if(!response.ok)throw new Error(value.error||'Revision rejected');return value})}).then(function(){status.textContent='Revision saved. Return to the terminal and choose Apply browser revision.'}).catch(function(error){status.textContent=error.message});return}
