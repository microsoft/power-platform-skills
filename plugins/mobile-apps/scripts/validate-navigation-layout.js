...
function navigatorEntryName(file, appRoot) {
  const segments = path.relative(appRoot, file).replace(/\\/g, '/').replace(/\.tsx$/, '').split('/');
  for (let length = 1; length < segments.length; length += 1) {
    const folder = segments.slice(0, length).join('/');
    if (fs.existsSync(path.join(appRoot, folder, '_layout.tsx'))) return folder;
  }
...
function validateNavigationLayout(projectRoot, manifest) {
  ...
  const expectedByEntry = new Map(expectedDestinations.map((destination) => {
    const source = routeFiles.find((entry) => entry.route === destination.targetPath);
    return [source ? navigatorEntryName(source.file, appRoot) : routeEntryName(destination.targetPath), destination];
  }));
  ...
  for (const registration of expectedNavigator === 'Stack' ? [] : visibleRegistrations) {
  ...
  if (manifest.pattern === 'tabs-plus-stacks') {
    ...
    const childEntries = new Set(routeFiles.map((entry) => navigatorEntryName(entry.file, appRoot)));
    for (const entryName of childEntries) {
