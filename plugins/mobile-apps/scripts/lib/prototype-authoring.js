...
function configurePrototypeAuthoring(projectRoot, { check = false, readyScreenIds = [], screenSources, wireScreenIds = [] } = {}) {
  ...
  return {
    ...
    screenSources: registry.screens.map(({ screenId, route, sourceFile }) => ({ screenId, route, sourceFile })),
    authoringContract: {
      module: '@/authoring', screenComponent: 'AuthoringScreen', screenIdProp: 'screenId',
      readyProp: 'ready', dirtyProp: 'hasUnsavedChanges',
      targetComponent: 'AuthoringTarget', targetIdProp: 'targetId',
      scrollHook: 'useAuthoringRemeasure', scrollEventThrottle: 16,
    },
