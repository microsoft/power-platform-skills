...
function statusError(status, code) {
  if (status === 403 && ['approval_required', 'dataverse_approval_required', 'dataverse_plan_required'].includes(code)) {
    return new AuthoringError('approval-required', 'The current plan requires maker approval before this operation. Review the current plan in Player; the runner credential was not revoked.');
  }
  if (status === 401 || status === 403 && code === 'runner_unauthorized') return new AuthoringError('revoked', 'Authoring runner authorization is missing or revoked');
  if (status === 403) return new AuthoringError('forbidden', 'The bridge denied this operation. Review its permissions and approvals before continuing.');
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 403) {
            // Only fixed error codes influence classification. Error text may
            // contain credentials or source and is never copied into a CLI error.
            const rejected = await readResponse(response, MAX_JSON_BYTES);
            code = rejected && typeof rejected === 'object' && typeof rejected.code === 'string' ? rejected.code : undefined;
          } else await response.body?.cancel();
          throw statusError(response.status, code);
        }
