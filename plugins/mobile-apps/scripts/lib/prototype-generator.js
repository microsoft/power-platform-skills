...
const { readBundledImages } = require('./prototype-image-assets');
...
function sampleImagesSource(images, facts) {
  return `import type { ImageSourcePropType } from 'react-native';
import type { SampleImageAsset } from './model';

const images: Record<string, { uri: string; source: ImageSourcePropType }> = {
${images.map(image => `  ${JSON.stringify(image.key)}: {
    uri: ${JSON.stringify(facts.mediaAssets.find(asset => asset.key === image.key).source.value)},
    source: require(${JSON.stringify(`../../${image.path}`)}),
  },`).join('\n')}
};

export function getBundledSampleImage(asset: SampleImageAsset | null): ImageSourcePropType | undefined {
  if (!asset || !Object.prototype.hasOwnProperty.call(images, asset.key)) return undefined;
  const image = images[asset.key];
  return image.uri === asset.source.value ? image.source : undefined;
}
`;
}
...
function generatePrototype(root, { check = false, validateOnly = false } = {}) {
  ...
  const facts = readJson(root, PATHS.facts);
  const compiled = compilePrototype(domain, readJson(root, PATHS.bindings), {
    persistence, facts,
  }, rules);
  const images = readBundledImages(root, facts);
  if (images.length) compiled.inputRevisions.imageAssets = revision(images);
...
`,
    ...
    'src/data/sample-images.ts': sampleImagesSource(images, facts),
