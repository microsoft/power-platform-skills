...
function gateBinding(root, gate) {
  ...
  const phone = require('./phone-app-plan');
  if (phone.isPhoneApp(root)) {
    phone.compilePhoneScreens(root, { check: true });
    const required = [
      phone.INPUT, phone.COMPILED, phone.NAVIGATION,
      '.tmp/product-scope-contract.json', '.tmp/architecture-decisions.json', '.tmp/persistence-contract.json',
      ...PROTOTYPE_CANONICAL,
    ];
    const optional = [
      '.tmp/dataverse-schema-contract.json', '.tmp/dataverse-execution-contract.json',
      '.tmp/dataverse-operation-manifest.json', '.tmp/dataverse-execution-reconciliation.json',
      'brand/design-system.md', 'brand/tokens.ts',
    ].filter(file => exists(root, file));
    const binding = artifactBinding(root, [...required, ...optional]);
    const plan = readFile(inside(root, 'native-app-plan.md')).toString('utf8');
    const sections = Object.fromEntries(['App Requirements', 'Data Model', 'Native Capabilities', 'Connectors', 'Screens']
      .map(heading => [heading, digest(planSection(plan, heading))]));
