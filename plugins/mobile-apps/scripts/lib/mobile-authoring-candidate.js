...
function runCandidateChecks(root, screenFiles, { run = spawnSync } = {}) {
  ...
  const checks = [
    ...
    ...(fs.existsSync(path.join(root, '.tmp/prototype-image-assets.json')) ? [['sample-image-assets', [
      path.join(PLUGIN_ROOT, 'scripts/materialize-prototype-images.js'), '--project-root', root, '--check',
    ]]] : []),
...
async function prepareCandidate(client, { readyScreenIds, final = false, validate = runCandidateChecks } = {}) {
  ...
  let screenFiles = readyFiles(root, before, readyScreenIds);
  if (final && ['edit', 'teach'].includes(descriptor.operation)) {
    // A final app replacement must also revalidate unchanged destinations, not
    // publish only the edited screen and lose their native authoring readiness.
    readyScreenIds = readJson(root, '.tmp/compiled-screen-build-pack.json').screens.map((screen) => screen.screenId);
    screenFiles = readyFiles(root, before, readyScreenIds);
  }
