...
function renderMakerSummary(model) {
  ...
  return `<section class="maker-summary" aria-labelledby="maker-summary-title">
    ...
    <div class="summary-grid">
      ...
      <article class="summary-block"><small>Data ownership</small><h3>Prototype and real data</h3>${owners.length ? `<ul class="plain-list">${owners.map((item) => `<li><strong>${escapeHtml(item.name)}</strong><span>${escapeHtml(statusLabel(item.role))} · ${escapeHtml(statusLabel(item.realization))}</span>${item.note ? `<small>${escapeHtml(item.note)}</small>` : ''}</li>`).join('')}</ul>` : '<p class="muted-copy">Data ownership is being resolved.</p>'}</article>
