...
function validateScreenContract(contract, screenContract, issues) {
  ...
  for (const [field, expectedValue] of fields) {
    const routeIndexEquivalent = field === 'file'
      && primary.route === contract.primaryScreen.route
      && primary[field] === expectedValue.replace(/\.tsx$/, '/index.tsx');
    if (primary[field] !== expectedValue && !routeIndexEquivalent) {
...
function validateBuiltPrimary(projectRoot, contract, screenContract, foundation, issues) {
  const primary = screenContract?.screens?.find((screen) => screen.role === 'primary') || screenContract?.primaryScreen;
  if (!primary) return;
...
function validateBuildPackAgreement(projectRoot, contract, screenContract, issues) {
  ...
  const keyFlowScreen = (pack.screens || []).find((screen) => screen.role === 'key-flow');
  const keyFlowRoute = keyFlowScreen?.route || pack.navigation?.keyFlowRoute;
  if (pack.experience?.entryMode !== contract.entryMode || pack.experience?.primarySurface !== contract.primarySurface || launchRoute !== contract.primaryScreen.route || keyFlowRoute !== screenContract?.keyFlow?.route) {
