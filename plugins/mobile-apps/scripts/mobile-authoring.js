...
function capabilities() {
  ...
  const operations = [];
  if (candidates && [
    'skills/create-mobile-prototype/SKILL.md', 'scripts/generate-prototype.js',
    'scripts/compile-phone-app-plan.js', 'scripts/compile-screen-build-pack.js',
    'scripts/lib/phone-app-plan.js', 'skills/create-mobile-app/references/phone-app-profile.md',
  ].every(present)) operations.push('prototype');
...
async function run(args, options = {}) {
  ...
  if (args.command === 'request-question') {
    ...
    if (args.recordGate !== undefined && result.receipt.action === 'approve') {
      ...
      // Phone workflows consume this persisted signed receipt directly. Do not
      // overwrite main's separate Dataverse schema receipt with a legacy plan.
      if (!require('./lib/phone-app-plan').isPhoneApp(root)) {
  ...
  if (args.command === 'event') {
    ...
    const input = readJson(root, args.input);
    if (client.descriptor.operation === 'prototype' && input.kind === 'plan'
      && (!Array.isArray(input.screens) || input.screens.length < 2 || input.screens.length > 3)) {
      throw new Error('The first app plan must contain 2-3 usable screens. Save the remainder for Build more screens.');
    }
    await client.event(input);
