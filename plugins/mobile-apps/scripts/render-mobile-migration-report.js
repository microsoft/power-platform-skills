...
function main() {
  ...
  const behaviorContract = readJson(cli.dir, 'behavior-contract.json', { stats: {}, shards: [] });
  const criticalObligations = readJson(cli.dir, 'critical-obligations.json', { stats: {}, obligations: [] });
  ...
  const workflowPlan = readJson(cli.dir, 'workflows.json', { stats: {}, workflows: [] });
  ...
  const semanticReview = (controls.rows || []).filter((row) =>
    row.role === 'repeating-records-review' || row.role === 'component-review');
  ...
  const workflowRows = workflowPlan.workflows || [];
  const workflowPending = workflowRows.filter((row) => row.approval?.status === 'pending');
  const workflowHardBlocks = workflowRows.filter((row) => row.approval?.status === 'blocked');
  const workflowUnresolvedDecisions = Number(workflowPlan.stats?.unresolvedDecisions || 0);
  const blockers = [
    ...(validation.errors || []),
    ...
    ...workflowHardBlocks.map((row) => `Workflow is blocked: ${row.source?.screen}/${row.source?.control}.${row.source?.event}`),
  ];
  const reviewCount = unmatched.length + unsupported.length + highRisk.length + semanticReview.length + missingFlows.length + pcfPending.length
    + workflowPending.length + workflowUnresolvedDecisions + (validation.warnings || []).length;
  ...
  const obligationStats = criticalObligations.stats || {};
  const designBaseline = input.app?.sourceDesignBaseline || null;
...
<section class="hero"><div><div class="eyebrow">Canvas → native / ${esc(input.migrationMode || 'modernize')}</div><h1>${esc(input.app?.name || 'Canvas app')}</h1><p>This report measures what can be carried into the current native mobile workflow before any target app is changed. It tracks business behavior, data, shared commands, reachability, saved-view security semantics, and source identity—not Canvas pixels.</p>${badge(status,statusTone)}</div><div class="ledger"><div class="ledger-row"><span>Source actions</span><strong>${behaviorSource}</strong></div><div class="ledger-row"><span>Accounted</span><strong>${behaviorAccounted}</strong></div><div class="ledger-row"><span>Dropped</span><strong>${dropped}</strong></div><div class="ledger-row"><span>Critical obligations</span><strong>${obligationStats.critical || 0}</strong></div></div></section>
<section class="metrics"><div class="metric"><strong>${screens.length}</strong><span>Screens</span></div><div class="metric"><strong>${controls.stats?.totalControls || controls.rows?.length || 0}</strong><span>Controls</span></div><div class="metric"><strong>${tables.length}</strong><span>Dataverse tables</span></div><div class="metric"><strong>${requirements.length}</strong><span>Connections / flows</span></div><div class="metric"><strong>${highRisk.length}</strong><span>High-risk controls</span></div><div class="metric"><strong>${workflowRows.length}</strong><span>Decomposed workflows</span></div><div class="metric"><strong>${obligationStats.sourceScreens || 0}</strong><span>Screen obligations</span></div><div class="metric"><strong>${obligationStats.componentCommands || 0}</strong><span>Shared commands</span></div><div class="metric"><strong>${obligationStats.componentCommandPlacements || 0}</strong><span>Command placements</span></div><div class="metric"><strong>${obligationStats.savedViews || 0}</strong><span>Saved views</span></div><div class="metric"><strong>${obligationStats.navigation || 0}</strong><span>Navigation obligations</span></div></section>
<section class="grid">
<div class="card"><h2>Readiness</h2>${blockers.length ? list(blockers,(item)=>`<span class="mono">${esc(item)}</span>`) : '<p>No blocking contract errors detected.</p>'}<h3>Review queue</h3><p>${reviewCount} item(s): ${unmatched.length} unmatched formula(s), ${unsupported.length} unsupported item(s), ${highRisk.length} high-risk control(s), ${semanticReview.length} semantic control review(s), ${missingFlows.length} target flow ID(s), ${pcfPending.length} PCF approval(s), ${workflowPending.length} workflow approval(s), ${workflowUnresolvedDecisions} correctness-critical workflow answer(s), ${(validation.warnings||[]).length} validation warning(s).</p></div>
...
<div class="card"><h2>Semantic control review</h2>${list(semanticReview.slice(0,24),(row)=>`<strong>${esc(row.screen)} / ${esc(row.control)}</strong><br>${esc(row.role)} — ${esc((row.roleEvidence?.signals||[]).join(', ')||row.nativeSuggestion)}`)}</div>
<div class="card wide"><h2>Core behavior and regenerated native intent</h2><p>${badge(`${behaviorContract.stats?.coreBehaviors||0} exact core`,'good')} ${badge(`${behaviorContract.stats?.builderCoreBehaviors ?? behaviorContract.stats?.coreBehaviors ?? 0} builder-owned`,'info')} ${badge(`${behaviorContract.stats?.workflowCoreBehaviors||0} workflow-owned`,'warn')} ${badge(`${behaviorContract.stats?.regenerableBehaviors||0} native intent`,'info')} ${badge(`${behaviorContract.stats?.shards||0} screen/App feeds`,'warn')} ${badge(`${behaviorContract.stats?.workflowShards||0} workflow feeds`,'warn')}</p><p>The lossless global ledger remains available to deterministic coverage. Screen builders receive only compact screen-owned behavior/intent plus workflow call refs; pathological handler actions and outcomes are isolated in bounded orchestrator-only implementation feeds.</p></div>
<div class="card wide"><h2>Critical source obligations</h2><p>${badge(`${obligationStats.critical||0} total`,'good')} ${badge(`${obligationStats.componentCommands||0} shared commands`,'info')} ${badge(`${obligationStats.componentCommandPlacements||0} placements`,'info')} ${badge(`${obligationStats.navigation||0} navigation`,'warn')} ${badge(`${obligationStats.savedViews||0} saved views`,'warn')} ${badge(`${obligationStats.designBaselines||0} design baseline`,'info')}</p><p>Final generation requires 100% exact implementation evidence or an explicit user-approved semantic delta. Weighted behavior coverage cannot waive these obligations.</p>${designBaseline ? `<p><strong>Source identity:</strong> ${esc((designBaseline.colors||[]).slice(0,6).map((color)=>color.value).join(', ')||'theme captured')} · ${esc(designBaseline.typography?.dominantFont||'font not dominant')} · confidence ${esc(designBaseline.confidence||'unknown')}</p>` : '<p class="empty">No high-confidence source design baseline was captured.</p>'}</div>
...
<div class="card wide"><h2>Pathological event workflows</h2>${workflowRows.length ? `<p>AI owns routine code and UX decomposition. Only correctness-critical questions require user input before one workflow approval.</p><table><thead><tr><th>Workflow</th><th>Source</th><th>Signals</th><th>Steps</th><th>Critical questions</th><th>Approval</th></tr></thead><tbody>${workflowRows.map((row)=>`<tr><td><code>${esc(row.workflowId)}</code></td><td>${esc(row.source?.screen)} / ${esc(row.source?.control)}.${esc(row.source?.event)}</td><td>${esc((row.detection?.reasons||[]).join(', '))}</td><td>${esc(row.proposal?.steps?.length||0)}</td><td>${esc(row.requiredDecisions?.length||0)}</td><td>${badge(row.approval?.status||'missing',row.approval?.status==='approved'?'good':row.approval?.status==='blocked'?'bad':'warn')}</td></tr>`).join('')}</tbody></table>` : '<p>No event handler crossed the deterministic decomposition threshold.</p>'}</div>
