...
<div class="card wide"><h2>Core behavior and regenerated native intent</h2><p>${badge(`${behaviorContract.stats?.coreBehaviors||0} exact core`,'good')} ${badge(`${behaviorContract.stats?.regenerableBehaviors||0} native intent`,'info')} ${badge(`${behaviorContract.stats?.shards||0} builder shards`,'warn')}</p><p>The lossless global ledger remains available to deterministic coverage. Builders receive only their screen's exact load-bearing behavior and structured, raw-free intent hints for disconnected Canvas UI plumbing.</p></div>
