...
const { readyFiles, prepareCandidate } = require('../lib/mobile-authoring-candidate');
...
test('final edits revalidate unchanged destinations before publishing the complete app', { skip: HAS_SOURCE ? false : SOURCE_REASON }, async (t) => {
  const f = fixture(t);
  f.descriptor.operation = 'edit';
  const compiled = readJson(f.root, '.tmp/compiled-screen-build-pack.json');
  compiled.screens.push({ screenId: 'inspection-details', route: '/inspection-details' });
  write(f.root, '.tmp/compiled-screen-build-pack.json', compiled);
  write(f.root, 'app/(app)/inspection-details.tsx', 'export default function Details() { return null; }\n');
  const result = await prepareCandidate(f.client, {
    readyScreenIds: ['inspection-list'], final: true,
    validate: (_root, files) => {
      assert.deepEqual(files, ['app/(app)/inspections/index.tsx', 'app/(app)/inspection-details.tsx']);
      return ['all-screen-fixture-check'];
    },
  });
  assert.deepEqual(result.candidate.readyScreenIds, ['inspection-list', 'inspection-details']);
  assert.deepEqual(result.candidate.checks, ['all-screen-fixture-check']);
