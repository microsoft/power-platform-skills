...
test('preparation accepts the approved plan produced before Step 5', () => {
  const projectRoot = copyTemplate();
  fs.writeFileSync(
    path.join(projectRoot, 'native-app-plan.md'),
    '# Native app plan\n\nApproved planning artifact.\n',
  );

  prepareMobileTemplate({
    workingDir: projectRoot,
    displayName: 'Planned App',
    slug: 'planned-app',
  });
...
test('preparation still rejects post-scaffold created-app markers', () => {
  const projectRoot = copyTemplate();
  fs.writeFileSync(path.join(projectRoot, 'memory-bank.md'), '# Existing app\n');

  assert.throws(() => prepareMobileTemplate({
    workingDir: projectRoot,
    displayName: 'Existing App',
    slug: 'existing-app',
  }), /Template already contains created-app markers: memory-bank\.md/);
});
...
test('root preparation accepts semicolonless imports without duplicating them', () => {
...
test('root preparation creates the exact colorScheme binding used by provider props', () => {
  const projectRoot = writeLayoutFixture(`${fixtureImports}
import { useColorScheme } from 'react-native';
export default function RootLayout() {
