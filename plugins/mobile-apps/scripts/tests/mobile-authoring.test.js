...
async function fixture(t, overrides = {}) {
  ...
  const server = http.createServer(async (request, response) => {
    ...
    if (state.status !== 200) {
      ...
      response.writeHead(state.status).end(JSON.stringify({ error: token, ...(state.errorCode ? { code: state.errorCode } : {}) }));
      return;
    }
...
test('SDK metadata grant refusal, invalid/oversized responses and credentials never become a fallback fetch', async (t) => {
  ...
  for (const status of [401, 403, 409, 410]) {
    ...
    await assert.rejects(f.client().connectorMetadata(input), (error) => {
      ...
      return /revoked|stale|cancelled|denied/.test(error.message);
    });
...
test('Dataverse execution never blindly retries a write, follows redirects, or continues after a revoked grant', async (t) => {
  ...
  for (const status of [401, 403, 409, 410, 503]) {
    ...
    await assert.rejects(client.dataverse(input), (error) => {
      assert.ok(!error.message.includes(f.token));
      return /revoked|stale|cancelled|denied|outcome is unknown/.test(error.message);
    });
...
test('missing plan approval is not reported as revoked runner authorization', async (t) => {
  const f = await fixture(t, { status: 403, errorCode: 'approval_required' });
  await assert.rejects(f.client().verify(), (error) => {
    assert.equal(error.code, 'approval-required');
    assert.match(error.message, /maker approval/);
    assert.ok(!error.message.includes(f.token));
    return true;
  });
  assert.equal(f.state.requests.length, 1);
  f.state.errorCode = 'unrecognized';
  await assert.rejects(f.client().verify(), (error) => error.code === 'forbidden');
  f.state.errorCode = 'runner_unauthorized';
  await assert.rejects(f.client().verify(), (error) => error.code === 'revoked');
});
...
test('all nested foreground questions inherit Player transport and standalone editing keeps its ordinary question tool', () => {
  const root = path.resolve(__dirname, '../..');
  const core = fs.readFileSync(path.join(root, 'shared/shared-instructions.md'), 'utf8');
  ...
  assert.match(core, /exact wrapper for every plugin script, including nested skills/);
  assert.match(core, /Product\s+questions and approvals stay in Player/);
  assert.match(core, /question tool actually\s+exposed by its host/);
  assert.match(transport, /Do not also call `AskUserQuestion`/);
  assert.match(transport, /every nested foreground skill/);
  assert.match(workflow, /native-app-plan\.md` remains the source of truth/);
  assert.match(workflow, /player-authoring\.md/);
  assert.match(reference, /All inspection[\s\S]*saves/);
  assert.match(reference, /do not issue a[\s\S]*second Apply question/);
  assert.match(reference, /ownership changes reopen Gate 1/);
});
