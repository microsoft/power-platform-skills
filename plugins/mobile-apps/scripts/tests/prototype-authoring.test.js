...
test('planned screen shells can prepare the registry before targets or readiness exist', options, (context) => {
  const root = project(context);
  const skeleton = 'export default function Home() { return null; }\n';
  write(root, 'app/(app)/home.tsx', skeleton);
  const sources = [{ screenId: 'home', sourceFile: 'app/(app)/home.tsx' }];
  const registry = compileRegistry(root, ts, [], sources);
  assert.deepEqual(registry.screens[0].targets, []);
  assert.equal(fs.readFileSync(path.join(root, 'app/(app)/home.tsx'), 'utf8'), skeleton);
  assert.throws(() => compileRegistry(root, ts, ['home'], sources), /const authoringTargets/);
});
