...
test('environment discovery is non-persisting before the rough plan gate', () => {
  ...
  assert.doesNotMatch(
    beforePreview,
    /write `native-app-plan\.md` placeholder/i,
  );
...
test('app identity and Power Apps initialization respect existing state', () => {
  ...
  assert.match(initialize, /scripts\/initialize-mobile-app\.js/);
  assert.match(initialize, /generated `environmentId` and `appDisplayName`/);
  assert.match(initialize, /argument array rather than shell interpolation/);
  assert.doesNotMatch(initialize, /--display-name '<displayName>'/);
});

test('Step 5 accepts the plan created by Step 3 while retaining created-app guards', () => {
  const start = skill.indexOf('### Step 5 — Prepare existing template');
  const end = skill.indexOf('### Step 6 — Initialize');
  const step = skill.slice(start, end);

  assert.match(step, /`native-app-plan\.md` is expected here because Step 3 owns planning/);
  assert.doesNotMatch(
    step,
    /already-created markers appear \(`memory-bank\.md`, `native-app-plan\.md`/,
  );
});
