...
const { materializePrototypeImages, readBundledImages, inspectImage, downloadImage } = require('../lib/prototype-image-assets');
...
const pngBytes = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+aWQAAAABJRU5ErkJggg==', 'base64');
...
test('sample images materialize once, preserve canonical fixtures, and verify local bytes without network', async t => {
  const { root, facts } = project(t, null);
  const before = fs.readFileSync(path.join(root, 'src/data/fixtures.ts'), 'utf8');
  const requested = [];
  const download = async url => { requested.push(url); return { bytes: pngBytes, mimeType: 'image/png' }; };
  const result = await materializePrototypeImages(root, { download });
  assert.equal(result.ok, true);
  assert.equal(result.bundledCount, 1);
  assert.deepEqual(requested, [facts.mediaAssets[0].source.value]);
  const [image] = readBundledImages(root, facts);
  assert.match(image.path, /^assets\/sample-media\/[a-f0-9]{64}\.png$/);
  assert.deepEqual(fs.readFileSync(path.join(root, image.path)), pngBytes);
  assert.equal(image.width, 1);
  assert.equal(image.height, 1);
  assert.equal(image.byteLength, pngBytes.length);
  assert.deepEqual(JSON.parse(fs.readFileSync(path.join(root, '.tmp/scenario-facts.json'))), facts);
  assert.equal(fs.readFileSync(path.join(root, 'src/data/fixtures.ts'), 'utf8'), before);
  await materializePrototypeImages(root, { download });
  await materializePrototypeImages(root, { check: true, download: () => { throw new Error('No network during check'); } });
  assert.equal(requested.length, 1);
  fs.appendFileSync(path.join(root, image.path), 'changed');
  assert.throws(() => readBundledImages(root, facts), /changed|corrupt/);
  await assert.rejects(materializePrototypeImages(root, { download }), /changed|corrupt/);
});

test('corrupt, oversized and disguised non-image responses cannot become bundled samples', async t => {
  for (const [bytes, mimeType] of [
    [Buffer.from('<html>Unavailable</html>'), 'image/jpeg'],
    [Buffer.alloc(5 * 1024 * 1024 + 1), 'image/png'],
    [pngBytes, 'text/html'],
    [pngBytes, 'image/jpeg'],
  ]) {
    const { root } = project(t, null);
    await assert.rejects(materializePrototypeImages(root, { download: async () => ({ bytes, mimeType }) }), /image|Image/);
    assert.equal(fs.existsSync(path.join(root, '.tmp/prototype-image-assets.json')), false);
    assert.equal(fs.existsSync(path.join(root, 'assets/sample-media')), false);
  }
  const huge = Buffer.from(pngBytes);
  huge.writeUInt32BE(100000, 16);
  assert.throws(() => inspectImage(huge, 'image/png'), /dimensions|pixels/);
});

test('sample downloads reject private DNS and credential URLs before requesting bytes', async () => {
  for (const address of ['127.0.0.1', '10.0.0.4', '169.254.169.254', '192.168.1.5', '::1', 'fc00::1']) {
    await assert.rejects(downloadImage(sampleImageAsset().source.value, {
      lookup: async () => [{ address, family: address.includes(':') ? 6 : 4 }],
      request: () => { throw new Error('Network must not be reached'); },
    }), /public|private/);
  }
  await assert.rejects(downloadImage('https://user:secret@photos.example.com/image.jpg'), /public|credentials/);
});

test('sample downloads pin DNS, recheck redirects, and reject invalid HTTP responses', async () => {
  ...
  const request = (url, options, respond) => {
    ...
    queueMicrotask(() => {
      ...
      const response = Readable.from(next.chunks ?? [pngBytes]);
  ...
  const url = sampleImageAsset().source.value;
  responses = [{ statusCode: 302, headers: { location: '/fixed-image.png' } }, {}];
  const result = await downloadImage(url, { request, lookup: async () => addresses });
  assert.deepEqual(result.bytes, pngBytes);
  assert.equal(calls.length, 2);
  assert.equal(new URL(calls[1]).pathname, '/fixed-image.png');

  for (const location of ['https://127.0.0.1/image.png', 'http://photos.example.com/image.png',
    'https://photos.example.com/image.png?access_token=secret', 'https://private.example.com/image.png']) {
    const previous = calls.length;
    responses = [{ statusCode: 302, headers: { location } }];
    await assert.rejects(downloadImage(url, { request, lookup: async host => host === 'private.example.com'
      ? [{ address: '10.0.0.4', family: 4 }] : addresses }), /public|credential/);
    assert.equal(calls.length, previous + 1);
  }
  responses = Array.from({ length: 4 }, () => ({ statusCode: 302, headers: { location: '/same.png' } }));
  await assert.rejects(downloadImage(url, { request, lookup: async () => addresses }), /redirect limit/);
  for (const response of [
    { statusCode: 404 }, { headers: { 'content-type': 'text/html' } },
    { headers: { 'content-type': 'image/png', 'content-length': String(6 * 1024 * 1024) } },
    { chunks: [Buffer.alloc(5 * 1024 * 1024), pngBytes] },
  ]) {
    responses = [response];
    await assert.rejects(downloadImage(url, { request, lookup: async () => addresses }), /image|Image/);
  }
});

test('cancelled or changed image inputs never commit downloaded assets', async t => {
  for (const mode of ['cancelled', 'changed']) {
    const { root, facts } = project(t, null);
    let downloaded = false;
    await assert.rejects(materializePrototypeImages(root, {
      verify: async () => { if (mode === 'cancelled' && downloaded) throw new Error('Job cancelled'); },
      download: async () => {
        downloaded = true;
        if (mode === 'changed') write(root, '.tmp/scenario-facts.json', { ...facts, records: [] });
...
test('bundled image checks reject stale canonical metadata instead of reseeding or fetching', async t => {
  const { root, facts } = project(t, null);
  await materializePrototypeImages(root, { download: async () => ({ bytes: pngBytes, mimeType: 'image/png' }) });
  facts.mediaAssets[0].provenance.changes = 'A reviewed replacement image.';
  delete facts.scenarioRevision;
  facts.scenarioRevision = revision(facts);
  write(root, '.tmp/scenario-facts.json', facts);
  await assert.rejects(materializePrototypeImages(root, { check: true }), /missing or stale/);
});

test('the image CLI checks an explicit project from another working directory and candidates include the same gate', async testContext => {
  const { root } = project(testContext, null);
  await materializePrototypeImages(root, { download: async () => ({ bytes: pngBytes, mimeType: 'image/png' }) });
  const manifestBefore = fs.readFileSync(path.join(root, '.tmp/prototype-image-assets.json'), 'utf8');
  const result = spawnSync(process.execPath, [path.join(PLUGIN, 'scripts/materialize-prototype-images.js'),
    '--project-root', root, '--check'], {
    cwd: PLUGIN, encoding: 'utf8', env: { ...process.env, POWER_PLATFORM_SKILLS_TELEMETRY_MOBILE_APP_OPTOUT: '1' },
  });
  assert.equal(result.status, 0, result.stderr);
  assert.equal(JSON.parse(result.stdout).bundledCount, 1);
  assert.equal(fs.readFileSync(path.join(root, '.tmp/prototype-image-assets.json'), 'utf8'), manifestBefore);
  write(root, 'node_modules/typescript/bin/tsc', 'test compiler placeholder');
  const commands = [];
  const checks = runCandidateChecks(root, [], { run(command, args, options) {
    assert.equal(options.cwd, root);
    commands.push(args);
    return { status: 0, stdout: JSON.stringify({ issues: [] }) };
  } });
  assert.ok(checks.includes('sample-image-assets'));
  assert.ok(commands.some(args => args[0].endsWith('materialize-prototype-images.js')
    && args.includes(root) && args.includes('--check')));
});
...
function imageComponent(root) {
  ...
  compiled.require = (specifier) => {
    ...
    if (specifier === './sample-images') {
      const bundledFile = path.join(root, 'src/data/sample-images.ts');
      const bundled = new Module(bundledFile);
      bundled.filename = bundledFile;
      bundled.require = name => {
        if (name.startsWith('../../assets/sample-media/')) {
          assert.ok(fs.existsSync(path.resolve(path.dirname(bundledFile), name)));
          return 42;
        }
        return originalRequire(name);
      };
      bundled._compile(ts.transpileModule(fs.readFileSync(bundledFile, 'utf8'), {
        compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2020 },
      }).outputText, bundledFile);
...
test('generated images use the bundled canonical bytes without overriding later source or captured-photo changes', installed, async t => {
  const { root, facts } = project(t);
  await materializePrototypeImages(root, { download: async () => ({ bytes: pngBytes, mimeType: 'image/png' }) });
  const generated = generatePrototype(root);
  assert.ok(generated.files.includes('src/data/sample-images.ts'));
  assert.equal(generatePrototype(root, { check: true }).ok, true);
  const h = imageComponent(root);
  h.render({ photo: photo(), alt: 'Destination' });
  assert.equal(h.all('Image')[0].props.source, 42);
  h.all('Image')[0].props.onError();
  h.render({ photo: photo(), alt: 'Destination' });
  assert.equal(h.all('Image').length, 0);
  assert.equal(h.roles('link').length, 2);
  const changed = sampleImageAsset();
  changed.source.value = changed.source.value.replace('960px', '1280px');
  h.render({ photo: photo(changed), alt: 'Changed destination' });
  assert.equal(h.all('Image')[0].props.source.uri, changed.source.value);
  h.render({ photo: { status: 'ready', id: facts.mediaAssets[0].key, uri: 'file:///capture.jpg' }, alt: 'Captured' });
  assert.equal(h.all('Image')[0].props.source.uri, 'file:///capture.jpg');
  assert.equal(h.roles('link').length, 0);
  const [entry] = readBundledImages(root, facts);
  fs.appendFileSync(path.join(root, entry.path), 'tampered');
  assert.throws(() => generatePrototype(root, { check: true }), /changed|corrupt/);
});

test('compact credits preserve creator, license, full attribution, links and failure feedback behind an explicit disclosure', installed, async t => {
  const { root } = project(t);
  const h = imageComponent(root);
  const props = { photo: photo(), alt: 'Destination', creditDisplay: 'compact', aspectRatio: 16 / 9 };
  h.render(props);
  const toggle = h.roles('button')[0];
  assert.equal(toggle.props.accessibilityState.expanded, false);
  assert.match(toggle.props.accessibilityLabel, /Hannes Röst.*CC BY-SA 3.0/);
  assert.equal(h.roles('link').length, 0);
  assert.ok(h.all('View').some(node => Array.isArray(node.props.style)
    && node.props.style.some(style => style?.aspectRatio === 16 / 9)));
  toggle.props.onPress();
  h.render(props);
  assert.equal(h.roles('button')[0].props.accessibilityState.expanded, true);
  assert.equal(h.roles('link').length, 2);
  assert.ok(h.all('Text').some(node => node.props.children === props.photo.sample.asset.provenance.changes));
  h.failCredit();
  h.roles('link')[0].props.onPress();
  await new Promise(resolve => setImmediate(resolve));
  h.render(props);
...
test('canonical repository images, actual screen wiring and gated startup compile together against installed template packages', installed, async (t) => {
  const { root, facts } = project(t);
  await materializePrototypeImages(root, { download: async () => ({ bytes: pngBytes, mimeType: 'image/png' }) });
  const generated = generatePrototype(root);
