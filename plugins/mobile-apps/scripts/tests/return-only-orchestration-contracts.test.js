...
test('Phase 3 uses one return-only planner and foreground planning roles', () => {
  ...
  assert.match(files.phase3, /schema-data-model-semantic-result\.json/);
  assert.match(files.phase3, /compile-data-model-semantic-result\.js/);
  assert.match(files.phase3, /data-model-compilation-receipt\.json/);
  assert.match(files.phase3, /Set `artifacts: \[\]`/);
  assert.match(files.phase3, /data-model-agent-partitions\.js/);
  assert.match(files.phase3, /65,536 UTF-8 bytes/);
  assert.match(files.phase3, /planning:data-model:detail-NNN/);
  assert.match(files.phase3, /repairPartitionIds/);
  assert.match(files.phase3, /data-model-merged\.json/);
  assert.match(files.phase3, /dispatch only `pendingPartitionIds`/);
  assert.match(files.phase3, /--refresh-request/);
  assert.match(files.phase3, /detail-only\s+change rebuilds only detail partitions/i);
  assert.match(files.common, /Screen work orders are unchanged/);
