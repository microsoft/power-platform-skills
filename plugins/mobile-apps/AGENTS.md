...
- ✅ Modernization source acquisition prefers Power Platform Git Integration/current extracted source, then `pac canvas download --extract-to-directory`; a modern local `.msapp` is extracted directly through the bounded `scripts/extract-msapp-source.js` ZIP path. Deprecated `pac canvas unpack --layout SourceCode` runs only after a safely readable archive reports no current `Src/*.pa.yaml`; malformed/unsafe archives never fall through. Packages still lacking current source require open/resave/publish in current Power Apps Studio.
- ✅ Modernization ownership invariant: deterministic code preserves business/data/connector contracts; AI owns all React Native implementation. Deterministic extraction/adaptation/validation preserves business rules, Dataverse reads/writes/relationships, connector/flow operations, workflow control flow/policy, and unsupported gaps. AI builders own screen/workflow/bootstrap TypeScript and native architecture inside those contracts. Do not introduce a deterministic Canvas-to-TypeScript operation emitter.
...
- ✅ Adapted semantic preservation has a separate 100% critical-obligation gate. Component-definition handlers become shared commands with per-instance placement obligations; every source screen, navigation edge, saved-view ownership/security contract, start screen, and high-confidence source design baseline receives a stable `obl-…` ID. Equivalent code uses exact target-file `source-obligation` markers; semantic changes require a target-side `source-deltas.json` entry explicitly approved by the user. Weighted behavior coverage cannot waive these obligations.
- ✅ Modernized runtime architecture separates screen interactions, intent-named shared commands/domain use cases, Expo Router journeys, and backend automation. Deterministic Canvas evidence never becomes a generic handler registry, formula interpreter, or runtime workflow engine.
- ✅ Source design is first-party conversion input. The extractor decodes quoted `App.Formulas`, named color/dimension records, `ColorValue(...)`, and control font frequency into `app.designBaseline`. `/design-system` preserves that identity by default for `faithful`/`modernize`/`repair-modernize`; generic industry presets are used only when no high-confidence source baseline exists.
