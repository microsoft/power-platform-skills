...
14. **Complete-app Metro and evidence** — Apply navigation before builders, build every screen in bounded disjoint waves, and start Metro only after complete-app static and TypeScript validation. Metro `--require-complete-app` rejects skeleton or non-type-safe routes. Native evidence, when available, must record stable dimensions/screen identity and explicitly exclude Metro refresh/disconnect banners, development errors, and host debug chrome. Missing capture is a concern for ordinary prototypes and blocking only when a reference contract requires it.
...
- ✅ Prototype foundations and deterministic navigation are serialized once, then all screens build in bounded disjoint waves; Metro starts only after complete-app validation.
