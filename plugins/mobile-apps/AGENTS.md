...
The Expo template snapshot is distributed with this plugin under `template/` and published from [`plugins/mobile-apps/template`](https://github.com/microsoft/power-platform-skills/tree/main/plugins/mobile-apps/template). `/create-mobile-app` is zero-prerequisite: it copies that bundled snapshot into a new app folder, installs the dependencies itself, then prepares the folder. A template the user materialized with `degit` is still accepted as-is.
...
The Expo template snapshot ships bundled inside this plugin at `template/`. It is synced from `microsoft/power-platform-skills` `main`, `plugins/mobile-apps/template/`. `/create-mobile-app` Step 2a copies it into the new app folder through [`scripts/bootstrap-mobile-project.js`](scripts/bootstrap-mobile-project.js), which only ever writes into a folder that is missing, empty, or already a template, and copies with `COPYFILE_EXCL` so it can never overwrite existing content. Fetching a template over the network (`degit`, `git clone`) is **not** an option: the bundled copy is the only shape [`scripts/prepare-mobile-template.js`](scripts/prepare-mobile-template.js) and its tests assert against, so pulling `main` could hand the preparation logic a template it does not understand. Step 5 then applies these preparation edits:
...
3. **Zero-prerequisite creation** — `/create-mobile-app` owns scaffolding end to end: Step 2a materializes the app folder from the bundled `template/` snapshot and starts a detached `npm install` via [`scripts/install-dependencies.js`](scripts/install-dependencies.js) so it overlaps requirements discovery and the planning gates; Step 5 blocks on that install before preparing the folder. Never require the user to run `degit` or `npm install` first, never copy the snapshot over existing content, and never start a second `npm install` alongside the tracked one.
...
- ✅ `/create-mobile-app` has no prerequisites. It bootstraps the app folder from the bundled `template/` snapshot, installs dependencies in the background during planning, then prepares the folder and runs `npx power-apps init`. A folder the user materialized with `degit` and installed themselves is still accepted unchanged. Install state lives in `<app>/.powernative/dependency-install/` (already git-ignored) so it never pollutes the user's repo.
