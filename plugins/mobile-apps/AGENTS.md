...
6. **Foreground approvals** — The foreground skill owns questions and per-section approvals. Use the question/approval tool actually exposed by the host; requesting a tool in agent metadata does not make it available. Child agents return `NEEDS_CONTEXT` for missing decisions and never ask the user, enter/exit plan mode, or approve their own proposals. If the foreground cannot capture required approval, preserve the pending decision and stop without mutation.
7. **Persisted plan and bounded context** — Keep `native-app-plan.md` as the human-readable source of approved requirements, primary journeys, per-screen UX/navigation contracts, and data/native decisions. Load only the active phase reference and the relevant plan section. Give a screen builder its bounded screen context and actual service/token interfaces, not every pattern or the complete app specification. Existing Dataverse execution/approval contracts remain authoritative; do not introduce duplicate product or preview authorities.
   Approved behavior and explicit brand requirements stay fixed; model-inferred layout is provisional until user approval. Use the [design authority boundary](shared/references/design-planning.md#entry-composition-and-reference-transfer) and static [authoring review](skills/preview-screens/references/intent-authoring.md#authoring-review). Generate/open previews without automatic browser testing or screenshot evidence gates. Functional checks alone are not visual evidence.
...
- ✅ **Foreground approval gates** (data platform + native capabilities + connectors → data model → screen graph → screen specs); architecture approval precedes Dataverse discovery
...
- ✅ `brand/` directory convention: `/design-system` writes compact `brand/design-system.md` decisions, `brand/tokens.ts`, and `_design_preview.html` for the approved intent journey. The component gallery `brand/design-system.html`, style comparisons, and history snapshots are optional. Screen-builders read relevant brand decisions; `## Negatives` remains binding. The foreground applies `skills/design-system/references/tamagui-integration.md` before screen building. Without brand artifacts, use neutral host tokens and task-appropriate composition, not an industry preset.
- ✅ **Host-owned offline runtime** — `/setup-offline-profile` authors Dataverse profile records and writes `offline-profile.json`. The bundled `@microsoft/power-apps-native-offline` package, consumed by `@microsoft/power-apps-native-host`, owns SQLite reads/writes, queued synchronization, reconnect behavior and the status overlay. Do not scaffold duplicate app-owned queues, screens or sync controls. Follow [connectivity intent ownership](shared/references/connectivity-intent-ownership.md).
...
- ✅ `offline-profile-architect` agent follows the existing `mobile-app:` namespace + status-code protocol (`DONE` / `DONE_WITH_CONCERNS:` / `NEEDS_CONTEXT:` / `BLOCKED:`). Read-only — proposes scope; never mutates Dataverse. Mutation lives in `/setup-offline-profile` after its required configuration approval.
