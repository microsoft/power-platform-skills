...
agents/                        ← planners and screen builders
...
8. **Tooling compatibility** — Use `npx power-apps ...` for code-app lifecycle and data-source commands, FlowAgent for Power Automate mutation/read-back, Firebase MCP for `/setup-fcm`, the installed Google Cloud CLI only through `scripts/run-allowlisted-gcloud.js` for `/setup-push-wif`, and Azure MCP for the documented bounded WIF read-back surfaces. The documented tested stable MCP baselines are Firebase MCP package `firebase-tools` 15.27.0 (`15.28.1` is main/unpublished) and Azure MCP GA 2.0.5. In this plugin, Azure MCP is limited to subscription, resource-group, and RBAC inventory; its `keyvault` namespace remains excluded because the available operations can carry secret values. RBAC mutations, Entra resource work, and secret-safe writes remain explicit `az` gaps alongside narrow local identity/token exceptions in [`shared/shared-instructions.md`](./shared/shared-instructions.md). Microsoft Learn MCP is the authoritative source for Microsoft-platform docs.
...
16. **Serial push orchestration** — Push setup does not use background agents. `/setup-fcm` performs Firebase authentication, project/app/config operations, and Android-then-iOS validation serially. For iOS, `/add-push-notifications` invokes `/setup-apple-ios` and then `/setup-apns` synchronously after exact Firebase identity is established. It implements runtime integration directly, then invokes `/setup-push-wif`, FlowAgent authoring, wrapped builds, and physical verification serially while preserving all three immutable Entra registration modes and the two-approval cold bootstrap.
...
- **Privacy:** Mobile Apps sends no prompt, tool input, cwd, path, URL, credential, username, hostname, Dataverse org/tenant ID, or Entra object ID. The dynamic `eventInfo` contains only `invocationSource` (`prompt`, `pretool`, or `checkpoint`), a random per-project `appInstanceId` (or `null` outside a prepared project), optional author-defined checkpoint `additionalInfo` validated as `snake_case` with a 64-character limit, and `appInsightsSelection` (`enabled` or `disabled`) from the `/setup-app-insights` skill.
...
- ✅ Push cloud setup uses the narrowest supported owner surface: `/setup-fcm` owns Firebase project/app selection and SDK config retrieval through the vendor-official Firebase MCP only and has no CLI fallback. `/setup-push-wif` uses the authenticated official Google Cloud CLI only through the checked-in allowlisted wrapper for Google-side WIF operations and Azure MCP for covered Azure read-back.
...
- ✅ Push prerequisite checks are stage-specific and use
  `shared/references/push-tool-readiness.md`. Missing local tools, MCP
  server/tool visibility, authentication/account context, IAM, API/service,
  policy, propagation, and transient read-back failures remain distinct.
  Manual installation/restart/login paths wait for confirmation and rerun the
  exact failed probe. A Firebase Resource Manager read error never implies that
  gcloud must be installed; gcloud is a WIF prerequisite, not a Firebase client
  prerequisite.
