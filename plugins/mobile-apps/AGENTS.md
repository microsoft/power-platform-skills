...
16. **MCP-owner isolation and bounded push parallelism** — Connected MCP servers are main-context owner capabilities; background `Task` agents must not declare or depend on inherited MCP tools. `/setup-fcm` performs Firebase authentication, project/app/config operations, and Android-then-iOS validation serially. `/setup-push-wif` performs gcloud/Azure MCP inventory, approvals, mutations, proof, and `sender-auth.json` creation serially while preserving all three immutable Entra registration modes and the two-approval cold bootstrap. Only the MCP-free `mobile-app:push-runtime-worker` and `mobile-app:push-ios-prerequisites-worker` may run concurrently, at most two. Every worker first receives `operation: preflight` and returns a mutation-free capability result; preflight proves only the local worker contract, never MCP inheritance. The parent assigns disjoint absolute exclusive files, supplies one pre-wave `memory-bank.md` SHA-256, resolves project-relative result paths, parses the literal status plus one `WORKER_RESULT`, groups `NEEDS_CONTEXT` questions, and writes memory once after a drift check. FlowAgent authoring, wrapped builds, and physical verification remain sequential owner boundaries.
...
- ✅ Push cloud setup is **official MCP-first**: `/setup-fcm` owns Firebase project/app selection and SDK config retrieval through the vendor-official Firebase MCP only and has no CLI fallback. `/setup-push-wif` separately prefers gcloud MCP for Google-side WIF operations; after MCP recovery fails, it may use the explicitly approved authenticated Google Cloud CLI only through the checked-in allowlisted wrapper.
