...
7. **Persisted V3 contracts** — `native-app-plan.md` remains the human source of truth. Experience, Context, Journey, neutral Domain, Screen, Foundation, Navigation, and Screen Action contracts carry approved structured intent. Deterministic Experience, Context, and Journey helpers emit provisional evidence only; the planner must finalize them with `decisionOwner: "model"`. Experience is finalized before Domain; Context values/bindings are finalized after prototype fixtures or real schema/connector evidence exists; Journey is finalized from that Context before screen planning. `finalize-context-from-domain.js` never chooses fields: it proves exact fixture JSON pointers/values and restamps Domain metadata. AI owns domain entities, fields, labels, operations, screens, controls, availability conditions, every visible action, and optional icon/badge presentation hints. Deterministic code validates references, maps supported icon intents, proves dynamic badge bindings, resolves navigation and generated service names, compiles typed handlers, and rejects enabled controls without executable bindings. Primary action text can never be hidden. Screen Planner deliberately writes a small schema-v1 Screen Contract with optional product-role/navigation hints plus the separate bounded `.tmp/screen-action-contract.json`; it does not own recipe JSON. Deterministic resolution projects model-owned primary action identities onto the normalized screens, rebinds Journey to actual screen IDs, synthesizes critical flow, preserves valid optional hints, and materializes the schema-v2 graph without removing declared jobs. `compile-screen-build-pack.js` owns additive composition guidance: domain-neutral profiles, structural roles, interaction patterns, and advisory recipe anatomy. Missing guidance or an equivalent implementation never blocks and falls back to the Screen Contract plus Domain layout decisions. `.tmp/route-manifest.json` tracks every screen's owner, actions, parameters, operations, reachability, build state, and evidence state. A screenshot/HTML input remains optional and never selects an industry preset.
    **Prototype content before design:** `/create-mobile-prototype` generates and validates typed data before automatic design. `compile-design-content-projection.js` deterministically bounds the validated domain fixtures into `.tmp/design-content-projection.json`; automatic design must read it and record it in design-context evidence. Never parallelize these two phases or substitute generic examples when the projection is missing.
