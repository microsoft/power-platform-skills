...
10. **Plugin isolation** — The one registered PostToolUse hook is the QG static AST gate. It must remain fail-open for every path except `.tsx` under `app/` or `src/components/` of a directory containing the standalone mobile template markers; plugin source and unrelated workspaces are excluded. Other mobile validators remain explicitly dispatched through `scripts/validate-mobile-files.js`.
