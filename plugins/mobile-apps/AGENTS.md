...
- ✅ Firebase native app setup is idempotent by Android package name and iOS bundle identifier. Validated client configs live in committed `firebase/` files and are auto-discovered by Expo config; APNs `.p8` upload remains a manual Firebase Console step and the agent never handles the key.
