...
- ✅ Notification privacy guidance is advisory for business content. The skill explains lock-screen and device payload exposure plus topic-authorization limits, but the user chooses title, body, additional string data, and navigation parameters. Credentials, tokens, private keys, authorization headers, and secret values remain prohibited.
