...
//   1  usage error, or the plan has no readable ## Pages table, or more than one with a File column
...
function checkPageFiles({ planPath, workingDir }) {
  ...
  if (unreadable) {
    return { exit: 1, result: { ok: false, error: `${abs} has table rows in a Pages section that are not its File table's rows (rows ahead of its header, a second delimiter row, a table with no File column beside it, or two File columns), so which files they name cannot be read — re-plan with the one table` } };
  }
  ...
  if (tables.length > 1) {
    return { exit: 1, result: { ok: false, error: `${abs} has ${tables.length} Pages tables with a File column, so which one the pages come from is ambiguous (a table quoted in the requirements counts too) — re-plan with exactly one` } };
  }
  const files = tables.length ? tables[0] : null;
