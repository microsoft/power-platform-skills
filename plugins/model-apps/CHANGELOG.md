...
- **A table key the build cannot honour now fails instead of being dropped** ([#537]). `entities[]`
  ...
  were unsupported at this version — **2.7.0 adds them** as an LCID map on the label field itself, so
  the `entities[].localizedLabels` *block* stays rejected while the capability it asked for now
  exists. The same check runs
