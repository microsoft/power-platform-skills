...
- **`/app-builder` is covered by the v2.3 hooks + telemetry.** Tracked-skill discovery is derived from
  ...
  `/app-builder` session: session detection accepts the app-builder working-dir markers
  (`app-spec.json`, `model-app-plan.md`) alongside `genpage-plan.md`, so the **write-safety** guard
  covers app-builder's `generate-pages` phase (which dispatches parallel page-builder workers — the same
  runaway-sub-agent write risk), and the **icon-import** allowlist opts in app-builder's generated pages
  by sibling marker as well as by the `export default GeneratedComponent` content marker.
