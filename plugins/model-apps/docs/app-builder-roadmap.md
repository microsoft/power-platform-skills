...
Source: `IMPROVEMENTS-07-15-app-builder.md` (Project Management V1/V2 diff + a sample data-load). Re-verified against current code 2026-07-27 — all still open; **line numbers in that doc have drifted** but the substance holds, and two items now have partial progress. Severity from the source doc.
- 🔲 **[High] Validate lookup binds; stop silent data-load lookup failures** — a sample/data-load `@odata.bind` against a non-existent lookup nav-property fails silently (run reports success, lookup left unset). Validate every lookup nav-property against **live entity metadata** before load, **surface bind failures as errors** (not silent skips), and adopt **stable lookup schema names** so specs/data scripts are portable across envs. (Today `resolveSampleRecords` resolves from the spec, not live metadata; the SDK's `seedRecordGraph` owns bind formation with no nav-prop pre-validation — `entity-provision.js` buildSeedGroup/provisionSampleData.)
- 🔲 **[Medium] Don't truncate parent lookups in default-view enrichment** — `defaultViewColumns` caps at `DEFAULT_VIEW_MAX_EXTRA = 6` (`sdk-build.js:354-379`). **Partial since 07-15:** Gap 6 now *includes* parent lookups in the default view, but they're added after scalar columns under the **same cap**, so a lookup-heavy table (≥6 scalar columns) can still drop the last parent lookup. Prioritize lookups over/within the cap (or raise it for lookup-heavy tables).
- 🔲 **[Medium] Normalize N:N relationship schema-name ordering** — `manyToManySchemaName` → `prefixedRelationshipName` composes `<entity1>_<entity2>` in **spec declaration order** (`app-spec.js:165-199`), so the same N:N comes out reversed across authorings and breaks a data-load assuming one order. Sort the two entity logical names alphabetically when composing the name for cross-spec stability.
- 🔲 **[Medium] Resolve Choice values from live metadata; lint sample data** — option-set integer values are **positional** and not stable across envs/generations, so a data-load hardcoding an int can hit the wrong option. Resolve Choice/MultiChoice **labels → ints from live metadata at load time** (never hardcode) and **lint** that sample/load data references only declared option labels. (Today `resolveSampleRecords` maps labels via the spec's positional option ints — `app-spec.js:206-234`.)
- 🔲 **[Medium] Sub-grid placement + titling** — **Partial since 07-15:** opt-in `forms[].autoSubgrids` now adds a sub-grid per child relationship (`sdk-build.js:954-977`), but (a) the label still defaults to the child entity's **logical name** (`sg.label || sg.childEntity`, `:976`) — default it to the child's display/collection name from metadata — and (b) sub-grids are not given their **own 1-column, full-width section** so they span the form.
- 🔲 **[Medium] Handle the stock "Information" form** — the build marks its own spec-named form default (`promoteDefaultForm`) but **deliberately leaves the stock "Information" form active** (`sdk-build.js:855`), so a second competing main form ships active. Give the spec control: **deactivate** the Information form, or **reuse / prompt for** the existing form instead of creating a second. (Design decision; pairs with in-place form-edit work.)
- 🔲 **[Low–Med] Drop "Created On" from enriched default views** — enrichment **unions** columns into the stock default views (which ship with `createdon`) and never removes it, so Created On persists on every table's default views. On a fresh build, drop `createdon` (or make the enriched set replace rather than union).
- 🔲 **[Medium] Auto-create a Quick Create form for key tables** — the build authors only **main** forms, so the inline **"+" quick-create** (from a lookup or a sub-grid *+ New*) has no Quick Create form and comes up empty, even though `formType: "QuickCreate"` is supported. Auto-generate a Quick Create form (primary + required + parent lookups) for lookup-target / sub-grid-child tables and enable "Allow quick create" on them.
