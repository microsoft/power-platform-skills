...
- 🔲 **#3 execution — run only the changed phases on `--apply` (relieves #7 build-invocation overhead).** The `phase-diff` foundation is in; the remaining work is a SAFE partial-apply. Today the I1 gate forbids a partial `--apply` because phases don't all self-hydrate cross-phase dependencies (e.g. the pages phase HALTS unless app-shell ran in the same invocation — `sdk-build.js` `pages-requires-app`). Plan: (a) audit each phase to hydrate its dependencies via deterministic discovery (app by unique name, tables by schema name); (b) make `diffPhases` dependency-close the changed set; (c) allow that closed set through I1 behind `--changed-only`; (d) live-validate (page-only edit → runs just app-shell+pages+publish). Needs the same live-test rigor as the 2026-07-15 fixes before it's on by default.
