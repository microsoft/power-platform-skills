...
- 🔄 **#3 execution / #7 — SAFE `--changed-only` partial apply.** Design is **Sol-approved (7 adversarial review rounds, v1 *unsafe* → v7 *safe-to-implement*)**; the canonical spec is [`docs/changed-only-design.md`](./changed-only-design.md). Because the build engine is ADDITIVE-not-convergent (existing chart/command/dashboard/web-resource-content defs are skipped on rebuild), the partial apply is **fail-closed**: an identity-bound atomic snapshot (schema 3, live-orgId/app/env match), the four proven-convergent shapes only (page-byte replace, placement-exact form field-set, append-only view columns, sitemap-only app-shell), a durable eligibility state machine + sticky debt + teardown tombstone, phase submodes with phase-local publish, and EXACT projection verifiers after apply. Sequenced deliverables (see design doc): **①✅ projection/verifier framework (`scripts/lib/projection.js` + 3 adversarial tests)** → **②✅ content-hash + `phase-diff` `.tsx`-invisible fix (`scripts/lib/hash.js` + `content-hash.js`; the diff now sees `.tsx`/`contentPath` byte edits, fail-closed)** → ③ envelope+lease+CAS+tombstone → ④ submodes+drift+phase-local publish → ⑤ classifier (✅ core `classify-changes.js`)+flow → ⑥ tests+eval → ⑦ Sol impl review → ⑧ live regression (aurorabapenv03468) → ⑨ docs. Needs the same live-test rigor as the 2026-07-15 fixes before it's on by default.
