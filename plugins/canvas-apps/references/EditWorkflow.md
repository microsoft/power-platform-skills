...
approved edit plan and original request using the `Original Requirements Contract`
template in `${PLUGIN_ROOT}/references/PlanTemplates.md`. Include every requested edit
clause and every existing behavior made regression-sensitive by the edit. This artifact
is orchestrator-owned and the planner must not edit it.
