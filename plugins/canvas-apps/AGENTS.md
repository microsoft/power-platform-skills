...
references/
  ...
  QAChecks.md                  ← 33 runtime anti-pattern checks for per-screen self-QA
...
| `canvas-screen-builder` | `canvas-app` | Creates or modifies exactly one screen from its shared plan and screen brief, then reports all 33 self-QA outcomes. Builders run in waves of at most three; `canvas-app` owns compilation. |
