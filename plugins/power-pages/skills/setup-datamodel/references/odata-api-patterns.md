...
Azure CLI tokens expire after ~60 minutes. Refresh before each major step:
...
| 400 | Bad request (malformed JSON) | Fix the JSON body and retry |
...
| 404 | Entity not found | Table/column doesn't exist (expected for pre-creation checks) |
| 409 | Conflict (duplicate) | Table/column already exists — skip |
...
For transient errors (401, 429, 5xx), use this pattern:
