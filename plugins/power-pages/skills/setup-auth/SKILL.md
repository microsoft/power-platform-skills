...
- **User profile display**: After login, the auth service's `getUserDisplayName()` falls back through `firstName + lastName` → `firstName` → `userName` → `email` → `'User'`. Only `contactId` and `userName` are guaranteed to be populated. `firstName`/`lastName`/`email` depend on the provider's `RegistrationClaimsMapping` site setting being configured (e.g., `firstname=given_name,lastname=family_name,emailaddress1=email` for Entra External ID) AND the IdP actually emitting those claims.
