...
- **403 Forbidden on `/_api/` calls**: "Check the following:\n  1. **Table permissions** — Ensure a table permission exists for this table with the correct scope and privileges (Read, Write, etc.) assigned to the appropriate web role.\n  2. **Site settings** — Verify `Webapi/<tablename>/enabled` is set to `true` and `Webapi/<tablename>/fields` explicitly lists every required ordinary column by exact Dataverse LogicalName. Lookup columns use `_<LogicalName>_value`. For aggregate OData (`$apply`, aggregate, grouped totals), include every grouping key, aggregate input, filter column, and ordered column.\n  3. **Web role assignment** — Confirm the authenticated user has the correct web role assigned."
