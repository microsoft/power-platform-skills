...
Guide the user through creating a complete, production-quality Power Pages code site from initial concept to deployed site. Follow a systematic approach: discover requirements, plan components, scaffold, implement with design applied from the start, validate, review, and deploy.
...
- **Design from the start**: Apply the chosen aesthetic (typography, colors, motion, backgrounds) from the moment code is written — never build with neutral styling first and redesign later. Use `${CLAUDE_PLUGIN_ROOT}/skills/create-site/references/design-aesthetics.md` for guidance.
...
**Goal**: Determine what pages, components, and design elements the site needs
...
5. Use best judgement to determine placeholder values based on the chosen aesthetic + mood:
...
**Goal**: Get user approval on the full implementation plan
...
2. Present the implementation plan directly to the user as a formatted message. **The plan MUST have ALL of the following sections:**
   ...
   **Section A — Project Setup**
   - Framework template and project configuration
   - Git repository initialization
   - Install dependencies and launch dev server
   - **Project setup must complete before design and pages begin**

   **Section B — Design & Pages**
   ...
   **Section C — Review & Deployment**
   - What to verify before handoff
   - Deployment options

   > **CRITICAL:** The plan is written for the user — do NOT reference internal phase numbers, tool names, or implementation details. Describe what will be built and what it will look like. Section A must complete before Section B begins.
...
## Phase 4: Scaffold & Launch Dev Server

**Goal**: Create the project from template, initialize git, install dependencies, and start the dev server
...
After copying, replace all `__PLACEHOLDER__` tokens in every file with the actual values determined in Phase 2. Use `Edit` with `replace_all: true` on each file.
...
### 4.6 Start Dev Server

**This MUST happen now — before any customization code is written.** The dev server enables the live preview feedback loop that makes Phase 5 work:
...
### 4.7 Verify in Playwright & Share URL
...
> **GATE: Do NOT proceed to Phase 5 until ALL of the following are true:**
...
> **Prerequisite:** The dev server MUST already be running and verified via Playwright. If it is not, go back and complete Phase 4.
>
...
- **One todo for design foundations** — "Apply global design tokens (fonts, colors, motion, backgrounds)"
...
1. **Design foundations** — Apply Google Fonts, CSS variables, motion/animation utilities, and background treatments globally
...
The user is previewing in their own browser via the dev server URL shared in Phase 4.7.
...
2. After Phase 3: Approve implementation plan
...
| Plan site components | Planning components | Determine pages, components, design direction, and routes |
| Approve implementation plan | Getting plan approval | Create implementation plan, get user approval |
| Scaffold and launch dev server | Scaffolding project | Copy template, replace placeholders, git init, npm install, start dev server |
| Implement pages and components | Building site | Create all pages, components, routing, navigation with design applied |
...
### Phase 3: Plan Approval
- Plan presented inline with all sections (setup, design & pages, review & deployment)
- User approved via AskUserQuestion

### Phase 4: Scaffold & Launch
- React template copied, placeholders replaced
...
- Built in order: design tokens → shared components → pages → router → nav
