...
function preflight(plan) {
  ...
  const snapshot = captureSite(plan.siteRoot);
  const hashes = new Map(snapshot.context.files.map((file) => [file.path, file.hash]));
  const capturedHash = (relative) => hashes.get(relative) ?? null;
  const changed = plan.writes.filter((write) => write.before !== write.after);
  const applied = changed.length > 0 && changed.every((write) => capturedHash(write.path) === write.afterHash);
  ...
  if (applied) {
    ...
    const actualPaths = snapshot.context.files.map((file) => file.path);
  ...
  for (const input of plan.inputs) {
    ...
    if (capturedHash(input.path) !== expected) throw new Error(`Source changed since proposal preparation: ${input.path}. Regenerate and obtain approval again.`);
  }
  for (const write of plan.writes) {
    const expected = applied ? write.afterHash : write.beforeHash;
    if (capturedHash(write.path) !== expected) throw new Error(`Conflicting local edit: ${write.path}. No changes applied.`);
  }
  if (!applied) {
    ...
    const rebuilt = preparePlanFromSnapshot(snapshot, plan.request, plan.allocatedIds);
...
function applyPlan(plan, options = {}) {
  ...
  if (applied || changed.length === 0) {
    ...
    return { status: receipt.status, receipt: receiptPath, files: [], warnings };
  }
  const staged = [];
  try {
  ...
  return { status: receipt.status, receipt: receiptPath, files: receipt.completed, warnings };
}
