...
|---|---|---|
| [Manage CSS files](https://learn.microsoft.com/en-us/power-pages/configure/manage-css) | Recommends Studio for out-of-box controls; preserves default CSS records/order and the `theme.css < custom < portalbasictheme.css` band | This is a recommendation, not a VS Code prohibition or exclusive ownership rule. Its older 3.3.x-only note conflicts with the dedicated Bootstrap overview |
