...
Read for a new visual treatment, palette/background, typography, or readability fix. Skip for isolated spacing/radius changes. Adapt the cohesive palette, hierarchy and spacing principles in [create-site's design reference](../../create-site/references/design-aesthetics.md); **do not execute that SPA workflow** or generate a preview.

## Design the complete treatment

- Match the existing brand and the user's direction. "Modern" does not automatically mean a dark hero, gradient, huge radius or oversized empty section. Preserve the site aesthetic unless a redesign was requested.
- Choose coordinated roles: surface, heading, body, muted text, link, button surface/text, and focus indicator. A background change includes all text and interactive states on that background, not just the container.
- Reuse existing fonts, tokens and Bootstrap spacing/grid conventions where suitable. Use coherent spacing, readable line lengths and heading/body hierarchy; keep short sections content-sized. Do not import the SPA reference's font bans/downloads, theme.css rewrite or dependency installation. General CSS fonts, tokens, animation and explicit global themes are supported through the reviewed classic-site contract, not the SPA workflow.
...
Inspect the winning foreground of each relevant child and its effective surface, not just the parent color or a screenshot. Normal text needs **4.5:1**; large text needs **3:1** (at least 24 CSS px regular, or 14pt = 18.667 CSS px at weight 700+). Eighteen-pixel bold is not large text. Ratios must meet the threshold **without rounding**. Do not enlarge text merely to pass.
