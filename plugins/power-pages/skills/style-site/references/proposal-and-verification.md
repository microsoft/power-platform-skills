...
- Keep complete local baseline CSS evidence. Missing referenced CSS or unknown/conflicting Bootstrap still blocks preparation; never substitute Bootstrap or fetch a CDN. Display-order metadata does not determine CSS priority or eligibility.
- Inspect applicable Web Files by parent/descendant scope, plus the selected page's CSS sidecar. The inspector's path-sorted CSS inventory is not runtime load order; its `order` field is literal metadata only.
...
Prefer an existing `targetId`. For **both existing and new custom Web File changes**, retain the advisory note in review, approval and the final report: custom CSS has higher priority than `theme.css` and lower priority than `portalbasictheme.css`. This is guidance only, not a `displayorder` comparison or a requirement to prove runtime load order. Missing, equal, adjacent or otherwise different metadata values do not block preparation.

New shared CSS uses verified parent/publication/attachment metadata and omits `displayorder`; do not allocate slots, copy an exemplar's order or renumber existing files. Unknown required schema/relationships or collisions **hard-stop**; never invent YAML, move defaults, or bypass preparation. New files for different site/section destinations must use **distinct CSS filenames** when they would resolve to the same physical path, even if their parent-page URLs differ. Multiple style groups can intentionally reuse one verified stylesheet; they cannot create competing records/assets at that path. Explicitly requested Studio styles produce handoff placements, not local writes or class additions.
...
Imported CSS requires `global: true`, declared external resource URLs when external, and valid beginning-of-stylesheet `@import` placement. Inspect existing CSS statement placement; a new custom CSS file can avoid a late import, but still requires valid identity/relationship metadata and the Web File priority note. Do not automatically fetch imports; review the imported source and any transitive resource/scope effects from available evidence, or disclose what remains unverified. Approval never authorizes silently adding later imports or URLs.
