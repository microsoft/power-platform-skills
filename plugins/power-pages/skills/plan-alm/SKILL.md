...
| `NoHost` | **Env-first prompt** — same shape as `ensure-pipelines-host` Phase 3.C so the user makes the host choice once, here, instead of being asked again at execution time. Build the eligible-env list from `HOST_RESOLUTION.candidates.eligibleForAppInstall[]` with role labels (`dev env`, `source env`, `staging env`, `production env`) per origin match. **Cap the visible list at 5 envs** with role-aware ranking (see "Eligible-env presentation cap" below) — `AskUserQuestion` becomes unusable past ~7-8 options, and tenants with many envs would otherwise dump 30+ options into a single prompt. Present: *"No Pipelines host bound to `{devEnvUrl}`. Which environment should host Pipelines? Pipelines lives in one env per tenant; pipelines, stages, and run history are stored there. Source envs deploy through it."* Options: 1. Each visible env (display name + URL + role labels) labeled "*Install Pipelines app on this env*" — sandbox-sku envs add a "(Sandbox — confirmation gate)" suffix; **append "Other (paste URL)" inside option 1 as the last per-env entry** (long-tail escape hatch when the user has more than 5 eligible envs); **N+1.** "Create a brand-new dedicated host env (D365_ProjectHost template, ~5–10 min, requires Power Platform admin)"; **N+2.** "Open PPAC and create one manually (admin fallback)"; **N+3.** "Switch to Manual export/import strategy"; **N+4.** "Cancel". When the eligible list is empty, drop the per-env options and present the four create/switch/cancel options. | Picked eligible env → `HOST_ENV_URL = picked.instanceApiUrl`, `CHOSEN_ENV_URL = picked.instanceApiUrl`, `WILL_ENSURE_HOST = true`, `WILL_PROVISION_CUSTOM = false`. (Sandbox confirmation gate, if applicable, must be passed before this resolution stands.) "Other (paste URL)" → ask for the env URL via free-text, set `HOST_ENV_URL = CHOSEN_ENV_URL = pasted URL`, then proceed as a picked eligible env. Create-new (N+1) → `HOST_ENV_URL = null`, `WILL_PROVISION_CUSTOM = true`, `WILL_ENSURE_HOST = true`. PPAC manual (N+2) → `HOST_ENV_URL = null`, `WILL_USE_PPAC = true`, `WILL_ENSURE_HOST = true`. Manual strategy (N+3) → restart Phase 2 with `STRATEGY = manual`. Cancel (N+4) → exit. |
