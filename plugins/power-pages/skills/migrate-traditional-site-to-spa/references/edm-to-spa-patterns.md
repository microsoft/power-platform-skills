...
5. **Wire the profile route into the AppShell navigation as a real `<Link>`.** The signed-in user's nav (typically the `AuthButton` or a user menu produced by `/setup-auth`) must link to the profile route using the framework's navigation component — `<Link to="/profile">` for React Router, `<router-link :to="/profile">` for Vue, `<a routerLink="/profile">` for Angular, `<a href="/profile">` for Astro. A static `<span>` or `<div>` that *displays* the user's name without being a navigation element does not count — the validator's `gate-route-reachability` will fail it as `blocker`. The same rule applies to every other authenticated/role-gated route: it must be reachable from an always-visible component, not just by typing the URL.
