...
// Reads docs/alm/last-host-check.json from the project and verifies the skill ran
...
runValidation((cwd) => {
  ...
  const markerPath = almPath(projectRoot, 'lastHostCheck');

  if (!fs.existsSync(markerPath)) return approve(); // Not an ensure-host session.

  let marker;
  try {
    marker = JSON.parse(fs.readFileSync(markerPath, 'utf8'));
  } catch {
    return block('docs/alm/last-host-check.json exists but could not be parsed as JSON.');
  }

  if (marker.schemaVersion !== 1 && marker.schemaVersion !== 2) {
    return block(`docs/alm/last-host-check.json has unsupported schemaVersion: ${marker.schemaVersion}. Expected 1 or 2.`);
  }
  if (!marker.tenantId) {
    return block('docs/alm/last-host-check.json is missing required field: tenantId');
  }
  if (!marker.sourceEnvUrl) {
    return block('docs/alm/last-host-check.json is missing required field: sourceEnvUrl');
  }
  if (!marker.resolutionStatus) {
    return block('docs/alm/last-host-check.json is missing required field: resolutionStatus');
  }
  if (!VALID_STATUSES.has(marker.resolutionStatus)) {
    return block(`docs/alm/last-host-check.json has unknown resolutionStatus: ${marker.resolutionStatus}`);
  }
  ...
  if (marker.ready !== true) {
    return block(`docs/alm/last-host-check.json has ready=${marker.ready} but resolutionStatus "${marker.resolutionStatus}" requires a usable host. The skill did not complete successfully.`);
  }

  // Sanity: when ready=true, finalHostEnvUrl must be set.
  if (!marker.finalHostEnvUrl) {
    return block('docs/alm/last-host-check.json has ready=true but finalHostEnvUrl is missing.');
  }
