...
description: >-
  ...
  SPA. Inspect SharePoint through a signed-in browser. Use for SharePoint-to-Power
  Pages migration or acceleration.
user-invocable: true
...
| Exposure | Intended fields, records, documents, page sections, visible metadata, exclusions, sensitive content, audience, and content owner. Exact schema choices may depend on discovery. |
...
**Once `PROJECT_ROOT` is confirmed, read `${PLUGIN_ROOT}/references/sharepoint-artifacts.md` and initialize its four HTML artifacts in the site's `docs/` folder.**
...
Use the reference's artifact aliases: **requirements**, **discovery**, **plan**, and **progress**.
...
**Complete when:** The brief and local project are approved, all four artifacts exist, and each remaining unknown has a recorded dependency.
...
> 🚦 **Gate (plan - sharepoint-to-power-pages:4.select-lists):** Choose which SharePoint lists the portal will expose.
...
Use `AskUserQuestion` with **multi-select**: "Which lists should Power Pages expose?"
...
|---|---|
| List | Selected source site/list identity to a Dataverse virtual table, exposed fields, connection owner, audience relationship, permitted operations, and provider restrictions. |
...
| Page sections and branding | React layout/tokens, approved assets, and authenticated runtime storage for source text; a page-content copy is separate from the list importer. |
...
Explain that virtual tables keep list records in SharePoint rather than importing a second copy.
...
The gate below approves both the exposure mapping and this implementation plan; it replaces a duplicate create-site plan-approval question.

<!-- gate: sharepoint-to-power-pages:4.exposure-plan | category=plan | cancel-leaves=approved-migration-state -->

> 🚦 **Gate (plan - sharepoint-to-power-pages:4.exposure-plan):** Approve the content-to-audience mapping and implementation scope.
...
| Virtual lists, when connected | Every selected list has its own verified source-to-virtual-table mapping; provider metadata, selected columns, query results, and connection behavior match the approved plan without copying source rows. |
...
| Runtime source access | Virtual-list reads use the approved connection; source availability, provider limits, and failures are explicit. Any separate page-copy/refresh work has its own verification status. |
...
They still block any claim that data migration or authenticated runtime access is complete.
...
- **Lists:** Documented maker import, then `/power-pages:integrate-webapi` for verified tables, fields, operations, and audience scopes.
...
- **Page-content copy, refresh, or write-back:** Separately approved supported integration; these are not list-importer capabilities.
