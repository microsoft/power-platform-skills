...
const NEXT_STEP = {
  ...
  Dirty: {
    cmd: '/power-pages:git-sync --dry-run',
    desc: 'Run pre-flight validators on the pending Changes (dry run), then commit via /power-pages:git-sync.',
    alts: ['/power-pages:git-sync --commit', '/power-pages:revert-workspace'],
  },
  Stale: {
    cmd: '/power-pages:git-sync --pull',
    desc: 'Pull the incoming Updates from the bound branch into this environment.',
    alts: [],
  },
  Mixed: {
    cmd: '/power-pages:git-sync',
    desc: 'Recommended order: pull updates first (produces one merged commit), then commit your local changes on top. /power-pages:git-sync handles the ordering.',
    alts: ['/power-pages:git-sync --commit-then-pull'],
  },
  Conflicted: {
    cmd: '/power-pages:git-sync',
    desc: 'Conflicts gate both directions. /power-pages:git-sync detects them and walks through each conflict (Keep environment OR Accept incoming); sync resumes after all conflicts are resolved.',
    alts: ['/power-pages:revert-workspace'],
  },
  Broken: {
