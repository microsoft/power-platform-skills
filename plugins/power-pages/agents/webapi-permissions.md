...
Each table permission file has this format:
...
adx_create: true
adx_delete: false
adx_entitylogicalname: cra5b_order
adx_entitypermissionname: Order - Authenticated Access
adx_read: true
adx_scope: 756150004
adx_write: false
```
...
adx_create: true
adx_delete: false
adx_entitylogicalname: cra5b_orderitem
adx_entitypermissionname: Order Item - Authenticated Access
adx_parententitypermission:
  adx_entitypermissionname: Order - Authenticated Access
adx_parentrelationshipname: cra5b_order_orderitem
adx_read: true
adx_scope: 756150003
adx_write: false
```
...
adx_create: false
adx_delete: false
adx_entitylogicalname: <table_logical_name>
adx_entitypermissionname: <Display Name - Role Access>
adx_read: true
adx_scope: <scope_code>
adx_write: false
```
...
adx_create: true
adx_delete: false
adx_entitylogicalname: <child_table_logical_name>
adx_entitypermissionname: <Child Table - Role Access>
adx_parententitypermission:
  adx_entitypermissionname: <Parent Table - Role Access>
adx_parentrelationshipname: <relationship_schema_name>
adx_read: true
adx_scope: 756150003
adx_write: false
```
...
- Fields must be in the exact order shown above
...
1. Write a temporary HTML file (e.g., `permissions-diagram.html` in the project directory):
...
2. Navigate Playwright to the file using a `file:///` URL:
   - On Windows: `file:///C:/path/to/permissions-diagram.html`
   - Convert backslashes to forward slashes in the path

3. Wait for the diagram to render (wait ~3 seconds for Mermaid to process).

4. Take a screenshot using `browser_take_screenshot` — this captures the rendered diagram.
...
After the user approves the plan, delete the temporary `permissions-diagram.html` file if it was created.
...
4. **Files to Create** — Complete list of files with their full YAML content (using placeholder `<GENERATE-UUID>` for IDs that need to be generated)
...
- **No file writes**: Do not use `Write` tool for any YAML files in `.powerpages-site/`. The only file you may write is the temporary `permissions-diagram.html` for visualization.
