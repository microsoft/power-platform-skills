...
A `user-deferred` manifest entry downgrades any dependent check (e.g., profile route stays stubbed because `/setup-auth` is deferred) from `fail` to `deferred`. Deferred checks contribute to a `Partial` verdict, not `Blocked`.
