...
- `scripts/lib/alm-thresholds.js`: Central default thresholds for the split decision tree. Loads optional `.alm-config.json` from project root and merges over defaults. Exports `DEFAULTS`, `loadConfig`, `classifyTier`, `deepMerge`.
- `scripts/lib/estimate-solution-size.js`: Queries Dataverse and estimates solution size + component counts. Args: `--envUrl`, `--websiteRecordId`, `--token`, `--publisherPrefix`, `--siteName`, `--datamodelManifest`. Output includes `totalSizeMB`, `componentCount`, `schemaAttrCount`, `webFilesAggregateMB`, `webFilesIndividual[]`, `breakdown{}`. Metadata-based with ±15% caveat.
- `scripts/lib/compute-split-plan.js`: Runs the decision tree. Args: `--estimate`, `--projectRoot`, `--siteName`, `--publisherPrefix`. Output: `{ sizeAnalysis, assetAdvisory, splitStrategy, appliedStrategies, proposedSolutions[], recommendations[] }`. Strategy priority: Schema Segmentation → Layer → Change-Frequency → Config Isolation (additive). Supports `strategyOverride` in `.alm-config.json`.

#### Solution Splitting Decision Tree (v1.3.0+)

- `scripts/lib/alm-thresholds.js`: Central default threshold constants for the split decision tree. Loads optional `.alm-config.json` from project root and merges over defaults. Exports `DEFAULTS`, `DEFAULT_CONFIG`, `loadConfig(projectRoot)`, `classifyTier(value, green, yellow)`, `deepMerge(target, source)`. Used by `estimate-solution-size.js` and `compute-split-plan.js`.
