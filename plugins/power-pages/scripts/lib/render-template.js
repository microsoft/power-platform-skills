 ...
 * @param {boolean} [options.emitStatus=true] - Disable when a wrapper publishes the final output path.
 */
function renderTemplate({ templatePath, outputPath, dataPath, dataObject, requiredKeys, emitStatus = true }) {
  ...
  if (emitStatus) console.log(JSON.stringify({ status: 'ok', output: outputPath }));
}
