...
test('completion: a refresh that leaves every non-skip step completed flips PLAN_STATUS to Completed', (t) => {
  const root = makeProject(t);
  writeJson(path.join(root, 'docs', '.alm-plan-data.json'), {
    SITE_NAME: 'T', STRATEGY: 'pp-pipelines', PLAN_STATUS: 'In Execution',
    validationRuns: { Staging: null },
    steps: [
      { name: 'Setup solution', status: 'completed' },
  ...
  // test-site flips the last pending step -> all done -> plan completes.
  writeJson(path.join(root, 'docs', 'alm', 'last-test-site.json'), { runOutcome: 'passed', runAt: '2026-06-16T00:00:00.000Z' });

  refresh({ projectRoot: root, phase: 'test-site', render: false, stageName: 'Staging' });

  const planData = readJson(path.join(root, 'docs', '.alm-plan-data.json'));
  assert.equal(planData.PLAN_STATUS, 'Completed', 'all steps done -> Completed');
  assert.ok(planData.COMPLETED_AT, 'COMPLETED_AT stamped');
});

test('completion: a still-pending step keeps PLAN_STATUS at In Execution', (t) => {
  const root = makeProject(t);
  writeJson(path.join(root, 'docs', '.alm-plan-data.json'), {
    SITE_NAME: 'T', STRATEGY: 'pp-pipelines', PLAN_STATUS: 'In Execution',
    steps: [
  ...
  writeJson(path.join(root, 'docs', 'alm', 'last-deploy.json'), { stageName: 'Staging', status: 'Succeeded', deployedAt: '2026-06-16T00:00:00.000Z' });

  refresh({ projectRoot: root, phase: 'deploy-pipeline', render: false });

  const planData = readJson(path.join(root, 'docs', '.alm-plan-data.json'));
  assert.equal(planData.steps[0].status, 'completed', 'Staging deploy flipped');
  assert.equal(planData.PLAN_STATUS, 'In Execution', 'Production still pending -> not Completed');
  assert.equal(planData.COMPLETED_AT, undefined);
});

test('completion: a failed step blocks completion even if all others are done', (t) => {
  const root = makeProject(t);
  writeJson(path.join(root, 'docs', '.alm-plan-data.json'), {
    SITE_NAME: 'T', STRATEGY: 'pp-pipelines', PLAN_STATUS: 'In Execution',
    steps: [
  ...
  // A FAILED deploy marker flips the deploy step to 'failed', not 'completed'.
  writeJson(path.join(root, 'docs', 'alm', 'last-deploy.json'), { stageName: 'Staging', status: 'Failed', deployedAt: '2026-06-16T00:00:00.000Z' });

  refresh({ projectRoot: root, phase: 'deploy-pipeline', render: false });

  const planData = readJson(path.join(root, 'docs', '.alm-plan-data.json'));
  assert.equal(planData.steps[1].status, 'failed');
  assert.equal(planData.PLAN_STATUS, 'In Execution', 'a failed step must NOT complete the plan');
});

test('completion: skip:true steps are ignored when deciding completion', (t) => {
  const root = makeProject(t);
  writeJson(path.join(root, 'docs', '.alm-plan-data.json'), {
    SITE_NAME: 'T', STRATEGY: 'pp-pipelines', PLAN_STATUS: 'In Execution',
    validationRuns: { Staging: null },
    steps: [
      { name: 'Deploy via pipeline to Staging', status: 'completed' },
      { name: 'Test site in Staging', status: 'pending', skip: true },
    ],
  });
  // Re-run a no-op-ish refresh; the skipped Test step must not block completion.
  writeJson(path.join(root, 'docs', 'alm', 'last-deploy.json'), { stageName: 'Staging', status: 'Succeeded', deployedAt: '2026-06-16T00:00:00.000Z' });

  refresh({ projectRoot: root, phase: 'deploy-pipeline', render: false });

  const planData = readJson(path.join(root, 'docs', '.alm-plan-data.json'));
  assert.equal(planData.PLAN_STATUS, 'Completed', 'only non-skip steps count -> Completed');
});

test('completion: a Draft plan is never auto-completed', (t) => {
  const root = makeProject(t);
  writeJson(path.join(root, 'docs', '.alm-plan-data.json'), {
    SITE_NAME: 'T', STRATEGY: 'pp-pipelines', PLAN_STATUS: 'Draft',
