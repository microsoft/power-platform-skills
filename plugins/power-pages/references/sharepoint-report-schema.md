...
|---|---|
| `version` | `1`. |
| `artifact` | `requirements`, `discovery`, `plan`, `sharing`, or `progress`. The **sharing** record is generated by `scripts/build-sharing-map.js`; do not assemble its model by hand. |
...
| `links` | All five records, each `{ artifact, label, href }`; `href` is the actual neighboring HTML filename, without a directory or URL. A record written before the sharing map existed still loads with four links, but any regeneration must name all five. |
