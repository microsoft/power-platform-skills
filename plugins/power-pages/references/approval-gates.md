...
| `git-configure:1.artifact-path` | gate | plan | 1 | `<workspace>` is not an explicit `--project-root`, a persisted `artifactRoot`, or a real non-pac code-site root you're already inside — i.e. fresh / home-directory runs **and** pac-managed roots both prompt. | Where should this project's workspace live? Both the working clone (`<chosen>/clone/`) and run artifacts (`<chosen>/docs/inner-loop/`) live here. | Use `{suggestedRoot}` (recommended); Choose another directory. One workspace holds clone + artifacts as siblings, and Phase 9 reuses it (no second prompt). Never a `pp-merge-clones` path. `<workspace>` persisted to manifest `artifactRoot`; clone block records `<workspace>/clone`; `--non-interactive` requires `--project-root`. | nothing |
