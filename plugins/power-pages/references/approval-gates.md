...
Deployment delegates activation status and any activation offer to `deploy-site`; data-copy and permission changes remain separately approved.
The suitability and intake sub-prompts collect missing facts without granting write or publication consent.
Once the project folder is confirmed, local HTML artifacts record intake and findings even if a subsequent gate is declined.
Invoked skills retain their own Approval Gates.
