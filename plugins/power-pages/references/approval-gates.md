...
| `setup-auth:2.1.2.provision-idp` | gate | consent | 2.1.2 | *"Run this exact Entra app-registration command now? Yes / No"* - asked immediately before each fixed-allowlist `az ad app create` or `az ad app update` mutation. Approval applies to one command only. Guided setup does not reach this gate. | the last read-back-verified app state |
