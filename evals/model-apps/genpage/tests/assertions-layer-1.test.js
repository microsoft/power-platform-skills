...
test('agent message contract: passes every upload with escaped structured sections', () => {
  const check = WORKFLOW_ASSERTIONS.get('Every pac model genpage upload records --agent-message with escaped # Agent Thoughts, OOB-style Step-by-Step Processing with bold numbered headings on new lines, a bulleted # Summary, and # Final Code sections');
...
test('agent message contract: fails an unbolded numbered heading', () => {
  const check = WORKFLOW_ASSERTIONS.get('Every pac model genpage upload records --agent-message with escaped # Agent Thoughts, OOB-style Step-by-Step Processing with bold numbered headings on new lines, a bulleted # Summary, and # Final Code sections');
  const log = `pac model genpage upload --agent-message "# Agent Thoughts\\nStep-by-Step Processing\\n1. Requirements Analysis\\n- Planned the page.\\n# Summary\\n- Built the page.\\n# Final Code\\n"`;
  const result = check({ fixture: fix({ workflowLog: log }), eval: evalStub() });
  assert.equal(result.status, 'fail');
  assert.match(result.reason, /bold every numbered heading/);
});

test('agent message contract: fails a non-bulleted Summary item', () => {
  const check = WORKFLOW_ASSERTIONS.get('Every pac model genpage upload records --agent-message with escaped # Agent Thoughts, OOB-style Step-by-Step Processing with bold numbered headings on new lines, a bulleted # Summary, and # Final Code sections');
  const log = `pac model genpage upload --agent-message "# Agent Thoughts\\nStep-by-Step Processing\\n\\n**1. Requirements Analysis**\\n- Planned the page.\\n# Summary\\nBuilt the page.\\n# Final Code\\n"`;
  const result = check({ fixture: fix({ workflowLog: log }), eval: evalStub() });
  assert.equal(result.status, 'fail');
  assert.match(result.reason, /Markdown bullets/);
});

test('agent message contract: fails a bold heading without a preceding blank line', () => {
  const check = WORKFLOW_ASSERTIONS.get('Every pac model genpage upload records --agent-message with escaped # Agent Thoughts, OOB-style Step-by-Step Processing with bold numbered headings on new lines, a bulleted # Summary, and # Final Code sections');
  const log = `pac model genpage upload --agent-message "# Agent Thoughts\\nStep-by-Step Processing\\n**1. Requirements Analysis**\\n- Planned the page.\\n# Summary\\n- Built the page.\\n# Final Code\\n"`;
  const result = check({ fixture: fix({ workflowLog: log }), eval: evalStub() });
  assert.equal(result.status, 'fail');
  assert.match(result.reason, /blank line/);
});
